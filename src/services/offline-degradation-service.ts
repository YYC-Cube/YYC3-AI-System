...
export class OfflineDegradationService {
  ...
  private eventListeners: Map<OfflineEventType, Set<OfflineEventListener>> =
    new Map()
  ...
  private async loadConfig(): Promise<void> {
    try {
      const savedConfig = await this.storageService.getSession(
        'offline-degradation-config',
      )
  ...
  private async loadStatistics(): Promise<void> {
    try {
      const savedStats = await this.storageService.getSession(
        'offline-statistics',
      )
  ...
  private handleOfflineOperationFallback(
    ...
    const effectiveStrategy =
      strategy ||
      this.config.featureStrategies[resource] ||
      this.config.defaultDegradationStrategy
    ...
    if (listeners) {
      listeners.forEach((listener) => {
        try {
          ...
          console.error(
            `[OfflineDegradationService] 事件监听器错误:`,
            error,
          )
        }
      })
