...
describe('Search Algorithms', () => {
  ...
  it('should search objects by property', () => {
    const searchByProperty = <T, K extends keyof T>(arr: T[], prop: K, value: T[K]): T | null => {
  ...
  it('should perform merge sort', () => {
    const mergeSort = <T>(arr: T[], compareFn?: (a: T, b: T) => number): T[] => {
      ...
      while (i < left.length && j < right.length) {
        const comparison = compareFn ? compareFn(left[i], right[j]) : left[i] > right[j] ? 1 : -1;
...
describe('Conflict Resolution Algorithms', () => {
  it('should implement last-write-wins strategy', () => {
    ...
    expect(
      lastWriteWins({ value: 'local' }, { value: 'remote' }, { local: 100, remote: 200 })
    ).toEqual({ value: 'remote' });

    expect(
      lastWriteWins({ value: 'local' }, { value: 'remote' }, { local: 200, remote: 100 })
    ).toEqual({ value: 'local' });
  });
...
describe('Utility Algorithms', () => {
  it('should debounce function calls', () => {
    const debounce = <T extends (...args: unknown[]) => any>(fn: T, delay: number): T => {
  ...
  it('should throttle function calls', () => {
    const throttle = <T extends (...args: unknown[]) => any>(fn: T, delay: number): T => {
      let lastCall = 0;
