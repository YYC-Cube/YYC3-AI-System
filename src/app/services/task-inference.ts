...
const TASK_ACTION_VERBS =
  /\b(需要|必须|应该|请|要求|实现|完成|修复|添加|创建|优化|重构|升级|迁移|编写|测试|审查|部署|配置|移除|删除|清理|implement|fix|add|create|build|refactor|optimize|write|test|review|deploy|configure|remove|delete|clean|migrate|upgrade|setup)\b/i;
...
class TaskInferenceEngine {
  ...
  inferTasksFromCode(code: string, language?: string, filePath?: string): TaskInference[] {
    ...
    for (const pattern of TODO_PATTERNS) {
      ...
      while ((match = pattern.exec(code)) !== null) {
        ...
        const keyword = fullMatch.includes('fixme')
          ? 'fixme'
          : fullMatch.includes('bug')
            ? 'bug'
            : fullMatch.includes('hack')
              ? 'hack'
              : fullMatch.includes('xxx')
                ? 'xxx'
                : fullMatch.includes('note')
                  ? 'note'
                  : 'todo';
   ...
   */
  inferTasksFromConversation(messages: Array<{ role: string; content: string }>): TaskInference[] {
    const results: TaskInference[] = [];
