...
class MVPService {
  private readonly SYSTEM_PROMPT = buildSystemPromptWithRules(`You are YYC³ AI, an expert UI component generator.
...
Return a JSON object with:
{
  ...
  async refactor(code: string, instructions: string): Promise<string> {
    const messages: AIChatMessage[] = [
      {
        role: 'system',
        content: 'You are an expert code refactoring assistant. Improve code based on user instructions while maintaining functionality.',
      },
      {
        role: 'user',
   ...
   * Explain code
   */
  async explain(code: string, level: 'beginner' | 'intermediate' | 'advanced' = 'intermediate'): Promise<string> {
    const messages: AIChatMessage[] = [
      {
        role: 'system',
   ...
   * Generate unit tests
   */
  async generateTests(code: string, framework: 'vitest' | 'jest' | 'testing-library' = 'vitest'): Promise<string> {
    const messages: AIChatMessage[] = [
      {
        role: 'system',
