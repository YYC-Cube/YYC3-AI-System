...
class WorkflowExecutor {
    ...
    this.registerExecutor({
      ...
      execute: async (node, context) => {
        ...
        try {
          ...
          throw new Error(
            `脚本执行错误: ${error instanceof Error ? error.message : String(error)}`
          );
        }
      },
    });
    ...
    this.registerExecutor({
      ...
      execute: async (node, _context) => {
        ...
        try {
          ...
          throw new Error(
            `Webhook调用失败: ${error instanceof Error ? error.message : String(error)}`
          );
        }
      },
    });
    ...
    try {
      ...
      if (node.config.continueOnError) {
        ...
        this.addLog(
          ...
          node.name,
          'warn',
          `节点执行失败但继续: ${errorMessage}`
        );
        this.emit('node:warning', {
          executionId: context.executionId,
          nodeId: node.id,
          error: errorMessage,
        });
  ...
  private evaluateCondition(condition: ConditionExpression, context: ExecutionContext): boolean {
    switch (condition.type) {
      case 'equals':
        return (
          this.getOperandValue(condition.left, context) ===
          this.getOperandValue(condition.right, context)
        );
      ...
      case 'greater_than':
        return (
          Number(this.getOperandValue(condition.left, context)) >
          Number(this.getOperandValue(condition.right, context))
        );
