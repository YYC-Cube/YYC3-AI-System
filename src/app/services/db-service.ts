...
const SIMULATED_TABLES: DBTableInfo[] = [
  ...
  {
    name: 'projects', schema: 'public', rowCount: 342,
    columns: [
      { name: 'id', type: 'uuid', nullable: false, primaryKey: true },
  ...
  {
    name: 'files', schema: 'public', rowCount: 8420,
    columns: [
      { name: 'id', type: 'uuid', nullable: false, primaryKey: true },
  ...
  {
    name: 'ai_sessions', schema: 'public', rowCount: 5670,
    columns: [
      { name: 'id', type: 'uuid', nullable: false, primaryKey: true },
      ...
      { name: 'input_tokens', type: 'integer', nullable: false, primaryKey: false, defaultValue: '0' },
      { name: 'output_tokens', type: 'integer', nullable: false, primaryKey: false, defaultValue: '0' },
      { name: 'cost', type: 'decimal(10,6)', nullable: false, primaryKey: false, defaultValue: '0' },
      { name: 'created_at', type: 'timestamp', nullable: false, primaryKey: false, defaultValue: 'now()' },
    ],
  },
]
...
export class DBService {
  ...
  async getTableColumns(connId: string, schema: string, tableName: string): Promise<DBColumnInfo[]> {
    if (this.useRealConnection && this.isConnected(connId)) {
  ...
  async executeQuery(connId: string, sql: string, options?: { limit?: number; offset?: number }): Promise<DBQueryResult> {
    if (this.useRealConnection && this.isConnected(connId)) {
      try {
        const result = await realDBService.execute(connId, sql, options)
        this.queryHistory.push({ sql, timestamp: Date.now(), duration: result.duration, rowCount: result.rowCount })
    ...
    if (sqlLower.startsWith('select')) {
      ...
      result = { columns: ['affected_rows'], rows: [{ affected_rows: affected }], rowCount: affected, duration: Date.now() - start }
    } else {
      result = { columns: ['result'], rows: [{ result: 'Query executed successfully' }], rowCount: 1, duration: Date.now() - start }
    }
  ...
  async dumpDatabase(connId: string, _destPath: string): Promise<{ size: number; duration: number }> {
    if (this.useRealConnection && this.isConnected(connId)) {
      const start = Date.now()
    ...
    return { size: Math.floor(Math.random() * 50000000) + 1000000, duration: 1800 + Math.random() * 500 }
  }

  async restoreDatabase(connId: string, dumpFile: Uint8Array): Promise<{ tablesRestored: number; duration: number }> {
    if (this.useRealConnection && this.isConnected(connId)) {
      const start = Date.now()
  ...
  private generateValue(col: string, index: number): unknown {
    ...
    if (col.includes('role') || col.includes('status')) return ['admin', 'user', 'editor', 'viewer', 'active'][index % 5]
    if (col.includes('created') || col.includes('updated')) return new Date(Date.now() - index * 86400000).toISOString()
    if (col.includes('token') || col.includes('size') || col.includes('cost')) return Math.floor(Math.random() * 10000)
    ...
    if (col.includes('model')) return ['gpt-4o', 'claude-sonnet-4', 'glm-5', 'deepseek-v3'][index % 4]
