    ...
    (set, get) => ({
      ...
      createInstance: (type, config = {}) => {
        ...
        const instance: AppInstance = {
          ...
          isMinimized: false,
          isMaximized: false,
          position: config.position || { x: 100 + existing.length * 40, y: 100 + existing.length * 40 },
      ...
      activateInstance: (instanceId) =>
        set((s) => ({
          ...
          instances: s.instances.map(i => i.id === instanceId ? { ...i, lastActiveAt: Date.now() } : i),
        })),

      minimizeInstance: (instanceId) =>
        set((s) => ({ instances: s.instances.map(i => i.id === instanceId ? { ...i, isMinimized: true } : i) })),

      maximizeInstance: (instanceId) =>
        set((s) => ({ instances: s.instances.map(i => i.id === instanceId ? { ...i, isMaximized: true, isMinimized: false } : i) })),

      restoreInstance: (instanceId) =>
        set((s) => ({ instances: s.instances.map(i => i.id === instanceId ? { ...i, isMinimized: false, isMaximized: false } : i) })),

      moveInstance: (instanceId, position) =>
        set((s) => ({ instances: s.instances.map(i => i.id === instanceId ? { ...i, position } : i) })),

      resizeInstance: (instanceId, size) =>
        set((s) => ({ instances: s.instances.map(i => i.id === instanceId ? { ...i, size } : i) })),

      updateInstanceState: (instanceId, updates) =>
        set((s) => ({ instances: s.instances.map(i => i.id === instanceId ? { ...i, ...updates } : i) })),
  ...
  persist(
    (set, get) => ({
      ...
      updateWorkspace: (workspaceId, updates) =>
        set((s) => ({
          workspaces: s.workspaces.map(w => w.id === workspaceId ? { ...w, ...updates, updatedAt: Date.now() } : w),
        })),
      ...
      removeSessionFromWorkspace: (workspaceId, sessionId) =>
        set((s) => ({
          workspaces: s.workspaces.map(w => w.id === workspaceId
            ? { ...w, sessionIds: w.sessionIds.filter(id => id !== sessionId), updatedAt: Date.now() }
            : w),
        })),
  ...
  persist(
    (set, get) => ({
      ...
      updateSession: (sessionId, updates) =>
        set((s) => ({
          sessions: s.sessions.map(ses => ses.id === sessionId ? { ...ses, ...updates, updatedAt: Date.now() } : ses),
        })),
      ...
      activateSession: (sessionId) =>
        set((s) => ({
          ...
          sessions: s.sessions.map(ses => ses.id === sessionId ? { ...ses, status: 'active' as const } : ses),
        })),

      suspendSession: (sessionId) =>
        set((s) => ({
          sessions: s.sessions.map(ses => ses.id === sessionId ? { ...ses, status: 'suspended' as const } : ses),
        })),
      ...
      updateSessionData: (sessionId, data) =>
        set((s) => ({
          sessions: s.sessions.map(ses => ses.id === sessionId
