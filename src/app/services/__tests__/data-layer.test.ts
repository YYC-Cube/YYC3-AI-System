...
describe('数据层服务测试', () => {
  describe('数据导出服务', () => {
    ...
    it('应该正确计算数据哈希', async () => {
      ...
      const hash = await (dataExportService as unknown as { calculateChecksum: (data: unknown) => Promise<string> }).calculateChecksum({ test: content })
    ...
    it('应该正确执行快速检查', async () => {
      ...
      const mockStorageService = {
        getAllFiles: vi.fn().mockResolvedValue([
          { path: 'test.ts', content: 'test', modifiedAt: Date.now() }
        ])
      }
  ...
  describe('导入导出', () => {
    ...
    it('应该正确导入数据', async () => {
      ...
      const testData = JSON.stringify({
        files: [{ path: '/imported/file.ts', name: 'file.ts', type: 'file', content: 'imported', modifiedAt: Date.now() }],
        versions: [],
        snapshots: [],
        dbProfiles: [],
        keyValue: []
