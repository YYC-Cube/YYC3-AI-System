  ...
  describe('MCP核心协议', () => {
    ...
    describe('协议处理器', () => {
      ...
      it('应该正确处理响应', async () => {
        ...
        const responsePromise = handler.sendRequest(
          transport,
          'test/method',
          { param: 'value' },
          1000
        ...
        const pendingRequests = (handler as unknown as { pendingRequests: Map<string, unknown> })
          .pendingRequests;
      ...
      it('应该正确处理方法未找到错误', async () => {
        ...
        expect((sentMessage as { error?: { code: number } }).error?.code).toBe(
  ...
  describe('MCP服务端', () => {
    ...
    it('应该正确注册资源提供者', async () => {
      ...
      const resourceProvider = {
        list: async () => [{ uri: 'test://resource/1', name: '测试资源' }],
...
describe('MCP系统集成测试', () => {
  it('完整的客户端-服务端交互流程', async () => {
    ...
    server.registerResourceProvider('file://', {
      list: async () => [{ uri: 'file://test.txt', name: '测试文件', mimeType: 'text/plain' }],
      read: async (uri: string) => ({
        uri,
        mimeType: 'text/plain',
        text: '文件内容',
