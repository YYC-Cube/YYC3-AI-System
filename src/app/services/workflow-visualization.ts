...
class WorkflowVisualizationService {
  ...
  calculateMetrics(workflowId: string, snapshots: ExecutionSnapshot[]): WorkflowMetrics {
    ...
    for (const snapshot of workflowSnapshots) {
      const duration =
        snapshot.context.status === 'completed' || snapshot.context.status === 'failed'
    ...
    metrics.averageDuration =
      metrics.successfulExecutions > 0 ? totalDuration / metrics.successfulExecutions : 0;
    ...
    switch (format) {
      ...
      case 'txt':
        return logs
          .map(
            (l) =>
              `[${new Date(l.timestamp).toISOString()}] [${l.level.toUpperCase()}] [${l.nodeName}] ${l.message}`
          )
          .join('\n');
