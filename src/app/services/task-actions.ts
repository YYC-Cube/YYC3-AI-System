...
class TaskActionsService {
  ...
  syncTaskToCode(taskId: string, code: string, filePath: string): string {
    ...
    if (syncInfo && syncInfo.fileId === filePath) {
      ...
      if (lineNumber >= 0 && lineNumber < lines.length) {
        ...
        console.log(
          `[TaskActions] Synced task "${task.title}" status to ${filePath}:${lineNumber + 1}`
        );
        return lines.join('\n');
      }
    }
    ...
    if (taskIndex !== -1) {
      ...
      if (lineIndex >= 0 && lineIndex < lines.length) {
        ...
        console.log(
          `[TaskActions] First sync of task "${task.title}" to ${filePath}:${lineNumber}`
        );
        return lines.join('\n');
      }
    }
  ...
  private formatTaskPlain(task: Task): string {
    ...
    if (task.subtasks?.length) {
      ...
      task.subtasks.forEach((s, i) =>
        lines.push(`  ${i + 1}. [${s.isCompleted ? 'x' : ' '}] ${s.title}`)
      );
    }
    return lines.join('\n');
  }
  ...
  private formatTaskMarkdown(task: Task): string {
    ...
    const emoji: Record<TaskPriority, string> = {
      critical: '🔴',
      high: '🟠',
      medium: '🟡',
      low: '🟢',
