...
export type MessageType = 'request' | 'response' | 'notification' | 'error' | 'heartbeat';
...
) => void | Promise<void | MessageResponse>;
...
class AgentProtocol {
  ...
  private findMessageById(id: string): AgentMessage | undefined {
    return (
      this.messageHistory.find((m) => m.id === id) || this.messageQueue.find((m) => m.id === id)
    );
  }
