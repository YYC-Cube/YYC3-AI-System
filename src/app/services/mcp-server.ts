...
class MCPServer {
  ...
  private resourceSubscriptions: Map<string, Set<(content: MCPResourceContent) => void>> =
    new Map();
  ...
  async notifyResourceUpdated(uri: string, content: MCPResourceContent): Promise<void> {
    ...
    if (this.transport && this.clientCapabilities?.resources?.subscribe) {
      await this.protocolHandler.sendNotification(
        this.transport,
...
class SimplePromptProvider implements PromptProvider {
  private prompts: Map<
    string,
    { prompt: MCPPrompt; handler: (args: Record<string, string>) => MCPPromptResult }
  > = new Map();
