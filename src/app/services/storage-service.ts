...
export class StorageService implements StorageServiceInterface {
  ...
  async setEncrypted<T>(key: string, value: T): Promise<void> {
    ...
    await this.idb.put('keyValue', { key, value: encrypted, encrypted: true, updatedAt: Date.now() })
    this.kvCache.delete(key)
  }
  ...
  async getEncrypted<T>(key: string): Promise<T | null> {
    ...
    const record = await this.idb.get<{ key: string; value: string; encrypted?: boolean }>('keyValue', key)
  ...
  async setSecure(key: string, value: string): Promise<void> {
    ...
    await this.idb.put('keyValue', { key, value: JSON.stringify(encrypted), encrypted: true, updatedAt: Date.now() })
  }
  ...
  async exportAll(): Promise<string> {
    ...
    return JSON.stringify({ files, versions, snapshots, dbProfiles: profiles, keyValue: kv }, null, 2)
  }
...
class StorageOptimizer {
  ...
  async cleanupOldData(options: CleanupOptions = {}): Promise<CleanupResult> {
    ...
    const { maxAge = 90 * 24 * 60 * 60 * 1000, maxVersions = 20, keepLatest = true, dryRun = false } = options
  ...
  async saveLargeFile(path: string, content: string, metadata?: Partial<FileNode>): Promise<FileNode> {
