...
const OPTIONAL_PERMISSIONS: PluginPermission[] = ['editor', 'ai', 'ui', 'network', 'system', 'notification', 'database']
...
export interface PluginAPI {
  ...
  ai: {
    generateCode: (prompt: string, options?: { provider?: string; model?: string; language?: string; maxTokens?: number; temperature?: number }) => Promise<string>
    ...
    reviewCode: (code: string) => Promise<{ score: number; issues: { severity: string; line: number; message: string }[]; suggestions: string[] }>
    getActiveModel: () => { name: string; provider: string } | null
    isModelConfigured: () => boolean
  }
  database: {
    listConnections: () => Promise<string[]>
    executeQuery: (connId: string, sql: string) => Promise<{ columns: string[]; rows: Record<string, unknown>[] }>
  ...
  ui: {
    ...
    registerPanel: (config: { id: string; title: string; position: 'left' | 'right' | 'bottom'; icon?: string }) => void
    registerButton: (config: { id: string; label: string; position: 'toolbar' | 'editor' | 'sidebar'; onClick: () => void }) => void
    registerMenuItem: (config: { id: string; label: string; position: string; onClick: () => void }) => void
    showNotification: (message: string, type?: 'info' | 'success' | 'warning' | 'error') => void
    showDialog: (config: { title: string; message: string; confirmLabel?: string; cancelLabel?: string }) => Promise<boolean>
    showInputBox: (options: { title: string; placeholder?: string; value?: string }) => Promise<string | undefined>
  }
  logger: {
...
export class PluginRuntime {
  ...
  getPresetManifests(): PluginManifest[] {
    return [
      {
        id: 'plugin-code-analyzer', name: '代码分析器', version: '1.0.0',
        description: '静态代码分析与质量评分', author: 'YYC³ Team',
        appVersion: '1.0.0', main: 'index.ts', icon: '🔍',
        permissions: ['editor', 'notification'],
      },
      {
        id: 'plugin-design-sync', name: '设计同步', version: '1.0.0',
        description: 'Figma 设计稿自动同步', author: 'YYC³ Team',
        appVersion: '1.0.0', main: 'index.ts', icon: '🎨',
        permissions: ['network', 'editor', 'storage'],
      },
      {
        id: 'plugin-test-gen', name: '测试生成器', version: '1.0.0',
        description: 'AI 驱动自动生成单元测试', author: 'YYC³ Team',
        appVersion: '1.0.0', main: 'index.ts', icon: '🧪',
        permissions: ['editor', 'ai', 'storage'],
      },
      {
        id: 'plugin-doc-writer', name: '文档撰写', version: '1.0.0',
        description: 'AI 驱动自动生成 API 文档', author: 'YYC³ Team',
        appVersion: '1.0.0', main: 'index.ts', icon: '📝',
        permissions: ['editor', 'ai', 'notification'],
      },
      {
        id: 'plugin-i18n-helper', name: '国际化助手', version: '1.0.0',
        description: '自动提取和翻译 i18n 键值', author: 'YYC³ Team',
        appVersion: '1.0.0', main: 'index.ts', icon: '🌐',
