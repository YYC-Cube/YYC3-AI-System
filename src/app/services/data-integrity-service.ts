...
class DataIntegrityService {
  ...
  async performFullCheck(): Promise<IntegrityCheckResult> {
    ...
    result.summary.totalItems = files.length + versions.length + snapshots.length + dbProfiles.length
    ...
    result.summary.healthyItems = result.summary.totalItems - result.summary.corruptedItems - result.summary.missingItems - result.summary.orphanedItems
  ...
  async repairData(issues: IntegrityIssue[]): Promise<{ repaired: number; failed: number; details: string[] }> {
    ...
    for (const issue of issues) {
      ...
      try {
        ...
        result.failed++
        result.details.push(`修复错误: ${issue.description} - ${error instanceof Error ? error.message : '未知错误'}`)
      }
    }
  ...
  async createMigrationPlan(fromVersion: string, toVersion: string): Promise<MigrationPlan> {
    ...
    if (fromVersion === '1.0.0' && toVersion === '1.1.0') {
      steps.push(
        { order: 1, description: '备份现有数据', action: 'backup', rollbackAction: 'restore_backup' },
        { order: 2, description: '更新文件结构', action: 'migrate_file_structure', rollbackAction: 'revert_file_structure' },
        { order: 3, description: '更新版本历史格式', action: 'migrate_version_format', rollbackAction: 'revert_version_format' },
        { order: 4, description: '验证数据完整性', action: 'verify_integrity' }
      )
    } else if (fromVersion === '1.0.0' && toVersion === '2.0.0') {
      steps.push(
        { order: 1, description: '创建完整备份', action: 'full_backup', rollbackAction: 'restore_full_backup' },
        { order: 2, description: '迁移到新数据模型', action: 'migrate_data_model', rollbackAction: 'revert_data_model' },
  ...
  async executeMigration(plan: MigrationPlan): Promise<MigrationResult> {
    ...
    for (const step of plan.steps.sort((a, b) => a.order - b.order)) {
      try {
        ...
        result.errors.push(`步骤 ${step.order} 失败: ${error instanceof Error ? error.message : '未知错误'}`)
  ...
  private async checkVersions(versions: FileVersion[], result: IntegrityCheckResult): Promise<void> {
  ...
  private async checkSnapshots(snapshots: PreviewSnapshot[], result: IntegrityCheckResult): Promise<void> {
  ...
  private async checkDBProfiles(profiles: DBConnectionProfile[], result: IntegrityCheckResult): Promise<void> {
  ...
  private async checkConsistency(files: FileNode[], versions: FileVersion[], result: IntegrityCheckResult): Promise<void> {
  ...
  private async restoreFromVersion(path: string): Promise<boolean> {
    ...
    const fileVersions = versions.filter((v) => v.path === path).sort((a, b) => b.createdAt - a.createdAt)
