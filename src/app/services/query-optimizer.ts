...
const SIMULATED_INDEXES: IndexStats[] = [
  { name: 'users_pkey', table: 'users', columns: ['id'], size: 32768, sizeFormatted: '32 KB', scans: 4520, tuplesRead: 12800, tuplesFetched: 4520, lastUsed: Date.now() - 60000 },
  { name: 'users_email_idx', table: 'users', columns: ['email'], size: 24576, sizeFormatted: '24 KB', scans: 3100, tuplesRead: 3100, tuplesFetched: 3100, lastUsed: Date.now() - 120000 },
  { name: 'projects_pkey', table: 'projects', columns: ['id'], size: 16384, sizeFormatted: '16 KB', scans: 2340, tuplesRead: 6700, tuplesFetched: 2340, lastUsed: Date.now() - 300000 },
  { name: 'projects_owner_idx', table: 'projects', columns: ['owner_id'], size: 16384, sizeFormatted: '16 KB', scans: 1890, tuplesRead: 5400, tuplesFetched: 1890, lastUsed: Date.now() - 180000 },
  { name: 'files_project_idx', table: 'files', columns: ['project_id'], size: 40960, sizeFormatted: '40 KB', scans: 5600, tuplesRead: 28000, tuplesFetched: 5600, lastUsed: Date.now() - 45000 },
  { name: 'files_path_idx', table: 'files', columns: ['path'], size: 57344, sizeFormatted: '56 KB', scans: 8200, tuplesRead: 8200, tuplesFetched: 8200, lastUsed: Date.now() - 30000 },
  { name: 'ai_sessions_pkey', table: 'ai_sessions', columns: ['id'], size: 8192, sizeFormatted: '8 KB', scans: 340, tuplesRead: 1020, tuplesFetched: 340, lastUsed: Date.now() - 600000 },
  { name: 'ai_sessions_user_idx', table: 'ai_sessions', columns: ['user_id', 'created_at'], size: 16384, sizeFormatted: '16 KB', scans: 780, tuplesRead: 2340, tuplesFetched: 780, lastUsed: Date.now() - 240000 },
]
...
class QueryAnalyzer {
  async analyzeQuery(_connId: string, sql: string): Promise<QueryAnalysis> {
    ...
    if (sqlUpper.startsWith('SELECT') && !sqlUpper.includes('LIMIT') && !sqlUpper.includes('COUNT')) {
      recommendations.push({
        type: 'limit',
        severity: 'info',
  ...
  private simulatePlan(sql: string): QueryPlan {
    ...
    const basePlan: QueryPlan = {
      ...
      cost: { startup: hasWhere ? 0.28 : 0.00, total: hasWhere ? 12.45 : 245.00 },
    ...
    if (hasWhere) {
      ...
      basePlan.filter = sql.match(/WHERE\s+(.+?)(?:ORDER|LIMIT|GROUP|$)/i)?.[1]?.trim() || undefined
    }

    if (hasJoin) {
      basePlan.children = [
        { nodeType: 'Hash', cost: { startup: 4.25, total: 4.25 }, rows: 100, width: 64 },
        { nodeType: 'Seq Scan', cost: { startup: 0, total: 22.5 }, rows: 1250, width: 120, relation: 'joined_table' },
      ]
    }
...
class SlowQueryMonitor {
  ...
  private seedDemoData() {
    const demoQueries = [
      { sql: 'SELECT * FROM files WHERE content ILIKE \'%import%\' ORDER BY updated_at DESC', duration: 1240, rows: 342 },
      { sql: 'SELECT u.*, COUNT(p.id) FROM users u LEFT JOIN projects p ON p.owner_id = u.id GROUP BY u.id', duration: 890, rows: 1250 },
      { sql: 'SELECT * FROM ai_sessions WHERE created_at > NOW() - INTERVAL \'7 days\' ORDER BY tokens_used DESC', duration: 650, rows: 89 },
      { sql: 'UPDATE files SET content = $1 WHERE project_id = $2 AND path = $3', duration: 520, rows: 1 },
      { sql: 'SELECT f.*, p.name as project_name FROM files f JOIN projects p ON f.project_id = p.id WHERE f.size > 100000', duration: 780, rows: 56 },
    ]
  ...
  getStats(): { total: number; avgDuration: number; maxDuration: number; minDuration: number } {
    if (this.queries.length === 0) return { total: 0, avgDuration: 0, maxDuration: 0, minDuration: 0 }
  ...
  setThreshold(ms: number) { this.threshold = ms }
...
class BatchOperation {
  async batchInsert(_connId: string, options: BatchInsertOptions): Promise<{ inserted: number; duration: number }> {
  ...
  async batchUpdate(_connId: string, _table: string, updates: { id: string; data: Record<string, unknown> }[]): Promise<{ updated: number; duration: number }> {
  ...
  async batchDelete(_connId: string, _table: string, ids: string[]): Promise<{ deleted: number; duration: number }> {
