...
export async function testModelConnection(
  ...
  try {
    ...
    if (resp.status === 401 || resp.status === 403) {
      useAppStore
        .getState()
        .setModelStatus(modelId, 'error', `Authentication failed (HTTP ${resp.status})`);
    ...
    const errorMsg =
      errorObj.name === 'AbortError' || errorObj.name === 'TimeoutError'
        ? 'Request timed out'
...
export async function validateApiKey(
  ...
  try {
    ...
    if (resp.status === 401 || resp.status === 403) {
      return {
        valid: false,
        error: `Authentication failed (HTTP ${resp.status}) — check your API key`,
      };
    }
...
export function deepSearchSettings(query: string): SearchHit[] {
  ...
  const fieldMappings: {
    section: string;
    sectionId: string;
    fields: { path: string; label: string; aliases: string[]; getValue: () => string }[];
  }[] = [
    {
      section: 'Account',
      sectionId: 'account',
      fields: [
        {
          ...
          label: 'Username / 用户名',
          aliases: ['用户', 'user', '名前', '사용자'],
          getValue: () => ss.account.username,
        },
        {
          path: 'account.email',
          label: 'Email / 邮箱',
          aliases: ['邮件', 'email', 'メール', '이메일'],
          getValue: () => ss.account.email,
        },
      ],
    },
    {
      ...
      fields: [
        {
          ...
          label: 'Font Family / 字体',
          aliases: ['字体', 'font', 'フォント', '글꼴'],
          getValue: () => ss.editor.fontFamily,
        },
        {
          path: 'editor.fontSize',
          label: 'Font Size / 字体大小',
          aliases: ['字号', 'size', '大きさ', '크기'],
          getValue: () => String(ss.editor.fontSize),
        },
        {
          path: 'editor.tabSize',
          label: 'Tab Size / 制表符',
          aliases: ['制表', 'tab', 'タブ', '탭'],
          getValue: () => String(ss.editor.tabSize),
        },
        {
          path: 'editor.wordWrap',
          label: 'Word Wrap / 自动换行',
          aliases: ['换行', 'wrap', '折り返し', '줄바꿈'],
          getValue: () => String(ss.editor.wordWrap),
        },
        {
          path: 'editor.minimap',
          label: 'Minimap / 小地图',
          aliases: ['小地图', 'minimap', 'ミニマップ', '미니맵'],
          getValue: () => String(ss.editor.minimap),
        },
        {
          path: 'editor.lineNumbers',
          label: 'Line Numbers / 行号',
          aliases: ['行号', 'line', '行番号', '줄번호'],
          getValue: () => String(ss.editor.lineNumbers),
        },
        {
          path: 'shortcutScheme',
          label: 'Shortcut Scheme / 快捷键方案',
          aliases: ['快捷键', 'shortcut', 'キーボード', '단축키'],
          getValue: () => ss.shortcutScheme,
        },
        {
          path: 'nodeVersion',
          label: 'Node.js Version / Node版本',
          aliases: ['node', '版本', 'バージョン', '버전'],
          getValue: () => ss.activeNodeVersion,
        },
      ],
    },
    {
    ...
    },
    {
      ...
      fields: [
        {
          path: 'codeIndex',
          label: 'Code Index / 代码索引',
          aliases: ['索引', 'index', 'インデックス', '인덱스'],
          getValue: () => String(ss.codeIndexEnabled),
        },
        {
          path: 'ignorePatterns',
          label: 'Ignore Patterns / 忽略规则',
          aliases: ['忽略', 'ignore', '無視', '무시', 'gitignore'],
          getValue: () => ss.ignorePatterns,
        },
        ...ss.docSets.map((d) => ({
          path: `docSets.${d.id}`,
          label: `Doc: ${d.name}`,
    ...
    },
    {
      ...
      fields: [
        {
          path: 'todoList',
          label: 'Todo List / 待办清单',
          aliases: ['待办', 'todo', 'やること', '할일'],
          getValue: () => String(ss.todoListEnabled),
        },
        {
          path: 'autoCollapse',
          label: 'Auto Collapse / 自动折叠',
          aliases: ['折叠', 'collapse', '折りたたみ', '접기'],
          getValue: () => String(ss.autoCollapse),
        },
        {
          path: 'autoFixCode',
          label: 'Auto Fix / 自动修复',
          aliases: ['修复', 'fix', '修正', '수정'],
          getValue: () => String(ss.autoFixCode),
        },
        {
          path: 'agentAsk',
          label: 'Agent Question / 智能体提问',
          aliases: ['提问', 'ask', 'question', '質問', '질문'],
          getValue: () => String(ss.agentAsk),
        },
        {
          path: 'reviewScope',
          label: 'Review Scope / 审查范围',
          aliases: ['审查', 'review', 'レビュー', '리뷰'],
          getValue: () => ss.reviewScope,
        },
        {
          path: 'autoRunMcp',
          label: 'Auto Run MCP / 自动运行',
          aliases: ['运行', 'run', '実行', '실행'],
          getValue: () => String(ss.autoRunMcp),
        },
        {
          path: 'commandRunMode',
          label: 'Command Mode / 命令方式',
          aliases: ['命令', 'command', 'sandbox', 'コマンド', '명령'],
          getValue: () => ss.commandRunMode,
        },
        {
          path: 'volume',
          label: 'Volume / 音量',
          aliases: ['音量', 'volume', 'ボリューム', '볼륨'],
          getValue: () => String(ss.notifications.volume),
        },
        {
          path: 'temperature',
          label: 'Temperature / 温度',
          aliases: ['温度', 'temperature', '温度', '온도'],
          getValue: () => '',
        },
      ],
    },
      ...
      fields: [
        ...ss.rules.map((r) => ({
          ...
          aliases: [
            ...
            '규칙',
            r.name.toLowerCase(),
            r.content.toLowerCase().slice(0, 100),
          ],
        ...
        ...ss.skills.map((sk) => ({
          ...
          aliases: [
            ...
            '스킬',
            sk.name.toLowerCase(),
            sk.content.toLowerCase().slice(0, 100),
          ],
...
export function initializeSettingsIntegration(): () => void {
  ...
  const unregisterHandlers = [
    ...
    registerShortcutHandler('command-palette', () =>
    ...
    registerShortcutHandler('toggle-fullscreen', () =>
      useAppStore.getState().setViewMode('fullscreen')
    ),
  ];
