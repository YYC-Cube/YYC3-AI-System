...
export function useAIProvider() {
  ...
  return {
    ...
    refresh,
    addProvider: (p: AIProviderConfig) => {
      aiProviderService.addProvider(p);
      refresh();
    },
    ...
    setApiKey: (id: string, key: string) => {
      aiProviderService.setApiKey(id, key);
      refresh();
    },
    setActiveProvider: (id: string) => {
      aiProviderService.setActiveProvider(id);
      refresh();
    },
...
export function usePreviewSnapshots() {
  const [snapshots, setSnapshots] = useState<
    Awaited<ReturnType<typeof storageService.getSnapshots>>
  >([]);
  const [loading, setLoading] = useState(false);
