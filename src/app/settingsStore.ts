...
const defaultAgents: AgentConfig[] = [
  {
    ...
    prompt: 'You are a senior developer. Help the user write clean, efficient code.',
    builtin: true,
    enabled: true,
  },
  {
    ...
    prompt: 'You are a code reviewer. Identify bugs, security issues, and suggest improvements.',
    builtin: true,
    enabled: true,
  },
  {
    ...
    prompt:
      'You are a software architect. Help design scalable systems and suggest architectural patterns.',
    builtin: true,
    enabled: false,
  },
];
...
export const useSettingsStore = create<SettingsState>()(
  persist(
    (set) => ({
      ...
      customKeybindings: {},
      setCustomKeybinding: (actionId, shortcut) =>
        set((s) => ({ customKeybindings: { ...s.customKeybindings, [actionId]: shortcut } })),
      ...
      addNodeVersion: (v) =>
        set((s) => ({
          nodeVersions: s.nodeVersions.includes(v) ? s.nodeVersions : [...s.nodeVersions, v],
        })),
      removeNodeVersion: (v) =>
        set((s) => ({
          nodeVersions: s.nodeVersions.filter((n) => n !== v),
          activeNodeVersion:
            s.activeNodeVersion === v ? s.nodeVersions[0] || '' : s.activeNodeVersion,
        })),
      ...
      updateAgent: (id, u) =>
        set((s) => ({ agents: s.agents.map((a) => (a.id === id ? { ...a, ...u } : a)) })),
      ...
      updateMcp: (id, u) =>
        set((s) => ({ mcpServers: s.mcpServers.map((m) => (m.id === id ? { ...m, ...u } : m)) })),
      ...
      updateRule: (id, u) =>
        set((s) => ({ rules: s.rules.map((r) => (r.id === id ? { ...r, ...u } : r)) })),
      removeRule: (id) => set((s) => ({ rules: s.rules.filter((r) => r.id !== id) })),
      skills: [],
      addSkill: (s2) => set((s) => ({ skills: [...s.skills, { ...s2, id: 'skill-' + uid() }] })),
      updateSkill: (id, u) =>
        set((s) => ({ skills: s.skills.map((sk) => (sk.id === id ? { ...sk, ...u } : sk)) })),
