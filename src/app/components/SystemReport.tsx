...
export function SystemReport() {
  ...
  return (
    <div className={`flex flex-col h-full relative overflow-y-auto p-5 ${t.scrollbar} ${t.surface.glass} ${t.text.primary}`}>

      <div className="flex items-center space-x-3 mb-8">
        <Activity className={`w-8 h-8 ${t.accent.primary}`} />
        <div>
          <h2 className={`text-2xl tracking-tight ${t.text.primary}`} style={{ fontWeight: 700 }}>{i.systemReport}</h2>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          ...
          <div key={i} className={`${t.surface.widget} rounded-xl p-4 flex flex-col relative overflow-hidden group hover:opacity-90 transition-colors`}>
            <div className="absolute -right-4 -top-4 opacity-10 group-hover:opacity-20 transition-opacity">
              <stat.icon className="w-20 h-20" />
            </div>
            <span className={`text-xs mb-2 ${t.text.tertiary}`} style={{ fontWeight: 500 }}>{stat.label}</span>
            <span className={`text-2xl ${stat.color}`} style={{ fontWeight: 700 }}>{stat.value}</span>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div className={`${t.surface.widget} rounded-xl p-5`}>
          <h3 className={`text-sm mb-4 ${t.text.secondary}`} style={{ fontWeight: 600 }}>{i.srResourceTrend}</h3>
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={mockPerformanceData}>
                ...
                <Line type="monotone" dataKey="cpu" name="CPU (%)" stroke="#3b82f6" strokeWidth={2} dot={{ r: 4 }} activeDot={{ r: 6 }} />
                <Line type="monotone" dataKey="memory" name="Memory (%)" stroke="#14b8a6" strokeWidth={2} dot={{ r: 4 }} />
                <Line type="monotone" dataKey="latency" name="Latency (ms)" stroke="#8b5cf6" strokeWidth={2} strokeDasharray="5 5" />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className={`${t.surface.widget} rounded-xl p-5 flex flex-col`}>
          <h3 className={`text-sm mb-4 ${t.text.secondary}`} style={{ fontWeight: 600 }}>{i.srCodeGenMatrix}</h3>
      ...
      <div className={`${t.surface.widget} rounded-xl p-6`}>
        <h3 className={`text-lg mb-4 ${t.text.primary}`} style={{ fontWeight: 600 }}>{i.srChecklist}</h3>
        <div className="space-y-4">
          {[
            ...
            <div key={i} className="flex items-start space-x-3">
              <div className={`w-5 h-5 rounded ${t.status.successBg} ${t.status.success} flex items-center justify-center flex-shrink-0 mt-0.5`}>
                <span className="text-[11px]">✓</span>
              </div>
              <div>
                <h4 className={`text-sm ${t.text.secondary}`} style={{ fontWeight: 500 }}>{item.title}</h4>
