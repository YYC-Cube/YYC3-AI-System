...
const MOCK_PLUGINS: Plugin[] = [
  {
    ...
    version: '2.1.0',
    author: 'YYC3 Team',
    description: '智能代码质量分析，支持 TypeScript/React，检测性能瓶颈和安全漏洞',
    status: 'active',
    enabled: true,
    installed: true,
    permissions: [
  ...
  },
  {
    ...
    version: '1.5.2',
    author: 'YYC3 Team',
    description: '从 Figma 实时同步设计稿到代码组件，支持 token 映射和响应式转换',
    status: 'active',
    enabled: true,
    installed: true,
    permissions: [
  ...
  },
  {
    ...
    version: '1.2.0',
    author: 'Community',
    description: '基于 AI 自动生成单元测试和集成测试，支持 Vitest/Jest',
    status: 'inactive',
    enabled: false,
    installed: true,
    permissions: [
    ...
    hooks: [{ name: 'onTestRun', description: 'After test execution', type: 'after' }],
    apiEndpoints: ['generateTest', 'runTests', 'getCoverage'],
    size: '1.8 MB',
    downloads: 5200,
    rating: 4.3,
    tags: ['testing', 'ai', 'vitest'],
  },
  {
    id: 'pl-4',
    name: 'doc-writer',
    displayName: 'Smart Doc Writer',
    version: '1.0.3',
    author: 'Community',
    description: '自动为函数、组件、模块生成 JSDoc/TSDoc 文档和 README',
    status: 'error',
    enabled: true,
    installed: true,
    permissions: [
    ...
    hooks: [{ name: 'onFileOpen', description: 'Check docs on file open', type: 'after' }],
  ...
  },
  {
    ...
    version: '2.0.0',
    author: 'Cloud Team',
    description: '将项目文件备份到 AWS S3/阿里 OSS，支持版本管理和增量同步',
    status: 'inactive',
    enabled: false,
    installed: false,
    permissions: [
  ...
  },
  {
    id: 'pl-6',
    name: 'sqlite-backend',
    displayName: 'SQLite Backend',
    version: '1.1.0',
    author: 'Community',
    description: '本地 SQLite 数据库后端，适用于轻量级数据存储和离线应用',
    status: 'inactive',
    enabled: false,
    installed: false,
    permissions: [
    ...
    hooks: [{ name: 'onDbQuery', description: 'Query interceptor', type: 'filter' }],
...
export function PluginSystem({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const installPlugin = (id: string) => {
    setPlugins((prev) =>
      prev.map((p) => (p.id === id ? { ...p, installed: true, status: 'loading' as const } : p))
    );
    setTimeout(() => {
      setPlugins((prev) =>
        prev.map((p) =>
          p.id === id
            ? {
                ...
                permissions: p.permissions.map((pm) => ({ ...pm, granted: true })),
              }
            : p
        )
      );
    ...
    setPlugins((prev) =>
      prev.map((p) =>
        p.id === pluginId
          ? {
              ...p,
              permissions: p.permissions.map((pm) =>
                pm.id === permId ? { ...pm, granted: !pm.granted } : pm
              ),
            }
          : p
      )
  ...
  return (
    <>
      ...
      <div
        className={`fixed inset-6 z-[81] rounded-2xl overflow-hidden flex flex-col ${t.surface.popover} ${t.border.popover} shadow-2xl`}
      >
        {/* Header */}
        <div
              ...
              {filteredPlugins.length === 0 ? (
                ...
                filteredPlugins.map((pl) => (
                  ...
                  >
                    ...
                    <div className={`text-[10px] ${t.text.dimmed} line-clamp-2`}>
                      {pl.description}
                    </div>
                    ...
                    <div className="flex flex-wrap gap-1 mt-1.5">
                      {pl.tags.map((tag) => (
                        <span
                          key={tag}
                          className="text-[8px] px-1.5 py-0.5 rounded-full bg-indigo-500/15 text-indigo-400"
                        >
                          {tag}
                        </span>
                      ))}
              ...
              <>
                ...
                >
                  <div className="flex items-center space-x-3">
                    <Package
                      className={`w-8 h-8 p-1.5 rounded-lg ${t.isDark ? 'bg-indigo-500/20 text-indigo-400' : 'bg-indigo-100 text-indigo-600'}`}
                    />
                    <div>
                    ...
                    {selectedPlugin.installed ? (
                      <>
                        ...
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-lg text-[11px] flex items-center space-x-1 hover:bg-red-500/15 text-red-400 ${t.transition}`}
                        >
                          <Trash2 className="w-3.5 h-3.5" />
                          <span>{i.plUninstall}</span>
                        </button>
                      </>
                    ) : (
                      <button
                  ...
                  {detailTab === 'info' && (
                    <>
                      ...
                      <div className="grid grid-cols-3 gap-3">
                        <div
                          className={`p-3 rounded-xl text-center ${t.isDark ? 'bg-slate-800/40' : 'bg-slate-50'}`}
                        >
                      ...
                      <div>
                        ...
                        <pre
                          className={`p-3 rounded-xl text-[11px] font-mono ${t.isDark ? 'bg-slate-900/60 text-slate-300' : 'bg-slate-100 text-slate-700'}`}
                        >
                    ...
                    <div className="space-y-2">
                      {selectedPlugin.permissions.map((perm) => (
                        ...
                        >
                          <div className="flex items-center space-x-2.5">
                            <Shield
                              className={`w-4 h-4 ${perm.granted ? 'text-emerald-400' : 'text-slate-400'}`}
                            />
                      ...
                      {selectedPlugin.hooks.map((hook) => (
                        ...
                        >
                          <div className="flex items-center space-x-2 mb-1">
                            ...
                            <span
                              className={`text-[9px] px-1.5 py-0.5 rounded ${t.isDark ? 'bg-slate-700/60' : 'bg-slate-200'}`}
                            >
                              {hook.type}
                            </span>
                          </div>
                          <div className={`text-[11px] ${t.text.dimmed} ml-5.5`}>
                            {hook.description}
                          </div>
                        </div>
                      ))}
                  ...
                  {detailTab === 'api' && (
                    <div className="space-y-2">
                      {selectedPlugin.apiEndpoints.map((ep) => (
                        <div
                          key={ep}
                          className={`flex items-center space-x-2.5 p-3 rounded-xl ${t.isDark ? 'bg-slate-800/40' : 'bg-slate-50'}`}
                        >
                          <Code className="w-3.5 h-3.5 text-blue-400" />
                          <code className="text-[12px] text-blue-400">
                            {selectedPlugin.name}.{ep}()
                          </code>
                        </div>
                      ))}
                    </div>
