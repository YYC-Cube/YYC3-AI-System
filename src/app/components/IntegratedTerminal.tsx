...
import {
  Terminal, Plus, X, Zap, GitBranch,
  Maximize2, Minimize2,
  Search, Square, Columns
} from 'lucide-react'
...
const SHELL_CONFIG: Record<ShellType, { label: string; labelKey: keyof I18nStrings; prompt: string; color: string }> = {
...
export function IntegratedTerminal() {
  ...
  const [tabs, setTabs] = useState<TerminalTab[]>([
    { id: 'main', name: 'bash', shell: 'bash', lines: [...INITIAL_LINES], isRunning: true, pid: 1024 },
  ])
  ...
  // Search highlighting
  useEffect(() => {
    if (!searchQuery || !activeTab) { setSearchMatches(0); return }
    const matches = activeTab.lines.filter(l => l.text.toLowerCase().includes(searchQuery.toLowerCase())).length
  ...
  const executeCommand = useCallback((cmd: string) => {
    ...
    addOutput(tabId, [
      { type: 'prompt', text: `${shellPrompt} ${cmd}` },
    ])
  ...
  const handleKeyDown = useCallback((e: KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter' && input.trim()) {
      ...
      const commands = ['pnpm', 'git', 'npm', 'node', 'clear', 'ls', 'cd', 'cat', 'echo', 'pwd', 'mkdir', 'tree', 'help', 'whoami', 'date', 'which']
      const filePaths = ['src/', 'src/app/', 'src/app/components/', 'src/styles/', 'package.json', 'tsconfig.json', 'README.md', 'vite.config.ts']
      const gitSubs = ['status', 'log', 'add', 'commit', 'push', 'pull', 'branch', 'checkout', 'diff', 'stash']
      ...
      if (parts.length === 1) {
        ...
        candidates = gitSubs.filter(c => c.startsWith(last)).map(c => parts.slice(0, -1).join(' ') + ' ' + c)
      } else if (parts[0] === 'pnpm' || parts[0] === 'npm') {
        candidates = pnpmSubs.filter(c => c.startsWith(last)).map(c => parts.slice(0, -1).join(' ') + ' ' + c)
      } else if (['cd', 'cat', 'ls'].includes(parts[0])) {
        candidates = filePaths.filter(c => c.startsWith(last)).map(c => parts.slice(0, -1).join(' ') + ' ' + c)
      } else {
        candidates = commands.filter(c => c.startsWith(last))
      }
  ...
  const addTab = (shell: ShellType = 'bash') => {
    ...
    setTabs(prev => [...prev, { id, name, shell, lines: [], isRunning: false, pid: Math.floor(Math.random() * 9000) + 1000 }])
    setActiveTabId(id)
  ...
  const killProcess = (tabId: string) => {
    setTabs(prev => prev.map(t =>
      t.id === tabId ? { ...t, isRunning: false } : t
    ))
    addOutput(tabId, [{ type: 'error', text: '^C' }, { type: 'output', text: '' }])
  }
    ...
    >
      ...
      >
        {tab.lines.map((line, _idx) => {
          ...
          return (
            <div key={_idx} style={{ lineHeight: '19px' }} className={isHighlighted ? `${t.isDark ? 'bg-yellow-500/15' : 'bg-yellow-100'}` : ''}>
              ...
              {line.type === 'output' && <span className={t.terminal.output}>{line.text || '\u00A0'}</span>}
  ...
  return (
    <div className={`flex flex-col h-full overflow-hidden border-t ${t.transition} ${t.surface.glass} ${t.border.subtle}`}>
      {/* Terminal Header */}
      <div className={`h-7 flex items-center justify-between px-1 flex-shrink-0 ${t.surface.toolbar}`}>
        {/* Left: Terminal tabs */}
        <div className="flex items-center space-x-0 overflow-x-auto min-w-0">
          ...
          {tabs.map(tab => (
            ...
            >
              {/* Shell color dot */}
              <div className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ backgroundColor: SHELL_CONFIG[tab.shell].color }} />
          ...
          <div className="relative">
            ...
            {shellSelectorOpen && (
              <>
                ...
                <div className={`absolute left-0 top-full mt-1 z-50 rounded-lg overflow-hidden min-w-[100px] ${t.surface.popover} ${t.border.popover} ${t.shadow.popover}`}>
                  <div className={`px-2 py-1 text-[9px] uppercase tracking-wider ${t.text.dimmed}`} style={{ fontWeight: 600 }}>{i.tmSelectShell}</div>
                  {(Object.keys(SHELL_CONFIG) as ShellType[]).map(shell => (
                    ...
                    >
                      <div className="w-2 h-2 rounded-full" style={{ backgroundColor: SHELL_CONFIG[shell].color }} />
        ...
        <div className="flex items-center space-x-0.5 flex-shrink-0">
          ...
          <button
            onClick={() => {
              if (isSplit) { setIsSplit(false); setSplitTabId(null) }
              else handleSplit()
            }}
      ...
      {/* Search bar */}
      {searchOpen && (
        <div className={`h-6 flex items-center px-2 gap-2 border-b ${t.border.subtle} ${t.surface.toolbar}`}>
          ...
          <button onClick={() => { setSearchOpen(false); setSearchQuery('') }} className={`p-0.5 ${t.interactive.iconBtn}`}>
            <X className="w-2.5 h-2.5" />
          </button>
        </div>
      )}
        ...
        <>
          ...
          >
            {[
              ...
              { label: i.tmSplit, action: () => { setIsSplit(true); setSplitTabId(contextMenu.tabId); setContextMenu(null) } },
              { label: i.tmKillProcess, action: () => { killProcess(contextMenu.tabId); setContextMenu(null) } },
              null,
              { label: i.tmClearAll, action: () => { setTabs(prev => prev.map(t => t.id === contextMenu.tabId ? { ...t, lines: [] } : t)); setContextMenu(null) } },
              { label: i.ftClose, action: () => { removeTab(contextMenu.tabId); setContextMenu(null) } },
            ].map((item, _idx) =>
              item === null ? (
...
function getCommandResponse(cmd: string, i: I18nStrings): TerminalLine[] {
  ...
  if (cmd === 'pnpm lint') {
    return [
      ...
      { type: 'output', text: '' },
      { type: 'error', text: 'src/app/store.ts:35  error  \'initialDesignRoot\' is not defined  no-undef' },
      { type: 'output', text: '' },
  ...
  if (cmd.startsWith('which ')) {
    ...
    const known: Record<string, string> = {
      node: '/usr/local/bin/node', pnpm: '/usr/local/bin/pnpm', git: '/usr/bin/git',
      npm: '/usr/local/bin/npm', tsc: '/usr/local/bin/tsc', vite: './node_modules/.bin/vite',
    }
    return [{ type: known[target] ? 'output' : 'error', text: known[target] || `${target} not found` }]
  }
  if (cmd === 'tree') {
    return [
      { type: 'output', text: '.' },
  ...
  if (cmd.startsWith('ssh ')) {
    ...
    if (target.includes('@')) {
      ...
      return [
        ...
        { type: 'output', text: `The authenticity of host '${host} (${Math.floor(Math.random()*255)}.${Math.floor(Math.random()*255)}.${Math.floor(Math.random()*255)}.${Math.floor(Math.random()*255)})' can't be established.` },
        { type: 'output', text: 'ED25519 key fingerprint is SHA256:xKf3j8R2m5dPqZ7nB4hY9vL1wA6sC0eG8iU2tR4pO1M.' },
        { type: 'output', text: 'Are you sure you want to continue connecting (yes/no/[fingerprint])?' },
        { type: 'success', text: 'Warning: Permanently added \'' + host + '\' (ED25519) to the list of known hosts.' },
        { type: 'output', text: `${user}@${host}'s password: ` },
        { type: 'success', text: `Welcome to Ubuntu 22.04.3 LTS (GNU/Linux 5.15.0-91-generic x86_64)` },
        { type: 'output', text: '' },
        { type: 'output', text: ` * Documentation:  https://help.ubuntu.com` },
        { type: 'output', text: ` * Management:     https://landscape.canonical.com` },
        { type: 'output', text: '' },
        { type: 'output', text: `Last login: ${new Date().toUTCString()} from 192.168.1.${Math.floor(Math.random()*254)+1}` },
  ...
  if (cmd === 'ssh-keygen') {
    return [
      ...
      { type: 'output', text: 'Enter file in which to save the key (/home/yyc3-developer/.ssh/id_ed25519):' },
      { type: 'output', text: 'Enter passphrase (empty for no passphrase):' },
      { type: 'output', text: 'Enter same passphrase again:' },
      { type: 'success', text: 'Your identification has been saved in /home/yyc3-developer/.ssh/id_ed25519' },
      { type: 'success', text: 'Your public key has been saved in /home/yyc3-developer/.ssh/id_ed25519.pub' },
      { type: 'output', text: 'The key fingerprint is:' },
      { type: 'output', text: 'SHA256:xKf3j8R2m5dPqZ7nB4hY9vL1wA6sC0eG8iU2tR4pO1M yyc3-developer@yyc3-portaisys' },
    ]
  }
  ...
  if (cmd === 'ssh-add -l') {
    return [
      { type: 'output', text: '256 SHA256:xKf3j8R2m5dPqZ7nB4hY9vL1wA6sC0eG8iU2tR4pO1M yyc3-developer@yyc3-portaisys (ED25519)' },
    ]
  }
  return [
    { type: 'error', text: `zsh: command not found: ${cmd.split(' ')[0]}` },
  ]
}