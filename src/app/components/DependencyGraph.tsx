...
const MOCK_FILES: FileNode[] = [
  ...
  },
  {
    ...
    size: 3500,
    imports: [],
    exports: ['Message', 'DesignRoot', 'AIModel'],
  },
  {
    id: 'theme.ts',
    name: 'theme.ts',
    size: 6800,
    imports: [],
    exports: ['getThemeTokens', 'THEME_PRESETS'],
  },
  {
    id: 'i18n.ts',
    name: 'i18n.ts',
    size: 1100,
    imports: ['i18n-data'],
    exports: ['I18nStrings', 'resolveKey'],
  },
  {
    id: 'i18n-data.ts',
    name: 'i18n-data.ts',
  ...
  },
  {
    ...
    name: 'CodeEditor.tsx',
    size: 8500,
    imports: ['store', 'theme', 'collaboration'],
    exports: ['CodeEditor'],
  },
  {
    id: 'PreviewPanel.tsx',
  ...
  {
    id: 'collaboration.ts',
    name: 'collaboration.ts',
    size: 4200,
    imports: ['store'],
    exports: ['collabManager'],
  },
  {
    id: 'ModelSettings.tsx',
    name: 'ModelSettings.tsx',
    size: 9500,
    imports: ['store', 'theme', 'i18n'],
...
function detectCycles(files: FileNode[]): Set<string> {
  ...
  for (const f of files) {
    adjMap[f.id] = f.imports.filter((imp) =>
      files.some((ff) => ff.id === imp || ff.id === imp + '.ts' || ff.id === imp + '.tsx')
    );
  }
  ...
  function dfs(node: string, path: string[]) {
    ...
    for (const neighbor of adjMap[node] || []) {
      const resolvedNeighbor = files.find(
        (f) => f.id === neighbor || f.id === neighbor + '.ts' || f.id === neighbor + '.tsx'
      )?.id;
...
function resolveTarget(imp: string, files: FileNode[]): string | null {
  return (
    files.find((f) => f.id === imp || f.id === imp + '.ts' || f.id === imp + '.tsx')?.id || null
  );
}
      ...
      layers.forEach((layer, li) => {
        layer.forEach((nodeId, ni) => {
          positions[nodeId] = {
            x:
              80 +
              ((W - 160) / Math.max(layer.length - 1, 1)) * ni +
              (layer.length === 1 ? (W - 160) / 2 : 0),
            y: 60 + li * 80,
          };
        });
      });
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className={`w-56 border-r ${t.border.subtle} flex flex-col flex-shrink-0`}>
            ...
            <div className={`p-3 border-b ${t.border.subtle} space-y-1.5`}>
              <div className="flex justify-between text-[11px]">
                ...
                <span className={t.text.secondary} style={{ fontWeight: 600 }}>
              ...
              <div className="flex justify-between text-[11px]">
                <span className={cycleCount > 0 ? 'text-red-400' : t.text.muted}>
                  {i.dpCircular}
                </span>
                <span
                  className={cycleCount > 0 ? 'text-red-400' : t.text.secondary}
                ...
                >
                  ...
                  {viewMode === 'heatmap' && (
                    <div
                      ...
                      style={{ backgroundColor: getHeatColor(f.size) }}
                    />
                  )}
                </button>
              ))}
          ...
          >
            {cycleCount > 0 && highlightCycles && (
              <div className="absolute top-3 left-3 z-10 flex items-center space-x-1.5 px-3 py-1.5 rounded-lg bg-red-500/15 border border-red-500/30">
                ...
                <span className="text-[11px] text-red-400" style={{ fontWeight: 500 }}>
                  {i.dpCyclicWarning} ({cycleCount})
                </span>
              </div>
            )}
            ...
            <svg ref={svgRef} className="w-full h-full">
              ...
              {filteredFiles.map((f) => {
                ...
                const isConnected = selectedNode
                  ? edges.some(
                      (e) =>
                        (e.source === selectedNode && e.target === f.id) ||
                        (e.target === selectedNode && e.source === f.id)
                    )
                  : true;
                const nodeColor =
                  viewMode === 'heatmap'
                    ...
                    : isSelected
                      ...
                      : t.isDark
                        ...
                        : '#e2e8f0';
                const radius =
                  viewMode === 'heatmap' ? Math.max(12, Math.min(28, f.size / 1000)) : 18;

                return (
                  <g
                    key={f.id}
                    onClick={() => setSelectedNode(isSelected ? null : f.id)}
          ...
          {selectedNode &&
            (() => {
              ...
              return (
                <div
                  className={`w-52 border-l ${t.border.subtle} p-3 space-y-3 flex-shrink-0 overflow-y-auto ${t.scrollbar}`}
                >
                  <div>
                  ...
                  <div>
                    <div
                      ...
                      style={{ fontWeight: 600 }}
                    >
                      {i.dpImports} ({file.imports.length})
                    </div>
                    {file.imports.map((imp) => (
                      <div
                        key={imp}
                        className={`flex items-center space-x-1.5 px-2 py-1 rounded text-[11px] ${t.text.secondary}`}
                      >
                  ...
                  <div>
                    <div
                      ...
                      style={{ fontWeight: 600 }}
                    >
                      {i.dpExports} ({file.exports.length})
                    </div>
