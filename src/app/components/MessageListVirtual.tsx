...
import {
  Terminal, Bot, User, ArrowUpRight
} from 'lucide-react'
...
function getSyntaxHighlighter(): React.ComponentType<Record<string, unknown>> | null {
  ...
  if (!_loadPromise) {
    _loadPromise = import('react-syntax-highlighter')
      .then((mod) => {
        _SyntaxHL = (mod as unknown as { Prism?: React.ComponentType<Record<string, unknown>>; default?: React.ComponentType<Record<string, unknown>> }).Prism || mod.default
      })
...
export function MessageListVirtual({ messages, streamingContent = '', isStreaming = false }: MessageListVirtualProps) {
