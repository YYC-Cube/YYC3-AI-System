...
const TEMPLATES: Template[] = [
  {
    ...
    description:
      'Modern analytics dashboard with real-time data visualization, dark mode, and responsive grid layout.',
    category: 'dashboard',
    thumbnail: 'https://images.unsplash.com/photo-1761593280919-766a4acbcfca?w=400&h=250&fit=crop',
    author: 'YYC\u00B3 Team',
    version: '2.1.0',
    stars: 1847,
    downloads: 12400,
  ...
  },
  {
    ...
    description:
      'Full-featured e-commerce storefront with cart, checkout, product filtering, and payment integration.',
    category: 'ecommerce',
    ...
    featured: true,
    isNew: false,
    techStack: ['React', 'TypeScript', 'Stripe', 'Zustand'],
  ...
  },
  {
    ...
    name: 'DevFolio Pro',
    description:
      'Stunning developer portfolio with project showcase, blog, and contact form. Glassmorphism design.',
    category: 'portfolio',
    ...
    featured: false,
    isNew: true,
    techStack: ['React', 'Motion', 'Tailwind', 'MDX'],
  ...
  },
  {
    ...
    name: 'AdminStack',
    description:
      'Enterprise-grade admin panel with RBAC, data tables, forms, charts, and audit logging.',
    category: 'admin',
    ...
    featured: false,
    isNew: false,
    techStack: ['React', 'TypeScript', 'AG Grid', 'React Query'],
    colors: ['#14b8a6', '#334155'],
  },
  {
    id: 't5',
    name: 'LaunchPad',
    description:
      'High-converting landing page with hero section, features grid, pricing table, and testimonials.',
    category: 'landing',
    ...
    featured: true,
    isNew: false,
    techStack: ['React', 'Tailwind', 'Motion', 'Lucide'],
  ...
  },
  {
    ...
    name: 'InkPress Blog',
    description:
      'Minimal blog platform with MDX support, syntax highlighting, tag filtering, and RSS feed.',
    category: 'blog',
    ...
    featured: false,
    isNew: true,
    techStack: ['React', 'MDX', 'Tailwind', 'Shiki'],
  ...
  },
  {
    ...
    name: 'SaaSKit Pro',
    description:
      'Complete SaaS starter with auth, billing, team management, API dashboard, and onboarding flow.',
    category: 'saas',
  ...
  },
  {
    ...
    name: 'MobileFirst UI',
    description:
      'Mobile-optimized UI kit with gesture navigation, bottom sheets, and adaptive layouts.',
    category: 'mobile',
    ...
    featured: false,
    isNew: false,
    techStack: ['React', 'Tailwind', 'Motion', 'PWA'],
        ...
        >
          ...
          >
            <div className="flex items-center gap-3">
              <div
                className={`w-8 h-8 rounded-xl flex items-center justify-center ${t.isDark ? 'bg-gradient-to-br from-violet-500/20 to-cyan-500/20' : 'bg-gradient-to-br from-violet-50 to-cyan-50'}`}
              >
                <Sparkles
          ...
          <div className="flex flex-1 overflow-hidden">
            {/* ── Sidebar: Categories ── */}
            <div
              className={`w-44 flex-shrink-0 border-r ${t.border.subtle} p-3 space-y-0.5 overflow-y-auto`}
            >
              <div
                className={`text-[9px] uppercase tracking-wider px-2 py-1 ${t.text.dimmed}`}
              ...
              {categories.map((cat) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={`text-[8px] px-1 py-0.5 rounded ${t.isDark ? 'bg-white/[0.06]' : 'bg-slate-100'} ${t.text.dimmed}`}
                    >
                      {count}
                    </span>
                  </button>
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
              ...
              <div className={`px-4 py-3 border-b ${t.border.subtle}`}>
                <div
                  className={`flex items-center gap-2 px-3 py-1.5 rounded-xl ${t.isDark ? 'bg-white/[0.04]' : 'bg-slate-100/80'}`}
                >
              ...
              <div className={`flex-1 overflow-y-auto ${t.scrollbar} p-4`}>
                {filteredTemplates.length === 0 ? (
                  <div
                    className={`flex flex-col items-center justify-center gap-3 py-16 ${t.text.dimmed}`}
                  >
                        ...
                        >
                          ...
                          <div className="relative h-32 overflow-hidden">
                            ...
                            <div className="absolute top-2 left-2 flex gap-1">
                              {template.featured && (
                                <span
                                  className="px-1.5 py-0.5 rounded text-[7px] bg-violet-500/80 text-white backdrop-blur-sm"
                              ...
                              {template.isNew && (
                                <span
                                  className="px-1.5 py-0.5 rounded text-[7px] bg-emerald-500/80 text-white backdrop-blur-sm"
                                  style={{ fontWeight: 700 }}
                                >
                                  {i.tmpNew}
                                </span>
                              ...
                              {template.premium && (
                                <span
                                  className="px-1.5 py-0.5 rounded text-[7px] bg-amber-500/80 text-white backdrop-blur-sm"
                                  style={{ fontWeight: 700 }}
                                >
                                  {i.tmpPremium}
                                </span>
                          ...
                          <div className="p-3 space-y-2">
                            <div>
                              ...
                              <p className={`text-[9px] mt-0.5 line-clamp-2 ${t.text.muted}`}>
                            ...
                            <div className="flex flex-wrap gap-1">
                              {template.techStack.map((tech) => (
                                <span
                                  key={tech}
                                  className={`px-1.5 py-0.5 rounded text-[7px] ${t.isDark ? 'bg-white/[0.06] text-white/50' : 'bg-slate-100 text-slate-500'}`}
                                >
                                  {tech}
                                </span>
                              ))}
                            ...
                            <div className="flex items-center justify-between">
                              <div
                                className={`flex items-center gap-3 text-[8px] ${t.text.dimmed}`}
                              >
                                <span className="flex items-center gap-0.5">
                                  <Star className="w-2.5 h-2.5" /> {template.stars.toLocaleString()}
                                </span>
                                <span className="flex items-center gap-0.5">
                              ...
                              >
                                {isInstalled ? (
                                  ...
                                  <>
                                    <ArrowRight className="w-2.5 h-2.5" /> {i.tmpUseTemplate}
                                  </>
                                )}
                              </button>
                            </div>
              ...
              >
                <div className="flex items-center justify-between px-6 py-3">
                  <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-white text-[13px]" style={{ fontWeight: 600 }}>
                      {previewTemplate.name}
                    </h3>
                    <span className="text-white/50 text-[10px]">
                      v{previewTemplate.version} · {previewTemplate.author}
                    </span>
                  </div>
                  <button
                    onClick={() => {
                ...
                <div className="flex items-center justify-between px-6 py-3 border-t border-white/10">
                  <div className="flex items-center gap-4 text-white/50 text-[9px]">
                    <span className="flex items-center gap-1">
                      <User className="w-3 h-3" /> {previewTemplate.author}
                    </span>
                    <span className="flex items-center gap-1">
                      <Star className="w-3 h-3" /> {previewTemplate.stars.toLocaleString()}
                    </span>
                    <span className="flex items-center gap-1">
                      <Download className="w-3 h-3" /> {previewTemplate.downloads.toLocaleString()}
                    </span>
                    <span className="flex items-center gap-1">
                      <Clock className="w-3 h-3" /> {previewTemplate.updatedAt}
                    </span>
                  </div>
                  <div className="flex gap-1">
                    {previewTemplate.techStack.map((tech) => (
                      <span
                        key={tech}
                        className="px-1.5 py-0.5 rounded text-[7px] bg-white/10 text-white/60"
                      >
                        {tech}
                      </span>
                    ))}
