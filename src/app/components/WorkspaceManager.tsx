        ...
        children: [
          {
            ...
            children: [
                ...
                versions: [
                  {
                    ...
                    size: 12840,
                    author: 'You',
                    message: 'Add streaming response handler',
                    hash: 'a3f2e1d',
                  },
                  {
                    id: 'v4',
                    ...
                    size: 11200,
                    author: 'Alice',
                    message: 'Refactor message rendering',
                    hash: 'b4c3d2e',
                  },
                  {
                    id: 'v3',
                    ...
                    size: 10500,
                    author: 'You',
                    message: 'Add slash command support',
                    hash: 'c5d4e3f',
                  },
                  {
                    id: 'v2',
                  ...
                  },
                  {
                    ...
                    size: 6200,
                    author: 'You',
                    message: 'Initial chat interface',
                    hash: 'e7f6a5b',
                  },
                ],
              },
              {
                name: 'IDELayout.tsx',
                path: '/src/app/components/IDELayout.tsx',
                type: 'file',
                size: 18600,
                ...
                versions: [
                  {
                    ...
                    size: 18600,
                    author: 'You',
                    message: 'Add database manager panel',
                    hash: 'f1a2b3c',
                  },
                  {
                    id: 'v2',
                    ...
                    size: 16400,
                    author: 'You',
                    message: 'Integrate workspace manager',
                    hash: 'a2b3c4d',
                  },
                  {
                    id: 'v1',
                    ...
                    size: 12000,
                    author: 'Alice',
                    message: 'Initial multi-panel layout',
                    hash: 'b3c4d5e',
                  },
                ],
              },
              {
                ...
                path: '/src/app/components/PreviewPanel.tsx',
                type: 'file',
                size: 15200,
              ...
              {
                name: 'CodeEditor.tsx',
                path: '/src/app/components/CodeEditor.tsx',
                type: 'file',
                size: 9800,
              ...
              },
              {
                ...
                path: '/src/app/components/FileManager.tsx',
                type: 'file',
                size: 7600,
          ...
          },
          {
            ...
            children: [
              ...
              {
                name: 'i18n-data.ts',
                path: '/src/app/utils/i18n-data.ts',
                type: 'file',
                size: 62000,
                modified: Date.now() - 1800000,
                encoding: 'UTF-8',
              ...
              {
                name: 'preview-engine.ts',
                path: '/src/app/utils/preview-engine.ts',
                type: 'file',
                size: 14500,
                modified: Date.now() - 7200000,
                encoding: 'UTF-8',
...
export function WorkspaceManager({ open, onClose }: WorkspaceManagerProps) {
  ...
  const handleRollback = useCallback(
    (file: FileNode, versionId: string) => {
    ...
    nodes
      ...
      .map((node) => (
        <div key={node.path}>
          <button
            onClick={() => {
              node.type === 'folder' ? toggleExpand(node.path) : setSelectedFile(node);
            }}
          ...
          >
            {node.type === 'folder' ? (
              <>
                ...
                <Folder
                  className={`w-3.5 h-3.5 flex-shrink-0 ${node.expanded ? 'text-indigo-400' : 'text-slate-400'}`}
                />
              </>
            ) : (
              <>
            ...
            <span
              className={`truncate ${t.text.primary}`}
              style={{ fontWeight: node.type === 'folder' ? 500 : 400 }}
            >
              {node.name}
            </span>
  ...
  return (
    <>
      ...
      {contextMenu && (
        <div
          className={`fixed z-[80] w-44 rounded-xl overflow-hidden ${t.surface.popover} ${t.border.popover} shadow-2xl`}
        ...
        >
          {[
            ...
            { icon: Download, label: ii.wmDownload, action: () => toast.success(ii.wmDownload) },
            {
              icon: Trash2,
              label: ii.wmDelete,
              action: () => toast.success(ii.wmDelete),
              danger: true,
            },
          ].map((item, _idx) => (
            <button
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`text-[10px] ${t.text.dimmed}`}>
                  {ii.wmSubtitle} · {stats.files} {ii.wmTotalFiles} · {formatSize(stats.size)}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <div
                className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[8px] ${t.isDark ? 'bg-white/[0.03]' : 'bg-slate-50'} ${t.text.dimmed}`}
              >
          ...
          <div className="flex flex-1 overflow-hidden">
            ...
            <div className={`w-64 flex-shrink-0 flex flex-col border-r ${t.border.subtle}`}>
              ...
              <div className={`px-2 py-1.5 border-b ${t.border.subtle}`}>
                <div
                  className={`flex items-center gap-1.5 px-2 py-1 rounded-lg ${t.isDark ? 'bg-white/[0.03]' : 'bg-slate-50'}`}
                >
                ...
                {sideTab === 'files'
                  ...
                  : RECENT_FILES.map((rf) => (
                      ...
                      >
                        ...
                        <span className={`text-[7px] flex-shrink-0 ${t.text.dimmed}`}>
                          {timeAgo(rf.time)}
                        </span>
                      </button>
                    ))}
              ...
              <div className={`flex items-center gap-1 px-2 py-1.5 border-t ${t.border.subtle}`}>
                <button
                  ...
                  title={ii.wmCreateFile}
                >
                  <Plus className="w-3.5 h-3.5" />
                </button>
                <button
                  className={`p-1 rounded ${t.transition} ${t.interactive.iconBtn}`}
                  title={ii.wmCreateFolder}
                >
                  <FolderPlus className="w-3.5 h-3.5" />
                </button>
                <button
                  onClick={() => setShowDragZone(!showDragZone)}
                  className={`p-1 rounded ${t.transition} ${t.interactive.iconBtn}`}
                  title={ii.wmUpload}
                >
              ...
              {selectedFile ? (
                <>
                  ...
                  <div className={`flex items-center gap-3 px-4 py-3 border-b ${t.border.subtle}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div
                        className={`text-[12px] font-mono ${t.text.primary}`}
                    ...
                    <div className="flex items-center gap-3">
                      {selectedFile.size && (
                        <div className={`text-[8px] ${t.text.dimmed}`}>
                          {ii.wmSize}: {formatSize(selectedFile.size)}
                        </div>
                      )}
                      {selectedFile.encoding && (
                        <div className={`text-[8px] ${t.text.dimmed}`}>{selectedFile.encoding}</div>
                      )}
                      {selectedFile.modified && (
                        <div className={`text-[8px] ${t.text.dimmed}`}>
                          {ii.wmModified}: {timeAgo(selectedFile.modified)}
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className={`px-4 py-2 border-b ${t.border.subtle}`}>
                    ...
                    <span className={`text-[8px] ml-2 ${t.text.dimmed}`}>
                      {selectedFile.versions?.length || 0} {ii.wmVersions}
                    </span>
                  </div>
                      ...
                      <div className="p-4 space-y-0">
                        {selectedFile.versions.map((ver, idx) => (
                          <div key={ver.id} className="flex gap-3">
                            ...
                            >
                              <div
                                className={`w-3 h-3 rounded-full border-2 flex-shrink-0 ${idx === 0 ? 'bg-indigo-500 border-indigo-400' : t.isDark ? 'bg-slate-700 border-slate-600' : 'bg-slate-200 border-slate-300'}`}
                              />
                              {idx < (selectedFile.versions?.length || 0) - 1 && (
                                <div
                                  className={`w-px flex-1 min-h-[32px] ${t.isDark ? 'bg-slate-700' : 'bg-slate-200'}`}
                                />
                              )}
                            </div>
                            {/* Content */}
                            <div className={`flex-1 pb-4 ${idx === 0 ? '' : ''}`}>
                              <div className="flex items-center gap-2">
                                <span
                                  className={`text-[9px] font-mono px-1.5 py-0.5 rounded ${t.isDark ? 'bg-indigo-500/10 text-indigo-300' : 'bg-indigo-50 text-indigo-600'}`}
                                >
                                  {ver.hash}
                                </span>
                                {idx === 0 && (
                                  <span
                                    className={`text-[7px] px-1.5 py-0.5 rounded-full ${t.isDark ? 'bg-emerald-500/15 text-emerald-400' : 'bg-emerald-50 text-emerald-600'}`}
                                  >
                                    {ii.wmCurrentVersion}
                                  </span>
                                )}
                              </div>
                              <div className={`text-[10px] mt-1 ${t.text.primary}`}>
                                {ver.message}
                              </div>
                              <div className={`flex items-center gap-3 mt-1`}>
                                <span className={`text-[8px] ${t.text.dimmed}`}>{ver.author}</span>
                                <span className={`text-[8px] ${t.text.dimmed}`}>
                                  {timeAgo(ver.timestamp)}
                                </span>
                                <span className={`text-[8px] ${t.text.dimmed}`}>
                                  {formatSize(ver.size)}
                                </span>
                                {idx > 0 && (
                                  <button
                                    onClick={() => handleRollback(selectedFile, ver.id)}
                ...
                <div
                  ...
                  onDrop={(e) => {
                    e.preventDefault();
                    toast.success(ii.wmUpload);
                    setShowDragZone(false);
                  }}
                >
                  <div
                    className={`w-full max-w-md border-2 border-dashed rounded-2xl p-12 text-center ${t.isDark ? 'border-indigo-500/30' : 'border-indigo-200'}`}
                  >
                    <Upload
                      className={`w-10 h-10 mx-auto mb-3 ${t.isDark ? 'text-indigo-400/40' : 'text-indigo-300'}`}
                    />
                ...
                <div
                  className={`flex-1 flex flex-col items-center justify-center gap-2 ${t.text.dimmed}`}
                >
