...
import {
  Sparkles, Code, Play, Copy, Check, Loader2,
  Image, FileText, Zap, Download, Eye,
  ChevronRight, RotateCcw, Monitor
} from 'lucide-react'
import { useState, useRef, useCallback } from 'react'
import { toast } from 'sonner'

import { mvpService, type MVPGenerationRequest, type MVPStreamChunk } from '../services/mvp-service'
    ...
    <div className={`h-full flex flex-col ${t.surface.app}`}>
      ...
      <div className={`flex items-center justify-between px-4 py-3 border-b ${t.border.subtle}`}>
        <div className="flex items-center gap-2">
          <div className={`w-8 h-8 rounded-lg ${t.accent.primaryBg} flex items-center justify-center`}>
          ...
          <div className="p-4 space-y-3">
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex flex-wrap gap-1.5">
                <button
                  onClick={() => handleQuickTemplate('Create a responsive navigation bar with logo, menu items, and mobile hamburger menu')}
                ...
                <button
                  onClick={() => handleQuickTemplate('Create a pricing card component with 3 tiers: Basic, Pro, and Enterprise')}
                  className={`px-2 py-1 rounded text-[9px] ${t.isDark ? 'bg-white/[0.04] hover:bg-white/[0.08]' : 'bg-slate-50 hover:bg-slate-100'} ${t.text.secondary} transition-all`}
                  disabled={generating}
                >
                  💰 Pricing
                ...
                <button
                  onClick={() => handleQuickTemplate('Create a dashboard card showing user statistics with icons and charts')}
                  className={`px-2 py-1 rounded text-[9px] ${t.isDark ? 'bg-white/[0.04] hover:bg-white/[0.08]' : 'bg-slate-50 hover:bg-slate-100'} ${t.text.secondary} transition-all`}
                  disabled={generating}
                >
                  📊 Dashboard
            ...
            <div className="pt-2">
              ...
              <div className={`flex items-center gap-2 px-3 py-2 rounded-lg ${t.isDark ? 'bg-white/[0.02]' : 'bg-slate-50'} border ${t.isDark ? 'border-white/[0.06]' : 'border-slate-200'}`}>
            ...
            <button
              ...
              className={`w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl ${
                generating || !description.trim()
                  ? t.isDark ? 'bg-white/[0.05] text-white/30' : 'bg-slate-100 text-slate-400'
                  : t.accent.solidBtn
              } transition-all`}
            >
              {generating ? (
                <>
                  ...
                  <span className="text-[12px]" style={{ fontWeight: 500 }}>Generating...</span>
                </>
              ) : (
                <>
                  <Zap className="w-4 h-4" />
                  <span className="text-[12px]" style={{ fontWeight: 500 }}>Generate Component</span>
                </>
              )}
            </button>
          </div>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-hidden">
            {generatedCode ? (
              showPreview ? (
                /* Preview Panel */
                <div className={`h-full p-4 overflow-auto ${t.isDark ? 'bg-[#0a0a0a]' : 'bg-white'}`}>
                  <div className={`text-[12px] ${t.text.muted} mb-3`}>
                    Live Preview - {filename}
                  </div>
                  <div className={`p-6 rounded-xl border ${t.isDark ? 'border-white/[0.06] bg-white/[0.02]' : 'border-slate-200 bg-slate-50'}`}>
                    ...
                    <div className={`text-center py-12 ${t.text.muted}`}>
                      ...
                      <p className="text-[10px] mt-1">In production, this would render the generated component</p>
                    </div>
                  </div>
                </div>
              ) : (
                ...
                <div className="h-full flex flex-col">
                  <div className={`px-4 py-2 border-b ${t.border.subtle} flex items-center justify-between`}>
                    <div className="flex items-center gap-2">
                      <FileText className={`w-3.5 h-3.5 ${t.text.dimmed}`} />
                      <span className={`text-[11px] ${t.text.secondary} font-mono`}>{filename}</span>
                    </div>
                    {streaming && (
                  ...
                  <pre className={`flex-1 p-4 overflow-auto text-[11px] font-mono leading-relaxed ${t.isDark ? 'bg-[#0d1117] text-slate-300' : 'bg-slate-50 text-slate-800'}`}>
              ...
              <div className="h-full flex flex-col items-center justify-center text-center">
                <div className={`w-20 h-20 rounded-2xl ${t.isDark ? 'bg-white/[0.02]' : 'bg-slate-50'} flex items-center justify-center mb-4`}>
                ...
                <p className={`text-[11px] ${t.text.muted} max-w-[300px]`}>
                  Describe your component idea and watch AI generate production-ready code in seconds
                </p>
              </div>
            )}
          </div>
