...
const PRESET_LAYOUTS: {
  ...
  {
    id: 'preset-write',
    labelKey: 'lmLayoutWrite',
    icon: Edit3,
    panelMap: { left: 'files', middle: 'code', right: 'preview' },
    viewMode: 'code',
  },
  {
    id: 'preset-debug',
    labelKey: 'lmLayoutDebug',
    icon: Terminal,
    panelMap: { left: 'code', middle: 'terminal', right: 'preview' },
    viewMode: 'code',
  },
  {
    id: 'preset-review',
    labelKey: 'lmLayoutReview',
    icon: Eye,
    panelMap: { left: 'chat', middle: 'code', right: 'preview' },
    viewMode: 'code',
  },
  {
    id: 'preset-data',
    labelKey: 'lmLayoutData',
    icon: Database,
    panelMap: { left: 'database', middle: 'workspace', right: 'code' },
    viewMode: 'code',
  },
];
    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          <div
            className={`absolute top-full left-0 mt-1 z-50 w-36 rounded-xl overflow-hidden ${t.surface.popover} ${t.border.popover} shadow-xl`}
          >
            <div className={`px-2 py-1 text-[7px] uppercase tracking-wider ${t.text.dimmed}`}>
              {i.ptSelectPanel}
            </div>
            {PANEL_TYPES.map((pt) => (
              <button
                ...
                className={`w-full flex items-center gap-2 px-2.5 py-1.5 text-[9px] text-left ${t.transition} ${
                  current === pt.id
                    ? `${t.isDark ? 'bg-indigo-500/10' : 'bg-indigo-50'} ${t.accent.primary}`
...
export function LayoutManager({ open, onClose }: LayoutManagerProps) {
  ...
  const handleLoadPreset = useCallback(
    (preset: (typeof PRESET_LAYOUTS)[0]) => {
        ...
        >
          ...
          >
            <div className="flex items-center gap-3">
              <div
                className={`w-8 h-8 rounded-xl flex items-center justify-center ${t.isDark ? 'bg-gradient-to-br from-purple-500/20 to-pink-500/20' : 'bg-gradient-to-br from-purple-50 to-pink-50'}`}
              >
                <LayoutGrid
                  className={`w-4 h-4 ${t.isDark ? 'text-purple-400' : 'text-purple-500'}`}
                />
              </div>
              <div>
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() => {
                  ...
                  toast.success(layoutAutoSave ? i.lmAutoSaveOff : i.lmAutoSaveOn);
                }}
          ...
          <div className={`flex-1 overflow-y-auto ${t.scrollbar} p-4 space-y-4`}>
            ...
            <div
              className={`flex items-center gap-3 p-3 rounded-xl ${t.isDark ? 'bg-white/[0.02] border border-white/[0.04]' : 'bg-slate-50 border border-slate-100'}`}
            >
              <span
              ...
              <div className="flex items-center gap-2 flex-1">
                {(['left', 'middle', 'right'] as const).map((slot) => {
                  ...
                  return (
                    <div
                      key={slot}
                      className={`flex items-center gap-1.5 px-2 py-1 rounded-lg text-[9px] ${t.isDark ? 'bg-white/[0.04]' : 'bg-white'}`}
                    >
                      <meta.icon className="w-3 h-3" style={{ color: meta.color }} />
            ...
            <div>
              ...
              <div className="grid grid-cols-5 gap-2 mt-2">
                {PRESET_LAYOUTS.map((preset) => {
                  const isActive =
                    panelMap.left === preset.panelMap.left &&
                    panelMap.middle === preset.panelMap.middle &&
                    panelMap.right === preset.panelMap.right;
                  return (
                    <button
                      key={preset.id}
                    ...
                    >
                      <preset.icon
                        className={`w-5 h-5 ${isActive ? 'text-indigo-400' : t.text.muted}`}
                      />
                      <span
                        className={`text-[9px] ${isActive ? t.accent.primary : t.text.muted}`}
                        style={{ fontWeight: isActive ? 600 : 400 }}
                      >
                        {i[preset.labelKey as keyof I18nStrings]}
                      </span>
                      {/* Mini column preview */}
            ...
            <div>
              <span
                ...
                style={{ fontWeight: 600 }}
              >
                {i.lmSaved} ({savedLayouts.length})
              </span>
              <div className="space-y-1.5 mt-2">
                {savedLayouts.length === 0 ? (
                  <div
                    className={`flex items-center justify-center py-6 text-[10px] ${t.text.dimmed}`}
                  >
                  ...
                  savedLayouts.map((layout) => (
                    <div
                      key={layout.id}
                      className={`flex items-center gap-2 px-3 py-2 rounded-xl ${t.transition} ${
                    ...
                    >
                      {editingId === layout.id ? (
                        ...
                        <span
                          className={`flex-1 text-[10px] ${t.text.primary}`}
                      ...
                      <div className="flex items-center gap-0.5">
                        <button
                          onClick={() => handleLoadSaved(layout.id)}
                          className={`p-1 rounded ${t.transition} ${t.interactive.iconBtn}`}
                        >
            ...
            {/* Save new layout */}
            <div
              className={`flex items-center gap-2 p-3 rounded-xl border ${t.isDark ? 'border-white/[0.04] bg-white/[0.01]' : 'border-slate-100 bg-slate-50'}`}
            >
