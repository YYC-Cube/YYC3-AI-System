...
import {
  Shield, X, UserPlus, Trash2, ChevronDown, Check,
  Crown, Settings, Pen, Eye, Users, Mail, Ban,
  ShieldCheck, BookOpen, Pencil, Rocket
} from 'lucide-react'
...
const MOCK_MEMBERS: TeamMember[] = [
  { id: 'u1', name: 'You',   email: 'you@yyc3.dev',   avatar: 'Y', color: '#818cf8', role: 'owner',  status: 'active',  lastActive: Date.now() },
  { id: 'u2', name: 'Alice', email: 'alice@yyc3.dev',  avatar: 'A', color: '#6366f1', role: 'admin',  status: 'active',  lastActive: Date.now() - 600000 },
  { id: 'u3', name: 'Bob',   email: 'bob@yyc3.dev',    avatar: 'B', color: '#f59e0b', role: 'editor', status: 'active',  lastActive: Date.now() - 3600000 },
  { id: 'u4', name: 'Carol', email: 'carol@yyc3.dev',  avatar: 'C', color: '#10b981', role: 'viewer', status: 'pending', lastActive: Date.now() - 86400000 },
  { id: 'u5', name: 'Dave',  email: 'dave@yyc3.dev',   avatar: 'D', color: '#ec4899', role: 'editor', status: 'blocked', lastActive: Date.now() - 172800000 },
]
...
interface RbacPanelProps { open: boolean; onClose: () => void }
...
export function RbacPanel({ open, onClose }: RbacPanelProps) {
  ...
  const changeRole = useCallback((memberId: string, newRole: Role) => {
  ...
  const inviteMember = useCallback(() => {
    ...
    const newMember: TeamMember = {
      id: `u-${Date.now()}`, name: inviteEmail.split('@')[0],
      email: inviteEmail, avatar: inviteEmail[0].toUpperCase(),
      color: '#94a3b8', role: inviteRole, status: 'pending',
      lastActive: Date.now(),
    }
        ...
        >
          ...
          <div className={`flex items-center justify-between px-6 py-3 border-b ${t.border.subtle}`}>
            <div className="flex items-center gap-3">
              <div className={`w-8 h-8 rounded-xl flex items-center justify-center ${t.isDark ? 'bg-gradient-to-br from-amber-500/20 to-orange-500/20' : 'bg-gradient-to-br from-amber-50 to-orange-50'}`}>
                <Shield className={`w-4 h-4 ${t.isDark ? 'text-amber-400' : 'text-amber-500'}`} />
              </div>
              <div>
                <h2 className={`text-[14px] ${t.text.primary}`} style={{ fontWeight: 700 }}>{i.rbTitle}</h2>
                <p className={`text-[10px] ${t.text.dimmed}`}>{i.rbSubtitle} · {members.length} {i.rbMembers}</p>
              </div>
            </div>
          ...
          <div className={`flex items-center gap-2 px-6 py-2 border-b ${t.border.subtle}`}>
            {ROLE_CONFIG.map(rc => {
              ...
              return (
                <div key={rc.role} className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[8px] ${t.isDark ? 'bg-white/[0.03]' : 'bg-slate-50'}`}>
                  <Icon className="w-2.5 h-2.5" style={{ color: rc.color }} />
                  <span className={t.text.muted}>{(i as unknown as Record<string, string>)[rc.labelKey]}</span>
          ...
          <div className={`flex items-center gap-2 px-6 py-2 border-b ${t.border.subtle}`}>
            ...
            >
              {ROLE_CONFIG.filter(r => r.role !== 'owner').map(r => (
                <option key={r.role} value={r.role}>{(i as unknown as Record<string, string>)[r.labelKey]}</option>
              ))}
            </select>
            <button
              ...
              className={`flex items-center gap-1 px-3 py-1 rounded-lg text-[9px] ${t.transition} ${
                inviteEmail.includes('@') ? t.accent.solidBtn + ' text-white' : 'opacity-30 cursor-not-allowed'
              }`}
          ...
          <div className={`grid gap-0 px-6 py-1.5 border-b ${t.border.subtle}`} style={{ gridTemplateColumns: '2fr repeat(5, 1fr)' }}>
            <div className={`text-[7px] uppercase tracking-wider ${t.text.dimmed}`} style={{ fontWeight: 600 }}>{i.rbMembers}</div>
            {PERM_CONFIG.map(pc => {
              const PIcon = pc.icon
              return (
                <div key={pc.perm} className={`text-[7px] uppercase tracking-wider text-center ${t.text.dimmed}`} style={{ fontWeight: 600 }}>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    <div className="w-7 h-7 rounded-full flex items-center justify-center text-[10px] text-white flex-shrink-0" style={{ backgroundColor: member.color, fontWeight: 700 }}>
                      {member.avatar}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        <span className={`text-[10px] truncate ${t.text.primary}`} style={{ fontWeight: 600 }}>{member.name}</span>
                        {member.status === 'pending' && (
                          <span className={`text-[7px] px-1 py-0 rounded ${t.isDark ? 'bg-amber-500/15 text-amber-400' : 'bg-amber-50 text-amber-600'}`}>{i.rbPending}</span>
                        )}
                        {isBlocked && (
                          <span className={`text-[7px] px-1 py-0 rounded ${t.isDark ? 'bg-red-500/15 text-red-400' : 'bg-red-50 text-red-600'}`}>{i.rbBlocked}</span>
                        )}
                      </div>
                      <div className="flex items-center gap-1">
                        <span className={`text-[8px] ${t.text.dimmed}`}>{member.email}</span>
                        <span className={`text-[7px] ${t.text.dimmed}`}> · {i.rbLastActive}: {formatTime(member.lastActive)}</span>
                      </div>
                    </div>
                    {/* Role selector */}
                    <div className="relative">
                      ...
                      >
                        ...
                        <span style={{ color: rc.color, fontWeight: 500 }}>{(i as unknown as Record<string, string>)[rc.labelKey]}</span>
                        {member.role !== 'owner' && <ChevronDown className="w-2 h-2" />}
                      </button>
                      {roleDropdown === member.id && (
                        <div className={`absolute right-0 top-full mt-1 z-10 rounded-lg overflow-hidden min-w-[100px] ${t.surface.popover} ${t.border.popover} shadow-xl`}>
                          {ROLE_CONFIG.filter(r => r.role !== 'owner').map(r => {
                            const RI = r.icon
                            return (
                              <button
                                ...
                                className={`w-full flex items-center gap-1.5 px-3 py-1.5 text-[9px] text-left ${t.transition} ${
                                  member.role === r.role ? t.accent.primaryBg + ' ' + t.accent.primary : t.interactive.menuItem
                                }`}
                              >
                                <RI className="w-3 h-3" style={{ color: r.color }} />
                                {(i as unknown as Record<string, string>)[r.labelKey]}
                                {member.role === r.role && <Check className="w-2.5 h-2.5 ml-auto" />}
                              </button>
                            )
                          })}
                          <div className={`border-t ${t.border.subtle}`} />
                          <button onClick={() => toggleBlock(member.id)} className={`w-full flex items-center gap-1.5 px-3 py-1.5 text-[9px] text-left ${t.transition} ${t.interactive.menuItem} ${isBlocked ? 'text-emerald-400' : 'text-red-400'}`}>
                            <Ban className="w-3 h-3" /> {isBlocked ? i.rbUnblock : i.rbBlock}
                          </button>
                          {member.role !== 'owner' && (
                            <button onClick={() => removeMember(member.id)} className={`w-full flex items-center gap-1.5 px-3 py-1.5 text-[9px] text-left text-red-400 ${t.transition} ${t.interactive.menuItem}`}>
                    ...
                    return (
                      <div key={pc.perm} className="flex items-center justify-center">
                        <div className={`w-4 h-4 rounded flex items-center justify-center ${
                          hasPerm
                            ? (t.isDark ? 'bg-emerald-500/20' : 'bg-emerald-50')
                            : (t.isDark ? 'bg-white/[0.03]' : 'bg-slate-50')
                        }`}>
                          {hasPerm ? (
