...
import {
  MessageSquare, Plus, Check, RotateCcw, Trash2,
  Send, ChevronDown, ChevronRight, Eye, EyeOff,
...
import { codeReviewer } from '../services/ai-code-gen'
...
const MOCK_COMMENTS: ReviewComment[] = [
  {
    id: 'rc1', file: 'ChatInterface.tsx', line: 33,
    author: 'Alice', color: '#6366f1',
    ...
    timestamp: Date.now() - 3600000, resolved: false,
    replies: [
      { id: 'rr1', author: 'Bob', color: '#f59e0b', text: 'It\'s used in the useEffect on line 64. The warning is a false positive from the scope check.', timestamp: Date.now() - 3000000 },
      { id: 'rr2', author: 'Alice', color: '#6366f1', text: 'Ah I see, the effect accesses it via `.current`. Makes sense, thanks!', timestamp: Date.now() - 2400000 },
    ],
  },
  {
    id: 'rc2', file: 'ChatInterface.tsx', line: 88,
    author: 'Carol', color: '#10b981',
    ...
    timestamp: Date.now() - 7200000, resolved: false,
    replies: [],
  },
  {
    id: 'rc3', file: 'store.ts', line: 35,
    author: 'Dave', color: '#ec4899',
    text: 'Critical: `initialDesignRoot` is undefined here. This will cause a runtime error. Need to define or import it.',
    timestamp: Date.now() - 86400000, resolved: true,
    replies: [
      { id: 'rr3', author: 'You', color: '#818cf8', text: 'Fixed in commit a1b2c3d - moved initialDesignRoot to a constants file.', timestamp: Date.now() - 82800000 },
    ],
  },
  {
    id: 'rc4', file: 'ChatInterface.tsx', line: 102,
    author: 'Bob', color: '#f59e0b',
    text: 'The async function simulateAIResponse doesn\'t have error handling. Should wrap in try-catch for production.',
    timestamp: Date.now() - 1800000, resolved: false,
    replies: [],
  },
]
...
export function CodeReviewPanel() {
  ...
  const handleJumpToLine = useCallback((file: string, line: number) => {
  ...
  const addReply = useCallback((commentId: string) => {
    ...
    const reply: ReviewReply = {
      id: `rr-${Date.now()}`, author: 'You', color: '#818cf8', text, timestamp: Date.now(),
    }
    setComments(prev => prev.map(c =>
  ...
  const addNewComment = useCallback(() => {
    ...
    const comment: ReviewComment = {
      id: `rc-${Date.now()}`, file: currentFile, line: newCommentLine,
      author: 'You', color: '#818cf8', text: newCommentText.trim(),
      timestamp: Date.now(), resolved: false, replies: [],
    }
...
export function ${currentFile.replace('.tsx', '').replace('.ts', '')}() {
  ...
  return <div>{/* component content for ${currentFile} - a long line that exceeds one hundred and twenty characters in width for testing purposes */}</div>
      ...
      const aiComments: ReviewComment[] = result.issues.map((issue: any, idx: number) => ({
        id: `ai-${Date.now()}-${idx}`,
        file: currentFile,
        line: issue.line,
        author: 'YYC³ AI',
      ...
      result.suggestions.forEach((suggestion: any, idx: number) => {
        aiComments.push({
      ...
      if (aiComments.length > 0) {
        ...
        toast.success(`${i.acgReviewComplete} — ${i.acgScore}: ${result.score}/100 · ${result.issues.length} ${i.acgIssues}`)
      } else {
    ...
    <div className={`flex flex-col h-full overflow-hidden ${t.surface.glass}`}>
      ...
      <div className={`flex items-center justify-between px-3 py-2 border-b ${t.border.subtle}`}>
        <div className="flex items-center gap-2">
          <MessageSquare className={`w-3.5 h-3.5 ${t.isDark ? 'text-amber-400' : 'text-amber-500'}`} />
          <span className={`text-[11px] ${t.text.secondary}`} style={{ fontWeight: 600 }}>{i.crTitle}</span>
          <div className="flex items-center gap-1.5">
            <span className={`text-[9px] px-1.5 py-0.5 rounded-full ${t.isDark ? 'bg-amber-500/15 text-amber-400' : 'bg-amber-50 text-amber-600'}`}>
              {counts.open} {i.crOpen}
            </span>
            {counts.resolved > 0 && (
              <span className={`text-[9px] px-1.5 py-0.5 rounded-full ${t.isDark ? 'bg-emerald-500/15 text-emerald-400' : 'bg-emerald-50 text-emerald-600'}`}>
        ...
        <div className="flex items-center gap-1">
          <button onClick={() => setAddingComment(!addingComment)} className={`p-1 rounded ${t.transition} ${addingComment ? t.accent.activeText : ''} ${t.interactive.iconBtn}`} title={i.crAddComment}>
            <Plus className="w-3 h-3" />
          </button>
          <button onClick={() => setShowResolved(!showResolved)} className={`p-1 rounded ${t.transition} ${showResolved ? t.accent.activeText : ''} ${t.interactive.iconBtn}`} title={showResolved ? i.crHideResolved : i.crShowResolved}>
          ...
          <div className="flex items-center gap-1 ml-2 px-2 py-1 rounded-lg bg-white/[0.04] border border-white/[0.06]">
            {activeModel ? (
              <>
                ...
                <span className="text-[8px] text-white/40 truncate max-w-[80px]">{activeModel.name}</span>
              </>
            ) : (
              <>
          ...
          <button onClick={handleAiReview} disabled={!activeModel} className={`p-1 rounded ${t.transition} ${aiReviewing ? t.accent.activeText : ''} ${t.interactive.iconBtn} ${!activeModel ? 'opacity-30 cursor-not-allowed' : ''}`} title={activeModel ? i.acgReview : '请先配置 AI 模型'}>
          ...
          >
            <div className={`px-3 py-2 border-b ${t.border.subtle} space-y-1.5`}>
              <div className="flex items-center gap-2">
                ...
                <input type="number" min="1" value={newCommentLine} onChange={e => setNewCommentLine(Number(e.target.value))}
              ...
              <div className="flex justify-end gap-1">
                <button onClick={() => setAddingComment(false)} className={`px-2 py-0.5 rounded text-[9px] ${t.transition} ${t.interactive.iconBtn}`}>{i.arDismiss}</button>
                <button onClick={addNewComment} className={`flex items-center gap-1 px-2 py-0.5 rounded text-[9px] ${t.transition} ${t.accent.solidBtn}`} style={{ fontWeight: 600 }}>
          ...
          return (
            ...
            >
              ...
              >
                ...
                <div className="w-5 h-5 rounded-full flex items-center justify-center text-[8px] text-white flex-shrink-0" style={{ backgroundColor: comment.color, fontWeight: 700 }}>
                  {comment.author[0]}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    <span className={`text-[9px] ${t.text.primary}`} style={{ fontWeight: 600 }}>{comment.author}</span>
                    <button
                      onClick={(e) => {
                        e.stopPropagation()
                    ...
                    <span className={`text-[8px] ${t.text.dimmed}`}>{formatTime(comment.timestamp)}</span>
                  ...
                  {comment.replies.length > 0 && (
                    <span className={`text-[8px] ${t.text.dimmed}`}>{comment.replies.length} {i.crReply}</span>
                  )}
                </div>
              </button>
              ...
              {isExpanded && (
                <div className={`px-3 pb-3 space-y-2 ml-8`}>
                  {/* Full comment text */}
                  <div className={`p-2 rounded-lg text-[9px] ${t.isDark ? 'bg-white/[0.03]' : 'bg-slate-50'} ${t.text.primary}`}>
                    {comment.text}
                  </div>
                  ...
                  {comment.replies.map(reply => (
                    <div key={reply.id} className="flex items-start gap-2">
                      <div className="w-4 h-4 rounded-full flex items-center justify-center text-[7px] text-white flex-shrink-0 mt-0.5" style={{ backgroundColor: reply.color, fontWeight: 700 }}>
                        {reply.author[0]}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1">
                          <span className={`text-[8px] ${t.text.primary}`} style={{ fontWeight: 600 }}>{reply.author}</span>
                          <span className={`text-[7px] ${t.text.dimmed}`}>{formatTime(reply.timestamp)}</span>
                        </div>
                  ...
                  <div className="flex items-center gap-1.5">
                    <input
                      ...
                      onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); addReply(comment.id) } }}
                      placeholder={i.crPlaceholder}
                  ...
                  {/* Actions */}
                  <div className="flex items-center gap-1">
                    <button onClick={() => toggleResolve(comment.id)}
                      ...
                      {comment.resolved ? <><RotateCcw className="w-2.5 h-2.5" /> {i.crReopen}</> : <><Check className="w-2.5 h-2.5" /> {i.crResolve}</>}
                    </button>
