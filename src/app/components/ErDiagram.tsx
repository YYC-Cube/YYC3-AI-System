...
import { Database, X, Plus, Trash2, Key, Link2, Copy, Hash, Type } from 'lucide-react';
...
function generateSQL(entities: Entity[], relations: Relation[]): string {
  const lines: string[] = [
    '-- YYC3 PortAISys - Generated Schema',
  ...
  relations.forEach((r) => {
    ...
    if (from) {
      lines.push(
        `ALTER TABLE "${from.name}" ADD CONSTRAINT "fk_${r.toField}" FOREIGN KEY ("${r.toField}") REFERENCES "${entities.find((e) => e.id === r.from)?.name}" ("${r.fromField}");`
      );
    }
  });
  return lines.join('\n');
...
export function ErDiagram({ open, onClose }: ErDiagramProps) {
  ...
  const handleMouseDown = useCallback(
    (eid: string, ex: number, ey: number) => {
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent) => {
      ...
      setEntities((prev) =>
        prev.map((ent) =>
          ent.id === dragging.id ? { ...ent, x: Math.max(0, nx), y: Math.max(0, ny) } : ent
        )
      );
    },
    [dragging]
        ...
        >
          ...
          >
            <div className="flex items-center gap-3">
              <div
                className={`w-8 h-8 rounded-xl flex items-center justify-center ${t.isDark ? 'bg-gradient-to-br from-cyan-500/20 to-blue-500/20' : 'bg-gradient-to-br from-cyan-50 to-blue-50'}`}
              >
              ...
              <div>
                ...
                <p className={`text-[10px] ${t.text.dimmed}`}>
                  {i.erSubtitle} · {entities.length} {i.erEntityCount} · {totalFields}{' '}
                  {i.erFieldCount}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[9px] ${t.transition} ${showSQL ? t.accent.primaryBg + ' ' + t.accent.primary : t.interactive.iconBtn}`}
              >
                ...
                <defs>
                  <pattern id="er-grid" width="20" height="20" patternUnits="userSpaceOnUse">
                    <path
                      d="M 20 0 L 0 0 0 20"
                      fill="none"
                      stroke={t.isDark ? 'rgba(255,255,255,0.03)' : 'rgba(0,0,0,0.04)'}
                      strokeWidth="0.5"
                    />
                  </pattern>
                </defs>
                  ...
                  return (
                    <g key={rel.id}>
                      <path
                        ...
                        fill="none"
                        stroke={t.isDark ? 'rgba(99,102,241,0.4)' : 'rgba(99,102,241,0.3)'}
                        strokeWidth={1.5}
                        strokeDasharray={rel.type === 'M:N' ? '6 3' : 'none'}
                      />
                      {/* Type label */}
                      <text
                        x={midX}
                  ...
                  return (
                    <g key={entity.id}>
                      ...
                      <rect
                        ...
                        onMouseDown={(e) => {
                          handleMouseDown(
                            entity.id,
                            e.clientX -
                              e.currentTarget.closest('svg')!.getBoundingClientRect().left,
                            e.clientY - e.currentTarget.closest('svg')!.getBoundingClientRect().top
                          );
                      ...
                      <text
                        ...
                        fontSize={8}
                        textAnchor="end"
                        fill={t.isDark ? 'rgba(255,255,255,0.3)' : 'rgba(0,0,0,0.3)'}
                      >
                        ...
                        return (
                          <g key={field.id}>
                            ...
                            <text
                              ...
                              y={fy + 15}
                              fontSize={8}
                              fill={t.isDark ? 'rgba(148,163,184,0.6)' : 'rgba(100,116,139,0.6)'}
                              fontFamily="monospace"
                            >
                              {field.type}
                            </text>
                            ...
                            {field.nullable && (
                              <text
                                ...
                                y={fy + 14}
                                fontSize={7}
                                fill={t.isDark ? 'rgba(255,255,255,0.2)' : 'rgba(0,0,0,0.2)'}
                              >
                                ?
                              </text>
                            )}
              ...
              >
                {showSQL ? (
                  ...
                  (() => {
                    ...
                    return (
                      <div
                        className={`flex flex-col h-full overflow-y-auto p-4 space-y-3 ${t.scrollbar}`}
                      >
                        <div className="flex items-center gap-2">
                          <span
                        ...
                        <div className={`text-[8px] ${t.text.dimmed}`}>
                          {ent.fields.length} {i.erFieldCount}
                        </div>
                        <div className="space-y-1">
                          {ent.fields.map((f) => (
                            <div
                              key={f.id}
                              className={`flex items-center gap-1.5 px-2 py-1.5 rounded-lg ${t.isDark ? 'bg-white/[0.02]' : 'bg-slate-50'}`}
                            >
                              ...
                              <span className={`text-[7px] ml-auto ${t.text.dimmed}`}>
                                {f.type}
                              </span>
                            </div>
                          ))}
                        ...
                        <button
                          onClick={() => addField(ent.id)}
                          className={`w-full flex items-center justify-center gap-1 py-1.5 rounded-lg text-[9px] border border-dashed ${t.border.subtle} ${t.transition} ${t.interactive.menuItem}`}
                        >
                          <Plus className="w-3 h-3" /> {i.erAddField}
                        </button>
                        <button
                          onClick={() => removeEntity(ent.id)}
                          className={`w-full flex items-center justify-center gap-1 py-1.5 rounded-lg text-[9px] text-red-400 border border-dashed border-red-500/20 ${t.transition} hover:bg-red-500/5`}
                        >
