...
function getDiagnostics(file: string): Diagnostic[] {
    ...
    diags.push(
      {
        ...
        quickFixes: [
          {
            id: 'f1',
            labelKey: 'edRemoveUnusedVar',
            label: 'Remove unused variable',
            description: 'Remove the declaration of scrollRef',
            codeChange: '// Removed: const scrollRef = useRef<HTMLDivElement>(null)',
          },
          {
            id: 'f2',
            labelKey: 'edIgnore',
            label: 'Add @ts-ignore',
            description: 'Suppress this warning with @ts-ignore comment',
            codeChange: '// @ts-ignore\nconst scrollRef = useRef<HTMLDivElement>(null)',
          },
        ],
      },
      {
        ...
        quickFixes: [
          {
            id: 'f3',
            labelKey: 'edConvertToOptional',
            label: 'Convert to optional chaining',
            description: "Replace with cmd?.split(' ')",
            codeChange: "const command = cmd?.split(' ')[0] ?? ''",
          },
        ],
      },
      {
        ...
        quickFixes: [
          {
            id: 'f4',
            labelKey: 'edWrapInTryCatch',
            label: 'Wrap in try-catch',
            description: 'Add try-catch block around async operations',
            codeChange:
  ...
  if (file === 'store.ts') {
    diags.push({
      ...
      quickFixes: [
        {
          id: 'f5',
          labelKey: 'edAddMissingImport',
          label: 'Add missing import',
          description: 'Import initialDesignRoot from ./defaults',
          codeChange: "import { initialDesignRoot } from './defaults'",
        },
        {
          id: 'f6',
          labelKey: 'edAddTypeAnnotation',
          label: 'Define inline',
          description: 'Define initialDesignRoot as a local constant',
          codeChange:
  ...
  if (file === 'types.ts') {
    diags.push({
      ...
      severity: 'info',
      message:
        "Interface 'PanelStyle' is declared but not exported. Other modules cannot access it.",
      source: 'yyc3-ai-lint',
      quickFixes: [
        {
          id: 'f7',
          labelKey: 'edAddTypeAnnotation',
          label: 'Add export keyword',
          description: 'Export the interface for use in other modules',
          codeChange: 'export interface PanelStyle { ... }',
        },
      ],
    });
  }
...
export function ErrorDiagnostics() {
  ...
  const severityBorder = (s: DiagSeverity) => {
    switch (s) {
      case 'error':
        return t.isDark ? 'border-l-red-400/40' : 'border-l-red-400';
      case 'warning':
        return t.isDark ? 'border-l-amber-400/40' : 'border-l-amber-400';
      case 'info':
        return t.isDark ? 'border-l-blue-400/40' : 'border-l-blue-400';
    }
  };
      ...
      <div className={`flex items-center justify-between px-3 py-2 border-b ${t.border.subtle}`}>
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1.5">
            {counts.errors > 0 && (
              <span
                className={`flex items-center gap-0.5 text-[9px] px-1.5 py-0.5 rounded-full ${t.isDark ? 'bg-red-500/15 text-red-400' : 'bg-red-50 text-red-600'}`}
              >
            ...
            {counts.warnings > 0 && (
              <span
                className={`flex items-center gap-0.5 text-[9px] px-1.5 py-0.5 rounded-full ${t.isDark ? 'bg-amber-500/15 text-amber-400' : 'bg-amber-50 text-amber-600'}`}
              >
            ...
            {counts.infos > 0 && (
              <span
                className={`flex items-center gap-0.5 text-[9px] px-1.5 py-0.5 rounded-full ${t.isDark ? 'bg-blue-500/15 text-blue-400' : 'bg-blue-50 text-blue-600'}`}
              >
                ...
                >
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className={`text-[10px] ${t.text.primary}`} style={{ fontWeight: 500 }}>
                        {d.message}
                      </div>
                      <div className={`flex items-center gap-2 mt-0.5 text-[8px] ${t.text.dimmed}`}>
                        <span>{d.source}</span>
                        <span>
                          {i.edLine} {d.line}:{d.col}
                        </span>
                        {d.quickFixes.length > 0 && (
                          <span
                            className={`flex items-center gap-0.5 ${t.isDark ? 'text-emerald-400' : 'text-emerald-500'}`}
                          >
                    ...
                    >
                      <div className={`px-3 pb-3 space-y-1.5 ml-7`}>
                        {d.quickFixes.map((fix) => (
                          ...
                          >
                            <Wrench
                              className={`w-3 h-3 flex-shrink-0 ${t.isDark ? 'text-emerald-400' : 'text-emerald-500'}`}
                            />
                            <div className="flex-1 min-w-0">
                              <div
                                className={`text-[9px] ${t.text.primary}`}
