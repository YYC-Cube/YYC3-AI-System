...
import {
  Search, X, CaseSensitive, Regex,
  WholeWord, ChevronDown, ChevronRight, File,
  Replace, ReplaceAll, ChevronUp, Filter, FolderOpen, Check
} from 'lucide-react'
...
export function SearchPanel() {
  ...
  const results = useMemo<SearchResult[]>(() => {
    ...
    try {
      const pattern = useRegex
        ...
        : new RegExp(
            wholeWord ? `\\b${query.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b` : query.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'),
            caseSensitive ? 'g' : 'gi'
          )
      ...
      for (const [file, lines] of Object.entries(MOCK_FILES)) {
        if (searchMode === 'files') {
          ...
          lines.forEach((lineText, idx) => {
            ...
            if (match) {
              matches.push({ file, line: idx + 1, text: lineText, matchStart: match.index, matchEnd: match.index + match[0].length })
            }
  ...
  return (
    <>
      <div className="fixed inset-0 z-[60] bg-black/50 backdrop-blur-sm" onClick={() => setSearchPanelOpen(false)} />
      <div className="fixed inset-0 z-[61] flex items-start justify-center pt-[10vh]">
        <div className={`w-full max-w-2xl max-h-[70vh] rounded-2xl overflow-hidden flex flex-col ${t.surface.popover} ${t.border.popover} shadow-2xl`}>
          ...
          <div className={`px-4 py-3 border-b ${t.border.subtle}`}>
            <div className="flex items-center space-x-2">
              ...
              />
              <button onClick={() => setSearchPanelOpen(false)} className={`p-1 rounded-lg ${t.transition} ${t.interactive.iconBtn}`}>
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="flex items-center space-x-2 mt-2">
              <button
                onClick={() => { setReplaceOpen(!replaceOpen); if (!replaceOpen) setTimeout(() => replaceInputRef.current?.focus(), 50) }}
                className={`p-1 rounded-lg flex-shrink-0 ${t.transition} ${replaceOpen ? t.accent.primary : ''} ${t.interactive.iconBtn}`}
                title={i.srReplace}
              >
                {replaceOpen ? <ChevronUp className="w-3.5 h-3.5" /> : <Replace className="w-3.5 h-3.5" />}
              </button>
              {replaceOpen && (
                <>
                  <input
                  ...
                  <button
                    onClick={() => {
                      ...
                      const count = results.filter(r => !replacedItems.has(`${r.file}:${r.line}`)).length
            ...
            <div className="flex items-center space-x-2 mt-2">
              {/* Mode tabs */}
              <div className={`flex rounded-lg overflow-hidden ${t.isDark ? 'bg-slate-800/50' : 'bg-slate-100'}`}>
                <button
                  onClick={() => setSearchMode('code')}
                  className={`px-2.5 py-1 text-[11px] ${t.transition} ${
                    searchMode === 'code' ? `${t.accent.primaryBg} ${t.accent.primary}` : t.text.muted
                  }`}
                ...
                <button
                  ...
                  className={`px-2.5 py-1 text-[11px] ${t.transition} ${
                    searchMode === 'files' ? `${t.accent.primaryBg} ${t.accent.primary}` : t.text.muted
                  }`}
              ...
              <div className="p-2">
                ...
                {useVirtual ? (
                  <VirtualList
                    ...
                    overscan={8}
                    className="h-[400px]"
                    getKey={(item, idx) => item.kind === 'header' ? `hdr-${item.file}` : `res-${item.result.file}-${item.result.line}-${idx}`}
                    onItemActivate={(item) => {
                      if (item.kind === 'header') {
                    ...
                    focusedClassName={t.isDark ? 'bg-indigo-500/10 rounded-lg' : 'bg-indigo-50 rounded-lg'}
                      ...
                      if (item.kind === 'header') {
                        return (
                          ...
                          >
                            ...
                            <span className={`ml-auto flex-shrink-0 px-1.5 py-0.5 rounded-full text-[9px] ${t.isDark ? 'bg-slate-700/50' : 'bg-slate-200/80'} ${t.text.dimmed}`}>
                              {item.count}
                            </span>
                          </button>
                        )
                        ...
                        return (
                          ...
                          >
                            <button onClick={() => handleResultClick(r.file)} className="flex-1 flex items-center space-x-2 min-w-0">
                              <span className={`flex-shrink-0 text-[10px] w-6 text-right font-mono ${t.text.dimmed}`}>{r.line}</span>
                              <span className="flex-1 text-[12px] font-mono truncate">
                                {r.text.substring(0, r.matchStart)}
                                <span className={`${t.isDark ? 'bg-amber-500/30 text-amber-200' : 'bg-amber-200 text-amber-800'} rounded px-0.5`}>
                                  {r.text.substring(r.matchStart, r.matchEnd)}
                                </span>
                                {replaceOpen && replaceValue && !isReplaced && (
                                  <span className={`${t.isDark ? 'bg-emerald-500/30 text-emerald-300' : 'bg-emerald-100 text-emerald-700'} rounded px-0.5 ml-0.5`}>
                                    {replaceValue}
                                  </span>
                                )}
          ...
          <div className={`flex items-center justify-between px-4 py-2 border-t ${t.border.subtle} text-[10px] ${t.text.dimmed}`}>
            <div className="flex items-center space-x-3">
              ...
              <span><kbd className={`px-1 py-0.5 rounded ${t.kbd}`}>Enter</kbd> open</span>
