...
import {
  ChevronLeft, Eye, Code, Maximize, Search,
  MoreHorizontal, Terminal, Download, Keyboard,
  BookOpen, FilePlus, FolderPlus, RefreshCw, Layers
} from 'lucide-react'
...
export function MiddleToolbar() {
  const { theme, language, viewMode, setViewMode, toggleTerminal, setShortcutsDialogOpen } = useAppStore()
  ...
  const viewButtons: { mode: ViewMode; icon: React.ElementType; label: string; shortcut: string }[] = [
  ...
  return (
    <div className={`h-8 flex items-center px-2 justify-between flex-shrink-0 border-b ${t.transition} ${t.border.subtle} ${t.surface.glassHeader}`}>
      ...
      <div className="flex items-center space-x-0.5">
        ...
        {showSearch ? (
          <div className="relative">
            <Search className={`w-3 h-3 absolute left-2 top-1/2 -translate-y-1/2 ${t.text.muted}`} />
            <input
              type="text"
              value={searchQuery}
        ...
        <div className="relative">
          <button
            onClick={() => { setShowNewFileDialog(!showNewFileDialog); setShowNewFolderDialog(false); setShowMoreMenu(false) }}
          ...
          {showNewFileDialog && (
            <>
              ...
              <div className={`absolute right-0 top-full mt-1 w-52 rounded-xl overflow-hidden z-50 p-3 ${t.surface.popover} ${t.border.popover} ${t.shadow.popover}`}>
                <p className="text-[11px] mb-2" style={{ fontWeight: 600 }}>{i.newFile}</p>
                <input
                  type="text"
                  value={newFileName}
        ...
        <div className="relative">
          <button
            onClick={() => { setShowNewFolderDialog(!showNewFolderDialog); setShowNewFileDialog(false); setShowMoreMenu(false) }}
          ...
          {showNewFolderDialog && (
            <>
              ...
              <div className={`absolute right-0 top-full mt-1 w-52 rounded-xl overflow-hidden z-50 p-3 ${t.surface.popover} ${t.border.popover} ${t.shadow.popover}`}>
                <p className="text-[11px] mb-2" style={{ fontWeight: 600 }}>{i.newFolder}</p>
                <input
                  type="text"
                  value={newFolderName}
        ...
        <div className="relative">
          <button
            onClick={() => { setShowMoreMenu(!showMoreMenu); setShowNewFileDialog(false); setShowNewFolderDialog(false) }}
            className={`p-1 rounded ${t.transition} ${
          ...
          {showMoreMenu && (
            <>
              ...
              <div className={`absolute right-0 top-full mt-1 w-52 rounded-xl overflow-hidden z-50 p-1 ${t.surface.popover} ${t.border.popover} ${t.shadow.popover}`}>
                {[
                  { label: i.toggleTerminal, icon: Terminal, shortcut: 'Ctrl+Shift+T', action: () => toggleTerminal() },
                  { label: i.exportProject, icon: Download, shortcut: '', action: () => toast.success(i.toastExportedZip) },
                  { label: i.refresh, icon: RefreshCw, shortcut: '', action: () => toast.info(i.toastProjectRefreshed) },
                  { label: i.componentLayers, icon: Layers, shortcut: '', action: () => toast.info(i.toastComponentLayersOpened) },
                ...
                {[
                  ...
                  { label: i.documentation, icon: BookOpen, action: () => toast.info(i.toastDocsOpened) },
