...
import {
  FileCode, GitBranch, Braces, Wand2, Copy, Terminal,
  Check, GitCommit, GitMerge, GitPullRequest, RefreshCw,
  Loader2
} from 'lucide-react'
...
export function RightToolbar() {
  ...
  const gitActions = [
    { label: i.gitStatus, icon: GitBranch, desc: 'main • 2 changes', action: () => toast.info(i.toastGitStatusInfo) },
    { label: i.gitCommit, icon: GitCommit, desc: '', action: () => toast.success(i.toastGitCommitted) },
    { label: i.gitPush, icon: GitMerge, desc: '', action: () => toast.success(i.toastPushedToOrigin) },
    { label: i.gitPull, icon: GitPullRequest, desc: '', action: () => toast.success(i.toastAlreadyUpToDate) },
    ...
    <div className={`h-8 flex items-center px-2 justify-between flex-shrink-0 border-b ${t.transition} ${t.border.subtle} ${t.surface.glassHeader}`}>
      ...
      <div className="flex items-center space-x-0.5">
        ...
        <button
          ...
          title={i.formatCode}
        >
          {isFormatting ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Braces className="w-3.5 h-3.5" />}
        </button>
        ...
        >
          {isAiSuggesting ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Wand2 className="w-3.5 h-3.5" />}
        </button>
          ...
          {showGit && (
            <>
              ...
              <div className={`absolute left-0 top-full mt-1 w-56 rounded-xl overflow-hidden z-50 p-1.5 ${t.surface.popover} ${t.border.popover} ${t.shadow.popover}`}>
                <div className={`px-3 py-1.5 text-[10px] uppercase tracking-wider ${t.text.muted}`} style={{ fontWeight: 600 }}>Git</div>
                {gitActions.map(({ label, icon: Icon, desc, action }) => (
                  <button
                    key={label}
                    onClick={() => { action(); setShowGit(false) }}
      ...
      <div className="flex items-center space-x-1.5 min-w-0">
        ...
        <span className={`text-[11px] px-1.5 py-0.5 rounded truncate max-w-[140px] ${t.text.tertiary} ${t.isDark ? 'bg-slate-800/50' : 'bg-slate-100/50'}`} style={{ fontWeight: 400 }}>
          {currentFile}
        </span>
      </div>
    </div>
