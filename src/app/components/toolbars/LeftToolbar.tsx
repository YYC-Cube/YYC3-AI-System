...
import { Bot, Puzzle, Settings, HelpCircle, Loader2, BookOpen, MessageSquare, Zap, Download, Star, Info } from 'lucide-react'
...
export function LeftToolbar() {
  const {
    theme, language, aiModels, activeModelId, activateAIModel, openModelSettings
  } = useAppStore()
  ...
  const helpItems = [
    ...
    { label: i.shortcuts, icon: Zap, action: () => useAppStore.getState().setShortcutsDialogOpen(true) },
    { label: i.helpFeedback, icon: MessageSquare, action: () => toast.info(i.toastFeedbackSent) },
    { label: i.helpAbout, icon: Info, action: () => toast.info(`YYC³ PortAISys v1.0.0\n${i.brandTagline}`) },
  ]

  return (
    <div className={`h-8 flex items-center justify-between px-2 flex-shrink-0 border-b ${t.transition} ${t.border.subtle} ${t.surface.glassHeader}`}>
      {/* Left: AI icon + model selector + connectivity dot */}
      <div className="flex items-center space-x-1.5 min-w-0">
        <div className={`w-5 h-5 rounded flex items-center justify-center flex-shrink-0 ${t.accent.primaryBg}`}>
        ...
        {aiModels.length > 0 ? (
          <>
            ...
            >
              {!activeModelId && (
                <option value="">{i.selectModel}</option>
              )}
              {aiModels.map(m => (
            ...
            {activeModel && (
              <div className="flex items-center flex-shrink-0" title={activeModel.lastTestResult || (activeModel.status === 'connected' ? i.toastConnected : activeModel.status === 'error' ? i.toastConnectionFailed : i.toastNotTested)}>
      ...
      <div className="flex items-center space-x-0.5 flex-shrink-0">
        ...
        <div className="relative">
          <button
            onClick={() => { setShowPlugins(!showPlugins); setShowHelp(false) }}
          ...
          {showPlugins && (
            <>
              ...
              <div className={`absolute left-0 top-full mt-1 w-56 rounded-xl overflow-hidden z-50 p-1.5 ${t.surface.popover} ${t.border.popover} ${t.shadow.popover}`}>
                <div className={`px-3 py-1.5 text-[10px] uppercase tracking-wider ${t.text.muted}`} style={{ fontWeight: 600 }}>{i.pluginExtensions}</div>
                {pluginItems.map(p => (
                  <div key={p.name} className={`flex items-center space-x-2.5 px-3 py-2 rounded-lg ${t.transition} ${t.interactive.menuItem}`}>
                    ...
                    {p.installed ? (
                      <span className="text-[9px] px-1.5 py-0.5 rounded bg-emerald-500/15 text-emerald-400" style={{ fontWeight: 500 }}>{i.pluginInstalled}</span>
                    ) : (
                      <button
                ...
                <button
                  onClick={() => { toast.info(i.pluginMarket); setShowPlugins(false) }}
        ...
        <div className="relative">
          <button
            onClick={() => { setShowHelp(!showHelp); setShowPlugins(false) }}
          ...
          {showHelp && (
            <>
              ...
              <div className={`absolute right-0 top-full mt-1 w-48 rounded-xl overflow-hidden z-50 p-1 ${t.surface.popover} ${t.border.popover} ${t.shadow.popover}`}>
                <div className={`px-3 py-1.5 text-[10px] uppercase tracking-wider ${t.text.muted}`} style={{ fontWeight: 600 }}>{i.helpCenter}</div>
                {helpItems.map(({ label, icon: Icon, action }) => (
                  <button
                    key={label}
                    onClick={() => { action(); setShowHelp(false) }}
