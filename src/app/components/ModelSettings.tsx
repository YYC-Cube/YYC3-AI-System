...
const PROVIDERS: ProviderDef[] = [
  ...
  },
  {
    ...
    apiKeyPlaceholder: 'Enter Zhipu API Key...',
    openaiCompatible: true,
  ...
  },
  {
    ...
    models: [
      ...
      { id: 'qwen-plus', name: 'Qwen-Plus', description: 'msDescQwenPlus', contextWindow: '128K' },
      {
        ...
        description: 'msDescQwen3CoderPlus',
        contextWindow: '128K',
      },
    ],
  },
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span className="text-[13px] text-white/85" style={{ fontWeight: 500 }}>
              {provider.name}
            </span>
        ...
        <div className="flex items-center gap-2">
          ...
          {apiKey && (
            <div className="w-2 h-2 rounded-full bg-emerald-400/60" title="API Key configured" />
          )}
          ...
          <span className="text-[10px] text-white/20">
            {provider.models.length} {i.msModels}
          </span>
          {expanded ? (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[9px] text-white/20 truncate">
                        {resolveKey(i, model.description)}
                      </div>
                    </div>
                    {model.pricing && (
...
function MCPConfigPanel() {
  ...
  const handleImportJson = () => {
    try {
      ...
      const imported: MCPServerConfig[] = Object.entries(mcpServers).map(
        ([name, conf]: [string, unknown]) => ({
          id: 'mcp-' + Date.now() + '-' + name,
          name,
          description: ((conf as Record<string, unknown>).description as string) || name,
          command: ((conf as Record<string, unknown>).command as string) || '',
          args: ((conf as Record<string, unknown>).args as string[]) || [],
          env: ((conf as Record<string, unknown>).env as Record<string, string>) || {},
          enabled: true,
        })
      );
      setServers(imported);
  ...
  const inputCls =
    'bg-white/[0.04] border border-white/[0.06] rounded-lg px-3 py-1.5 text-[10px] text-white/70 font-mono focus:outline-none focus:border-violet-500/40 placeholder:text-white/10';
      ...
      {jsonMode && (
        <div className="space-y-2">
          ...
          <div className="text-[9px] text-white/20 px-1">
            Standard MCP JSON format, compatible with Claude Desktop / Cursor / Windsurf
          </div>
        </div>
      )}
            ...
            <div className="rounded-xl border border-dashed border-violet-500/20 bg-violet-500/[0.03] p-3 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <input
                  ...
                  placeholder={i.msMcpNamePlaceholder}
                  className={inputCls}
                />
                <input
                  value={newServer.command}
                  onChange={(e) => setNewServer({ ...newServer, command: e.target.value })}
                  placeholder={i.msMcpCommandPlaceholder}
                  className={inputCls}
                />
              </div>
              <input
                ...
                placeholder={i.msMcpArgsPlaceholder}
...
function SmartDiagnosticsPanel({
  ...
  const avgLatency = (() => {
    const ls = Object.values(diagnostics)
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          >
            ...
            <div className={`text-[16px] ${card.color}`} style={{ fontWeight: 600 }}>
              {card.value}
            </div>
          ...
          <div key={provider.id} className="space-y-1.5">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[9px] text-white/15">
                ...
                {providerDiags.length} {i.msOnline}
              </span>
            </div>
      ...
      {errorModels > 0 && (
        <div className="rounded-xl border border-amber-500/15 bg-gradient-to-br from-amber-500/[0.04] to-orange-500/[0.02] p-4 space-y-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[12px] text-amber-400/80" style={{ fontWeight: 500 }}>
              {i.arSuggestion}
            </span>
          </div>
...
function AIPerformanceCostPanel({ t }: { t: ReturnType<typeof getThemeTokens> }) {
  ...
  const totalTokens = Array.from(costData.values()).reduce(
    (s, c) => s + c.inputTokens + c.outputTokens,
    0
  );
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          {
            label: '活跃提供商',
            value: providers.filter((p) => p.enabled).length.toString(),
            sub: `/ ${providers.length} 总计`,
          },
          {
            label: '总成本',
          ...
          { label: '推荐提供商', value: bestProvider || '无数据', sub: '基于性能评分' },
          {
            ...
            sub: errors.length > 0 ? errors[0].errorType : '无错误',
          },
        ].map((c, idx) => (
          <div
            key={idx}
            className={`p-3 rounded-xl ${t.isDark ? 'bg-white/[0.03] border border-white/[0.06]' : 'bg-slate-50 border border-slate-200'}`}
          >
            <div className={`text-[10px] ${t.text.dimmed}`}>{c.label}</div>
            <div className={`text-[16px] ${t.text.primary} mt-1`} style={{ fontWeight: 600 }}>
              {c.value}
            </div>
      ...
      <div>
        <div className={`text-[12px] ${t.text.primary} mb-2`} style={{ fontWeight: 600 }}>
          性能指标
        </div>
        {metrics.length === 0 ? (
          <div className={`text-[11px] ${t.text.dimmed} text-center py-6`}>
            暂无性能数据 — 发送 AI 请求后将自动记录
          </div>
        ) : (
          <div
            className={`rounded-xl overflow-hidden border ${t.isDark ? 'border-white/[0.06]' : 'border-slate-200'}`}
          >
            <table className="w-full text-[10px]">
              <thead>
                <tr className={t.isDark ? 'bg-white/[0.03]' : 'bg-slate-50'}>
                  {['提供商', '模型', '延迟', '吞吐量', '成功率', '请求数'].map((h) => (
                    <th
                      key={h}
                      className={`px-3 py-2 text-left ${t.text.dimmed}`}
              ...
              <tbody>
                {metrics.slice(-10).map((m, idx) => (
                  <tr
                    key={idx}
                    className={`border-t ${t.isDark ? 'border-white/[0.04]' : 'border-slate-100'}`}
                  >
                    <td className={`px-3 py-1.5 ${t.text.secondary}`}>{m.providerId}</td>
                    <td className={`px-3 py-1.5 ${t.text.secondary}`}>{m.modelId}</td>
                    <td
                      className={`px-3 py-1.5 ${m.latency > 2000 ? 'text-amber-400' : 'text-emerald-400'}`}
                    >
                      {m.latency}ms
                    </td>
                    <td className={`px-3 py-1.5 ${t.text.secondary}`}>
                      {m.throughput.toFixed(1)} t/s
                    </td>
                    <td
                      className={`px-3 py-1.5 ${m.successRate >= 0.9 ? 'text-emerald-400' : 'text-red-400'}`}
                    >
        ...
        {costData.size === 0 ? (
          ...
          <div className="space-y-1.5">
            {Array.from(costData.entries()).map(([key, data]) => (
              <div
                key={key}
                className={`flex items-center justify-between px-3 py-2 rounded-lg ${t.isDark ? 'bg-white/[0.02]' : 'bg-slate-50'}`}
              >
        ...
        <div>
          ...
          <div className="space-y-1.5">
            {errors.slice(-5).map((e, idx) => (
              <div
                key={idx}
                className={`px-3 py-2 rounded-lg border ${t.isDark ? 'bg-red-500/[0.04] border-red-500/10' : 'bg-red-50 border-red-200'}`}
              >
                <div className="flex items-center justify-between">
                  <span
                    className={`text-[10px] px-1.5 py-0.5 rounded ${t.isDark ? 'bg-red-500/15 text-red-400' : 'bg-red-100 text-red-600'}`}
                  >
                    {e.errorType}
                  </span>
...
export function ModelSettings() {
    ...
    for (const provider of providers) {
      ...
      for (const storeModel of aiModels) {
        if (storeModel.endpoint === url && storeModel.apiKey !== key) {
          const isMatch = PROVIDERS.some(
            (p) =>
              p.id === provider.id &&
              p.models.some((m) => m.id === storeModel.name || m.name === storeModel.name)
          );
    ...
    async (providerId: string, modelId: string) => {
      ...
      try {
        ...
        if (providerId === 'ollama') {
          // Ollama endpoint
          const chatUrl = baseUrl.includes('/api/chat')
            ? baseUrl
            : baseUrl.replace(/\/api\/.*$/, '').replace(/\/+$/, '') + '/api/chat';
  ...
  const activeModelKey = useMemo(() => {
    ...
    for (const provider of allProviders) {
      for (const model of provider.models) {
        ...
        if (active.name.toLowerCase() === model.name.toLowerCase())
      ...
      .then((data) => {
        ...
        const models: OllamaDetectedModel[] = (data.models || []).map(
          (m: Record<string, unknown>) => ({
            ...
            quantization: ((m.details as Record<string, unknown>)?.quantization_level ||
              (m.details as Record<string, unknown>)?.family ||
              'N/A') as string,
          })
        );
  ...
  const handleImportOllama = useCallback(
    (model: OllamaDetectedModel) => {
      ...
      console.log('[ModelSettings] Adding model to aiProviderService:', {
        modelId,
        name: model.name,
      });
      ...
      const added = useAppStore
        .getState()
        .aiModels.find((m) => m.name === model.name && m.provider === 'ollama');
      if (added) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      <div
        className={`absolute inset-0 ${t.surface.modalBackdrop} backdrop-blur-md`}
        onClick={closeModelSettings}
      />
      <div
        className={`relative w-[920px] max-h-[88vh] ${t.surface.modal} rounded-2xl flex flex-col overflow-hidden backdrop-blur-xl`}
        style={{
          boxShadow: t.shadow.modal,
          animation: 'yyc3ModalIn 0.25s cubic-bezier(0.16,1,0.3,1)',
        }}
      >
        {/* Header */}
        <div className={`flex items-center gap-3 px-5 py-4 border-b ${t.border.subtle}`}>
          <div
            className={`w-9 h-9 rounded-xl ${t.accent.primaryBg} border border-indigo-500/20 flex items-center justify-center`}
          >
            <Sparkles className={`w-4 h-4 ${t.accent.primary}`} />
          </div>
          <div className="flex-1">
            <div className={`text-[14px] ${t.text.primary}`} style={{ fontWeight: 600 }}>
              {i.msTitle}
            </div>
            <div className={`text-[11px] ${t.text.label}`}>{i.msSubtitle}</div>
          </div>
          <div
            className={`flex items-center gap-2 px-3 py-1.5 rounded-xl ${t.isDark ? 'bg-white/[0.04] border border-white/[0.06]' : 'bg-slate-50 border border-slate-200'} w-48`}
          >
            <Search className={`w-3.5 h-3.5 ${t.text.dimmed}`} />
            <input
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              placeholder={i.msSearchPlaceholder}
              className={`bg-transparent text-[11px] ${t.text.secondary} placeholder:${t.text.dimmed} focus:outline-none w-full`}
            />
          </div>
          <button
            onClick={closeModelSettings}
            className={`p-2 rounded-lg ${t.transition} ${t.interactive.iconBtn}`}
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto min-h-0 p-5 custom-scrollbar">
            ...
            <div className="space-y-3">
                ...
                (() => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span className="text-[11px] text-indigo-300" style={{ fontWeight: 500 }}>
                            {activeModel?.name || i.msUnknown}
                          </span>
                          {matchedProvider && (
              ...
              {filteredProviders.map((rawProvider) => {
                ...
                return (
                  <ProviderCard
                    ...
                    onUrlChange={(url) =>
                      aiProviderService.updateProvider(provider.id, { baseURL: url })
                    }
                    ...
                    onRemoveModel={
                      !PROVIDERS.find((p) => p.id === provider.id)
                        ? (modelId) => handleProviderRemoveModel(provider.id, modelId)
                        : undefined
                    }
                  />
                );
              ...
              {addingProvider ? (
                <div className="rounded-xl border border-dashed border-pink-500/20 bg-pink-500/[0.03] p-4 space-y-3">
                  <div className="text-[11px] text-pink-400/70 mb-1" style={{ fontWeight: 500 }}>
                    {i.msAddCustomProvider}
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <input
                      value={newProvider.name}
                      onChange={(e) => setNewProvider((p) => ({ ...p, name: e.target.value }))}
                      placeholder={i.msProviderName}
                  ...
                  <input
                    ...
                    placeholder={i.msBaseUrlPlaceholder}
            ...
            <div className="space-y-4">
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <div
                    className={`ml-auto flex items-center gap-1.5 text-[10px] ${ollamaConnected ? 'text-emerald-400' : t.text.dimmed}`}
                  >
                    <div
                ...
                <div className={`text-[10px] ${t.text.dimmed}`}>
                  {i.msOllamaHint} (<code className="text-amber-400/30">ollama serve</code>)
                </div>
              </div>
              ...
              {ollamaModels.length > 0 && (
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <div className={`text-[10px] ${t.text.muted} uppercase tracking-wider`}>
                      {i.msDetectedModels} ({ollamaModels.length})
                    </div>
                    {!ollamaConnected && (
                      <span className="text-[9px] text-amber-400/40">{i.msMockData}</span>
                    )}
                  </div>
                  {ollamaModels.map((model) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div
                            className={`text-[12px] ${t.text.secondary}`}
                            style={{ fontWeight: 500 }}
                          >
                            {model.name}
                          </div>
                        ...
                        <span
                          className={`text-[9px] px-1.5 py-0.5 rounded ${model.status === 'online' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-white/[0.04] text-white/20'}`}
                        >
                          {model.status === 'online' ? i.msOnline : i.msOffline}
                        </span>
                        {alreadyImported ? (
                          <span className={`text-[10px] ${t.text.dimmed} flex items-center gap-1`}>
                            <Check className="w-3 h-3" /> {i.msImported}
                          </span>
                        ) : (
                          <button
              ...
              {ollamaModels.length === 0 && !ollamaScanning && (
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  <div
                    className={`w-16 h-16 rounded-2xl ${t.isDark ? 'bg-white/[0.02] border border-white/[0.06]' : 'bg-slate-50 border border-slate-200'} flex items-center justify-center mb-4`}
                  >
          ...
          {activeTab === 'performance' && <AIPerformanceCostPanel t={t} />}
        </div>

        {/* Footer */}
        <div
          className={`flex items-center justify-between px-5 py-3 border-t ${t.border.subtle} ${t.surface.inset}`}
        >
          <div className="flex items-center gap-3">
            <span className={`text-[10px] ${t.text.dimmed}`}>
              {allProviders.length} {i.msProviders} ·{' '}
              {allProviders.reduce((sum, p) => sum + p.models.length, 0)} {i.msModels}
            </span>
        ...
        {selectionToast && (
          ...
          >
            ...
            <span className="text-[12px] text-indigo-300">
              {i.msSwitchedTo} <strong>{selectionToast}</strong>
            </span>
          </div>
        )}
      </div>
