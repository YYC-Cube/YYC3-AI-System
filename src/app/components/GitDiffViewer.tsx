...
const MOCK_DIFFS: Record<string, DiffLine[]> = {
  'IDELayout.tsx': [
    { type: 'header', content: '@@ -48,6 +48,10 @@' },
    {
      ...
      lineOld: 48,
      lineNew: 48,
      content: "import { NotificationCenter } from './NotificationCenter'",
    },
    ...
    { type: 'addition', lineNew: 50, content: "import { GitPanel } from './GitPanel'" },
    {
      type: 'addition',
      lineNew: 51,
      content: "import { ActivityTimeline } from './ActivityTimeline'",
    },
    {
      type: 'addition',
      lineNew: 52,
    ...
    },
    {
      ...
      lineOld: 49,
      lineNew: 53,
      content: "import { getI18n } from '../utils/i18n'",
    },
  ...
  'store.ts': [
    ...
    { type: 'context', lineOld: 152, lineNew: 152, content: '  notificationCenterOpen: false,' },
    {
      ...
      lineOld: 153,
      lineNew: 153,
      content: '  setNotificationCenterOpen: (open) => set({ notificationCenterOpen: open }),',
    },
    ...
    { type: 'addition', lineNew: 156, content: '  aiCodeIntelOpen: false,' },
    {
      type: 'addition',
      lineNew: 157,
      content: '  setAiCodeIntelOpen: (open) => set({ aiCodeIntelOpen: open }),',
    },
    { type: 'addition', lineNew: 158, content: '  activityTimelineOpen: false,' },
    {
      type: 'addition',
      lineNew: 159,
      content: '  setActivityTimelineOpen: (open) => set({ activityTimelineOpen: open }),',
    },
    { type: 'addition', lineNew: 160, content: '  performanceMonitorOpen: false,' },
    {
      type: 'addition',
      lineNew: 161,
      content: '  setPerformanceMonitorOpen: (open) => set({ performanceMonitorOpen: open }),',
    },
  ...
  'i18n-data.ts': [
    { type: 'header', content: '@@ -487,6 +487,95 @@' },
    {
      ...
      lineOld: 487,
      lineNew: 487,
      content: "  ncViewDetails: '\\u67E5\\u770B\\u8BE6\\u60C5',",
    },
    { type: 'addition', lineNew: 488, content: "  aciTitle: 'AI \\u4EE3\\u7801\\u667A\\u80FD'," },
    {
      type: 'addition',
      lineNew: 489,
      content: "  aciSubtitle: '\\u667A\\u80FD\\u5206\\u6790\\u5F53\\u524D\\u6587\\u4EF6',",
    },
  ...
  'AiCodeIntel.tsx': [
    ...
    { type: 'addition', lineNew: 2, content: ' * @file AiCodeIntel.tsx' },
    {
      type: 'addition',
      lineNew: 3,
      content: ' * @description YYC3 PortAISys - AI Code Intelligence Panel',
    },
  ...
  'GitPanel.tsx': [
    ...
    { type: 'addition', lineNew: 2, content: ' * @file GitPanel.tsx' },
    {
      type: 'addition',
      lineNew: 3,
      content: ' * @description YYC3 PortAISys - Git Integration Panel',
    },
    { type: 'addition', lineNew: 4, content: ' */' },
      ...
      {gitDiffFile && (
        <>
          ...
          >
            ...
            >
              <div className="flex items-center gap-3">
                <div className={`p-1.5 rounded-lg ${t.isDark ? 'bg-amber-500/15' : 'bg-amber-50'}`}>
                  <GitCompare
                    className={`w-4 h-4 ${t.isDark ? 'text-amber-400' : 'text-amber-600'}`}
                  />
                </div>
                <div>
                  ...
                  <div className="flex items-center gap-3 mt-0.5">
                    <span
                      className={`flex items-center gap-1 text-[10px] ${t.isDark ? 'text-emerald-400' : 'text-emerald-600'}`}
                    >
                      <Plus className="w-3 h-3" /> {stats.additions} {ig.gdAdditions}
                    </span>
                    <span
                      className={`flex items-center gap-1 text-[10px] ${t.isDark ? 'text-red-400' : 'text-red-500'}`}
                    >
              ...
              <div className="flex items-center gap-2">
                {/* View mode toggle */}
                <div
                  className={`flex items-center rounded-lg border ${t.border.subtle} overflow-hidden`}
                >
                  <button
                ...
                >
                  {copied ? (
                    <Check className={`w-3.5 h-3.5 ${t.status.success}`} />
                  ) : (
                  ...
                  <tbody>
                    {diffLines.map((line, idx) => {
                      if (line.type === 'header') {
                        return (
                          <tr key={idx}>
                            <td
                              colSpan={3}
                              className={`px-4 py-1 ${t.isDark ? 'bg-blue-500/8 text-blue-400' : 'bg-blue-50 text-blue-600'} border-b ${t.border.subtle}`}
                            >
                              {line.content}
                            </td>
                          </tr>
                      ...
                      return (
                        <tr
                          key={idx}
                          className={`${bgClass} border-b ${t.isDark ? 'border-white/[0.02]' : 'border-slate-100'} hover:brightness-110`}
                        >
                          {/* Old line number */}
                          <td
                            className={`w-[50px] text-right px-2 select-none ${t.text.dimmed} ${t.isDark ? 'border-r border-white/[0.03]' : 'border-r border-slate-100'}`}
                          >
                            {line.lineOld || ''}
                          </td>
                          {/* New line number */}
                          ...
                          <td className={`px-3 ${textClass} whitespace-pre`}>
                            <span
                              className={`inline-block w-3 ${line.type === 'addition' ? 'text-emerald-400' : line.type === 'deletion' ? 'text-red-400' : 'text-transparent'}`}
            ...
            {/* Footer */}
            <div
              className={`flex items-center justify-between px-5 py-2.5 border-t ${t.border.subtle}`}
            >
