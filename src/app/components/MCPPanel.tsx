...
import {
  Plug, Plus, Trash2, Edit3, Check, X, RefreshCw,
    ...
    <div className={`h-full flex flex-col ${t.surface.app}`}>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-2">
        {mcpServers.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center">
            <div className={`w-16 h-16 rounded-2xl ${t.isDark ? 'bg-white/[0.02]' : 'bg-slate-50'} flex items-center justify-center mb-3`}>
            ...
            return (
              <div
                ...
                className={`rounded-xl border overflow-hidden transition-all ${
                  editingId === server.id
                    ? t.isDark ? 'border-indigo-500/30 bg-indigo-500/[0.05]' : 'border-indigo-500/30 bg-indigo-50'
                    : t.isDark ? 'border-white/[0.06] bg-white/[0.02]' : 'border-slate-200 bg-slate-50'
                }`}
              >
                {/* Server Header */}
                <div className="flex items-center gap-2 px-3 py-2.5">
                  {editingId === server.id ? (
                    // Edit Mode
                    <>
                      <div className={`w-8 h-8 rounded-lg ${t.isDark ? 'bg-white/[0.06]' : 'bg-slate-200'} flex items-center justify-center`}>
                    ...
                    <>
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                        ...
                        status === 'connecting' ? 'bg-amber-500/10' :
                        'bg-slate-500/10'
                      }`}>
                        {status === 'connected' ? (
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span className={`text-[12px] ${t.text.primary} truncate`} style={{ fontWeight: 500 }}>
                            {server.name}
                          </span>
                          {server.isProject && (
                            <span className={`text-[8px] px-1.5 py-0.5 rounded ${t.isDark ? 'bg-indigo-500/20 text-indigo-400' : 'bg-indigo-50 text-indigo-500'}`}>
                              Project
                            </span>
                          )}
                        </div>
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        >
                          {server.enabled ? <Wifi className="w-3.5 h-3.5" /> : <WifiOff className="w-3.5 h-3.5" />}
                        </button>
                        <button
                ...
                {expandedServer === server.id && mcpServer && (
                  <div className={`px-3 pb-3 pt-0 border-t ${t.isDark ? 'border-white/[0.04]' : 'border-slate-200'}`}>
                    <div className="grid grid-cols-3 gap-3 mt-3">
                      <div className={`p-2 rounded-lg ${t.isDark ? 'bg-white/[0.03]' : 'bg-slate-100'}`}>
                        <div className={`text-[9px] ${t.text.dimmed} mb-1`}>Status</div>
                        <div className={`text-[11px] ${
                          mcpServer.status === 'connected' ? 'text-emerald-400' :
                          mcpServer.status === 'error' ? 'text-red-400' :
                          'text-slate-400'
                      ...
                      <div className={`p-2 rounded-lg ${t.isDark ? 'bg-white/[0.03]' : 'bg-slate-100'}`}>
                        ...
                        <div className={`text-[11px] ${t.text.primary}`} style={{ fontWeight: 600 }}>
                          {tools.length}
                        </div>
                      </div>
                      <div className={`p-2 rounded-lg ${t.isDark ? 'bg-white/[0.03]' : 'bg-slate-100'}`}>
      ...
      {!editingId && (
        <div className={`p-4 border-t ${t.border.subtle}`}>
          <button
            onClick={() => handleEdit({ id: '', name: '', endpoint: '', type: 'stdio', enabled: true, isProject: false })}
            className={`w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl ${t.accent.solidBtn} transition-all`}
          >
            <Plus className="w-4 h-4" />
            <span className="text-[12px]" style={{ fontWeight: 500 }}>Add MCP Server</span>
          </button>
        </div>
      )}
    </div>
...
async function testMCPConnection(endpoint: string, type: string): Promise<{ ok: boolean; latency: number; error?: string }> {
