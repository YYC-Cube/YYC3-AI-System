...
const MOCK_CONFLICTS: ConflictHunk[] = [
  ...
  },
  {
    ...
    startLine: 34,
    endLine: 36,
    localLines: ['      designRoot: initialDesignRoot,'],
    incomingLines: ['      designRoot: createDefaultDesignRoot(),'],
    peerName: 'Dave',
    peerColor: '#ec4899',
    resolved: false,
    resolution: null,
...
export function ConflictResolver({ open, onClose }: ConflictResolverProps) {
  ...
  const resolveConflict = useCallback(
    (
      id: string,
      resolution: 'incoming' | 'current' | 'both' | 'manual',
      mergedLines?: string[]
    ) => {
      setConflicts((prev) =>
        prev.map((c) => (c.id === id ? { ...c, resolved: true, resolution, mergedLines } : c))
      );
  ...
  const resolutionLabel = (r: string | null) => {
    switch (r) {
      case 'incoming':
        return i.cfAcceptIncoming;
      case 'current':
        return i.cfAcceptCurrent;
      case 'both':
        return i.cfAcceptBoth;
      case 'manual':
        return i.cfManualMerge;
      default:
        return i.cfUnresolved;
    }
  };
        ...
        >
          ...
          >
            <div className="flex items-center gap-3">
              <div
                className={`w-8 h-8 rounded-xl flex items-center justify-center ${t.isDark ? 'bg-gradient-to-br from-red-500/20 to-orange-500/20' : 'bg-gradient-to-br from-red-50 to-orange-50'}`}
              >
              ...
              <div className="flex items-center gap-2 ml-4">
                {counts.unresolved > 0 && (
                  <span
                    className={`flex items-center gap-1 text-[9px] px-2 py-0.5 rounded-full ${t.isDark ? 'bg-red-500/15 text-red-400' : 'bg-red-50 text-red-600'}`}
                  >
                ...
                {counts.resolved > 0 && (
                  <span
                    className={`flex items-center gap-1 text-[9px] px-2 py-0.5 rounded-full ${t.isDark ? 'bg-emerald-500/15 text-emerald-400' : 'bg-emerald-50 text-emerald-600'}`}
                  >
          ...
          <div className="flex flex-1 overflow-hidden">
            ...
            <div
              className={`w-56 flex-shrink-0 border-r ${t.border.subtle} overflow-y-auto ${t.scrollbar}`}
            >
              {conflicts.map((cf, idx) => (
                ...
                >
                  <div
                    className={`w-2 h-2 rounded-full flex-shrink-0 ${cf.resolved ? 'bg-emerald-400' : 'bg-red-400'}`}
                  />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1">
                      <FileCode className={`w-3 h-3 flex-shrink-0 ${t.text.muted}`} />
                      <span
                        className={`text-[9px] truncate ${t.text.primary}`}
                    ...
                    <div className={`text-[8px] ${t.text.dimmed}`}>
                      L{cf.startLine}-{cf.endLine} ·{' '}
                      <span style={{ color: cf.peerColor }}>{cf.peerName}</span>
                    </div>
                  </div>
            ...
            {activeConflict && (
              <div className="flex-1 flex flex-col overflow-hidden">
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-[9px] ${t.text.dimmed}`}>
                      L{activeConflict.startLine}-{activeConflict.endLine}
                    </span>
                    <div className="flex items-center gap-1 ml-2">
                      <div
                        className="w-3 h-3 rounded-full flex items-center justify-center text-[6px] text-white"
                        style={{ backgroundColor: activeConflict.peerColor, fontWeight: 700 }}
                      >
                        {activeConflict.peerName[0]}
                      </div>
                      <span
                        className={`text-[9px]`}
                        style={{ color: activeConflict.peerColor, fontWeight: 500 }}
                      >
                        {activeConflict.peerName}
                      </span>
                    </div>
                  </div>
                  {activeConflict.resolved && (
                    <span
                      className={`flex items-center gap-1 text-[9px] px-2 py-0.5 rounded ${t.isDark ? 'bg-emerald-500/15 text-emerald-400' : 'bg-emerald-50 text-emerald-600'}`}
                    >
                      <CheckCircle2 className="w-2.5 h-2.5" />{' '}
                      {resolutionLabel(activeConflict.resolution)}
                    </span>
                  )}
                </div>
                ...
                <div className="flex-1 overflow-auto">
                  <div className="grid grid-cols-2 h-full">
                    ...
                    <div className={`border-r ${t.border.subtle}`}>
                      <div
                        className={`px-3 py-1.5 text-[9px] uppercase tracking-wider border-b ${t.border.subtle} ${t.isDark ? 'bg-blue-500/5 text-blue-400' : 'bg-blue-50 text-blue-600'}`}
                      ...
                      <div className="p-0.5">
                        {activeConflict.localLines.map((line, idx) => (
                          <div
                            key={idx}
                            className={`flex font-mono text-[10px] ${t.isDark ? 'hover:bg-blue-500/5' : 'hover:bg-blue-50/50'}`}
                          >
                            <span
                              className={`w-8 text-right pr-2 flex-shrink-0 select-none ${t.text.dimmed}`}
                            >
                              {activeConflict.startLine + idx}
                            </span>
                            <span
                              className={`flex-1 px-1 ${activeConflict.resolved && activeConflict.resolution === 'incoming' ? 'opacity-30 line-through' : ''} ${t.text.primary}`}
                            >
                              {line}
                            </span>
                          </div>
                    ...
                    <div>
                      <div
                        className={`px-3 py-1.5 text-[9px] uppercase tracking-wider border-b ${t.border.subtle} ${t.isDark ? 'bg-green-500/5 text-green-400' : 'bg-green-50 text-green-600'}`}
                        style={{ fontWeight: 700 }}
                      >
                      ...
                      <div className="p-0.5">
                        {activeConflict.incomingLines.map((line, idx) => (
                          <div
                            key={idx}
                            className={`flex font-mono text-[10px] ${t.isDark ? 'hover:bg-green-500/5' : 'hover:bg-green-50/50'}`}
                          >
                            <span
                              className={`w-8 text-right pr-2 flex-shrink-0 select-none ${t.text.dimmed}`}
                            >
                            ...
                            <span
                              className={`flex-1 px-1 ${activeConflict.resolved && activeConflict.resolution === 'current' ? 'opacity-30 line-through' : ''} ${t.text.primary}`}
                            >
                              {line}
                            </span>
                          </div>
                  ...
                  {manualEditId === activeConflict.id && (
                    <div className={`border-t ${t.border.subtle} p-3`}>
                      <div
                        className={`text-[9px] uppercase tracking-wider mb-1 ${t.text.dimmed}`}
                      ...
                      <div className="flex justify-end gap-1 mt-1">
                        ...
                        <button
                          onClick={() =>
                            resolveConflict(activeConflict.id, 'manual', manualText.split('\n'))
                          }
                ...
                {/* Action buttons */}
                <div
                  className={`flex items-center justify-between px-4 py-3 border-t ${t.border.subtle}`}
                >
                  <div className="flex items-center gap-2">
                    {!activeConflict.resolved ? (
                      <>
                        ...
                        <button
                          onClick={() => {
                            setManualEditId(activeConflict.id);
                            setManualText(
                              [
                                ...activeConflict.localLines,
                                '',
                                ...activeConflict.incomingLines,
                              ].join('\n')
                            );
                          }}
                      ...
                      <button
                        onClick={() => unresolveConflict(activeConflict.id)}
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => navigateConflict(-1)}
                      className={`px-2 py-1 rounded-lg text-[9px] ${t.transition} ${t.interactive.iconBtn}`}
                    >
                      ← {i.cfPrevConflict}
                    </button>
                    <button
