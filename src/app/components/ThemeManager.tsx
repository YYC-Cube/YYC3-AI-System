  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-[61] flex items-center justify-center p-8">
        <div
          className={`w-full max-w-2xl max-h-[85vh] rounded-2xl overflow-hidden flex flex-col ${t.surface.popover} ${t.border.popover} shadow-2xl`}
        >
          {/* Header */}
          <div
          ...
          <div className={`flex-1 overflow-y-auto p-5 space-y-5 ${t.scrollbar}`}>
            ...
            <div className="flex items-center space-x-2">
              <button
                ...
                className={`flex items-center space-x-1.5 px-3 py-2 rounded-lg text-[11px] ${t.transition} ${t.accent.activeBg} ${t.accent.activeText}`}
              >
                <Download className="w-3.5 h-3.5" />
                <span>{i.tmExportJson}</span>
              </button>
              <button
                onClick={() => fileInputRef.current?.click()}
                className={`flex items-center space-x-1.5 px-3 py-2 rounded-lg text-[11px] ${t.transition} ${t.interactive.iconBtn}`}
              >
                <Upload className="w-3.5 h-3.5" />
                <span>{i.tmImportJson}</span>
              </button>
              <input
                ref={fileInputRef}
                type="file"
            ...
            <div className={`p-4 rounded-xl ${t.isDark ? 'bg-slate-800/40' : 'bg-slate-50'}`}>
              ...
              <pre
                className={`p-3 rounded-lg text-[10px] font-mono max-h-32 overflow-y-auto ${t.isDark ? 'bg-slate-900/60 text-slate-400' : 'bg-slate-100 text-slate-600'}`}
              >
                    ...
                    >
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <div className={`text-[9px] ${t.text.dimmed}`}>
                            {st.config.theme} · {formatDate(st.createdAt)}
                          </div>
                        </div>
                      </div>
                      <div className="flex items-center space-x-1.5">
                        ...
                        <button
                          ...
                          className={`p-1.5 rounded-lg ${t.transition} hover:bg-red-500/15 text-red-400`}
                          title={i.tmDeleteTheme}
                        >
