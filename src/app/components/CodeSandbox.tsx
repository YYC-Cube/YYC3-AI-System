...
import {
  X, Shield, Play, RefreshCw, Zap, AlertTriangle,
  CheckCircle2, Clock, Cpu, MemoryStick, Terminal, Eye,
  Trash2, Lock, Box,
  Layers, Loader2, ArrowRight, Link2, Unlink, Code, Settings
} from 'lucide-react'
...
export function CodeSandbox({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      ...
      setMemoryUsage(prev => Math.max(8, Math.min(config.memoryLimit * 0.8, prev + (Math.random() - 0.5) * 3)))
    }, 1000)
  ...
  const executeCode = useCallback(() => {
    ...
    const timeout = window.setTimeout(() => {
      if (isRunning) {
        setIsRunning(false)
        setExecResults(prev => [...prev.slice(-20), {
          id: `exec-${Date.now()}`,
          status: 'timeout',
          startTime,
          endTime: Date.now(),
  ...
  useEffect(() => {
    ...
    return () => { if (hmrTimerRef.current) window.clearTimeout(hmrTimerRef.current) }
  ...
  const tabs: { id: SandboxTab; label: string; icon: React.FC<{ className?: string }>; badge?: number }[] = [
  ...
  return (
    <>
      <div className="fixed inset-0 z-[60] bg-black/50 backdrop-blur-sm" onClick={onClose} data-testid="sandbox-backdrop" />
      <div className="fixed inset-0 z-[61] flex items-center justify-center p-4">
        <div data-testid="sandbox-panel" className={`w-full max-w-5xl max-h-[85vh] rounded-2xl overflow-hidden flex flex-col ${t.surface.popover} ${t.border.popover} shadow-2xl`}>
          {/* Header */}
          <div className={`flex items-center justify-between px-6 py-4 border-b ${t.border.subtle} flex-shrink-0`}>
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h2 className="text-[15px]" style={{ fontWeight: 600 }}>{i.sbTitle}</h2>
            ...
            <div className="flex items-center space-x-2">
              {/* HMR indicator */}
              {config.hmrEnabled && (
                <div className={`flex items-center space-x-1 px-2 py-1 rounded-lg text-[10px] ${t.isDark ? 'bg-emerald-900/20' : 'bg-emerald-50'}`}>
                  <Zap className="w-3 h-3 text-emerald-400" />
                  <span className="text-emerald-400" style={{ fontWeight: 500 }}>HMR #{hmrCount}</span>
                </div>
              )}
              {/* Preview link toggle */}
              <button
                ...
                className={`flex items-center space-x-1 px-2 py-1 rounded-lg text-[10px] ${t.transition} ${
                  linkedToPreview
                    ? (t.isDark ? 'bg-indigo-900/20 text-indigo-400' : 'bg-indigo-50 text-indigo-700')
                    : t.interactive.menuItem
                }`}
                title={i.sbSyncToPreview}
              >
                {linkedToPreview ? <Link2 className="w-3 h-3" /> : <Unlink className="w-3 h-3" />}
                <span style={{ fontWeight: 500 }}>{linkedToPreview ? i.sbLinkedToPreview : i.sbSyncToPreview}</span>
              </button>
              {/* Status */}
              <div className={`flex items-center space-x-1 px-2 py-1 rounded-lg text-[10px] ${t.isDark ? 'bg-slate-800/50' : 'bg-slate-100'}`}>
                {isRunning ? <Loader2 className="w-3 h-3 text-amber-400 animate-spin" /> : <CheckCircle2 className="w-3 h-3 text-emerald-400" />}
                <span className={isRunning ? 'text-amber-400' : 'text-emerald-400'} style={{ fontWeight: 500 }}>
              ...
              <button onClick={onClose} className={`p-1.5 rounded-lg ${t.transition} ${t.interactive.iconBtn}`} data-testid="sandbox-close-btn">
                <X className="w-4 h-4" />
              </button>
            </div>
          </div>
              ...
              <div className="grid grid-cols-4 gap-3">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setConfig(prev => ({ ...prev, cpuTimeout: parseInt(e.target.value) }))}
                    className={`w-full px-2 py-1 rounded-lg text-[11px] ${t.isDark ? 'bg-slate-800 text-slate-300' : 'bg-white text-slate-700'} border ${t.border.subtle}`}
                  >
                ...
                <div className="flex items-end space-x-2">
                  <label className="flex items-center space-x-1.5 cursor-pointer">
                    <input
                      ...
                      onChange={(e) => setConfig(prev => ({ ...prev, hmrEnabled: e.target.checked }))}
                ...
                <div className="flex items-end space-x-2">
                  <label className="flex items-center space-x-1.5 cursor-pointer">
                    <input
                      ...
                      onChange={(e) => setConfig(prev => ({ ...prev, networkAccess: e.target.checked }))}
                      className="rounded accent-indigo-500"
                    />
          ...
          {/* Tabs */}
          <div className={`flex items-center justify-between px-6 py-2 border-b ${t.border.subtle} flex-shrink-0`}>
                ...
                return (
                  ...
                  >
                    ...
                    {tab.badge !== undefined && tab.badge > 0 && (
                      <span className="px-1.5 py-0.5 rounded-full text-[9px] bg-indigo-500/20 text-indigo-400" style={{ fontWeight: 600 }}>{tab.badge}</span>
                    )}
                  </button>
                )
              })}
            ...
            <div className="flex-1 overflow-hidden flex flex-col">
              {activeTab === 'editor' && (
                <div className="flex-1 overflow-hidden flex flex-col">
                  <textarea
                    ...
                    style={{ fontFamily: "'Fira Code', 'JetBrains Mono', monospace", lineHeight: '1.6', tabSize: 2 }}
                    spellCheck={false}
                  />
                </div>
              )}
              ...
              {activeTab === 'console' && (
                <div className="flex-1 overflow-y-auto p-4 space-y-0.5">
                  <div className="flex items-center justify-between mb-2">
                    <span className={`text-[11px] ${t.text.muted}`}>{i.sbConsoleOutput} ({consoleEntries.length})</span>
                    <button onClick={() => setConsoleEntries([])} className={`text-[10px] px-2 py-0.5 rounded ${t.interactive.menuItem}`}>
                  ...
                  {consoleEntries.map(entry => (
                    <div
                      ...
                      className={`flex items-start space-x-2 px-2 py-1 rounded text-[11px] font-mono ${
                        entry.type === 'error' ? (t.isDark ? 'bg-red-900/10 text-red-300' : 'bg-red-50 text-red-700') :
                        entry.type === 'warn' ? (t.isDark ? 'bg-amber-900/10 text-amber-300' : 'bg-amber-50 text-amber-700') :
                        entry.type === 'info' ? (t.isDark ? 'bg-blue-900/10 text-blue-300' : 'bg-blue-50 text-blue-700') :
                        (t.isDark ? 'text-slate-400' : 'text-slate-600')
                      }`}
                    >
                      <span className={`flex-shrink-0 text-[9px] uppercase ${t.text.dimmed}`} style={{ fontWeight: 600 }}>{entry.type}</span>
              ...
              {activeTab === 'resources' && (
                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className={`p-4 rounded-xl ${t.isDark ? 'bg-slate-800/30' : 'bg-slate-50'}`}>
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center space-x-2">
                          <Cpu className="w-4 h-4 text-indigo-400" />
                          <span className="text-[12px]" style={{ fontWeight: 500 }}>CPU</span>
                        </div>
                        <span className={`text-[12px] ${cpuUsage > 80 ? 'text-red-400' : cpuUsage > 50 ? 'text-amber-400' : 'text-emerald-400'}`} style={{ fontWeight: 600 }}>
                          {cpuUsage.toFixed(1)}%
                        </span>
                      </div>
                      <div className={`h-2 rounded-full overflow-hidden ${t.isDark ? 'bg-slate-700' : 'bg-slate-200'}`}>
                        <div
                    ...
                    <div className={`p-4 rounded-xl ${t.isDark ? 'bg-slate-800/30' : 'bg-slate-50'}`}>
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="text-[12px]" style={{ fontWeight: 500 }}>{i.sbMemory}</span>
                        </div>
                        <span className={`text-[12px] ${memoryUsage > config.memoryLimit * 0.8 ? 'text-red-400' : 'text-cyan-400'}`} style={{ fontWeight: 600 }}>
                  ...
                  {/* Execution history */}
                  <div>
                    <h3 className={`text-[12px] mb-2 ${t.text.muted}`} style={{ fontWeight: 600 }}>{i.sbExecHistory}</h3>
                    <div className="space-y-1">
                      {execResults.slice(-10).reverse().map(result => (
                        <div key={result.id} className={`flex items-center justify-between p-2 rounded-lg text-[11px] ${t.isDark ? 'bg-slate-800/20' : 'bg-slate-50'}`}>
                          <div className="flex items-center space-x-2">
                            {result.status === 'success' ? <CheckCircle2 className="w-3 h-3 text-emerald-400" /> :
                             result.status === 'timeout' ? <Clock className="w-3 h-3 text-amber-400" /> :
                             <AlertTriangle className="w-3 h-3 text-red-400" />}
                            <span>{result.status}</span>
                          </div>
                          <div className="flex items-center space-x-3">
                            <span className={t.text.muted}>{result.endTime - result.startTime}ms</span>
                            <span className={t.text.muted}>{result.memoryUsed.toFixed(1)}MB</span>
                            <span className={`text-[9px] ${t.text.dimmed}`}>{new Date(result.startTime).toLocaleTimeString()}</span>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <div>
                    <h3 className={`text-[12px] mb-2 ${t.text.muted}`} style={{ fontWeight: 600 }}>{i.sbSecurity}</h3>
                    <div className="grid grid-cols-2 gap-2">
                      {[
                        ...
                        <div key={item.label} className={`flex items-center space-x-2 p-2 rounded-lg text-[11px] ${t.isDark ? 'bg-slate-800/20' : 'bg-slate-50'}`}>
                          {item.enabled ? <CheckCircle2 className="w-3 h-3 text-emerald-400" /> : <Lock className="w-3 h-3 text-red-400" />}
              ...
              {activeTab === 'deps' && (
                <div className="flex-1 overflow-y-auto p-4 space-y-2">
                  ...
                  {deps.map(dep => (
                    <div key={dep.name} className={`flex items-center justify-between p-3 rounded-xl ${t.isDark ? 'bg-slate-800/30' : 'bg-slate-50'}`}>
                      <div className="flex items-center space-x-3">
                        <Box className={`w-4 h-4 ${dep.loaded ? 'text-emerald-400' : t.text.muted}`} />
                        <div>
                          <p className="text-[12px]" style={{ fontWeight: 500 }}>{dep.name}</p>
                          <p className={`text-[10px] ${t.text.muted}`}>v{dep.version} &bull; {dep.size}</p>
                        </div>
                      </div>
                      <span className={`px-2 py-0.5 rounded-full text-[9px] ${
                        dep.loaded ? 'bg-emerald-500/20 text-emerald-400' : 'bg-slate-500/20 text-slate-400'
            ...
            <div className={`w-[45%] flex-shrink-0 border-l ${t.border.subtle} flex flex-col`}>
              <div className={`flex items-center justify-between px-3 py-2 border-b ${t.border.subtle}`}>
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-[11px]" style={{ fontWeight: 500 }}>{i.sbPreview}</span>
                  {linkedToPreview && (
                    <span className="flex items-center space-x-1 px-1.5 py-0.5 rounded text-[8px] bg-indigo-500/20 text-indigo-400" style={{ fontWeight: 600 }}>
                ...
                <div className="flex items-center space-x-1">
                  ...
                  <button onClick={executeCode} className={`p-1 rounded-lg ${t.interactive.iconBtn}`}>
          ...
          {/* Footer */}
          <div className={`flex items-center justify-between px-6 py-2 border-t ${t.border.subtle} text-[10px] ${t.text.dimmed} flex-shrink-0`}>
            <div className="flex items-center space-x-3">
            ...
            <div className="flex items-center space-x-2">
              <span>{i.sbExecCount}: {execResults.length}</span>
              <span>&bull;</span>
