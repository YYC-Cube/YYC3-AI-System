...
import {
  BookOpen, X, Sparkles, Copy, Download, Loader2,
  FileText, Code, Package, Globe, ChevronRight,
  Check, AlertTriangle, BarChart3, Eye
} from 'lucide-react'
...
const MOCK_ITEMS: DocItem[] = [
  { id: 'd1', name: 'IDELayout', type: 'component', file: 'IDELayout.tsx', documented: true, coverage: 85 },
  { id: 'd2', name: 'ChatInterface', type: 'component', file: 'ChatInterface.tsx', documented: true, coverage: 72 },
  { id: 'd3', name: 'CodeEditor', type: 'component', file: 'CodeEditor.tsx', documented: false, coverage: 45 },
  { id: 'd4', name: 'FileManager', type: 'component', file: 'FileManager.tsx', documented: true, coverage: 90 },
  { id: 'd5', name: 'useAppStore', type: 'function', file: 'store.ts', documented: false, coverage: 30 },
  { id: 'd6', name: 'getThemeTokens', type: 'function', file: 'theme.ts', documented: true, coverage: 95 },
  { id: 'd7', name: 'getI18n', type: 'function', file: 'i18n.ts', documented: true, coverage: 88 },
  { id: 'd8', name: 'collabManager', type: 'module', file: 'collaboration.ts', documented: false, coverage: 20 },
  { id: 'd9', name: 'wsCollab', type: 'module', file: 'ws-collab.ts', documented: false, coverage: 15 },
  { id: 'd10', name: 'aiCompletion', type: 'module', file: 'ai-completion.ts', documented: true, coverage: 78 },
  { id: 'd11', name: '/api/v2/users', type: 'api', file: 'routes.ts', documented: false, coverage: 0 },
  { id: 'd12', name: '/api/v2/projects', type: 'api', file: 'routes.ts', documented: false, coverage: 0 },
  { id: 'd13', name: 'FlameGraph', type: 'component', file: 'FlameGraph.tsx', documented: true, coverage: 82 },
  { id: 'd14', name: 'ErDiagram', type: 'component', file: 'ErDiagram.tsx', documented: true, coverage: 80 },
  { id: 'd15', name: 'ApiTester', type: 'component', file: 'ApiTester.tsx', documented: true, coverage: 76 },
]
...
const TYPE_ICONS: Record<DocType, typeof FileText> = {
  component: Code, function: FileText, module: Package, api: Globe, readme: BookOpen, changelog: BarChart3,
}
const TYPE_COLORS: Record<DocType, string> = {
  component: '#6366f1', function: '#10b981', module: '#f59e0b', api: '#ef4444', readme: '#3b82f6', changelog: '#8b5cf6',
}

/* ═════════════════════════════════════════ */

interface DocGeneratorProps { open: boolean; onClose: () => void }
...
export function DocGenerator({ open, onClose }: DocGeneratorProps) {
  ...
  const generateDoc = useCallback((itemId: string) => {
  ...
  const generateAll = useCallback(() => {
    ...
    setGeneratedDoc(null)
    setTimeout(() => {
      const allDocs = items.map(it => {
        const doc = MOCK_DOCS[it.name] || `# ${it.name}\n\nAuto-generated doc for \`${it.name}\`.`
        return `---\n\n${doc}`
      }).join('\n\n')
        ...
        >
          ...
          <div className={`flex items-center justify-between px-6 py-3 border-b ${t.border.subtle}`}>
            <div className="flex items-center gap-3">
              <div className={`w-8 h-8 rounded-xl flex items-center justify-center ${t.isDark ? 'bg-gradient-to-br from-blue-500/20 to-violet-500/20' : 'bg-gradient-to-br from-blue-50 to-violet-50'}`}>
                <BookOpen className={`w-4 h-4 ${t.isDark ? 'text-blue-400' : 'text-blue-500'}`} />
              </div>
              <div>
                <h2 className={`text-[14px] ${t.text.primary}`} style={{ fontWeight: 700 }}>{ii.dgTitle}</h2>
                <p className={`text-[10px] ${t.text.dimmed}`}>{ii.dgSubtitle} · {items.length} items</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              {/* Coverage badge */}
              <div className={`flex items-center gap-1.5 px-3 py-1 rounded-lg ${t.isDark ? 'bg-white/[0.03]' : 'bg-slate-50'}`}>
                <BarChart3 className="w-3 h-3" style={{ color: coverageColor(overallCoverage) }} />
                <span className="text-[9px]" style={{ color: coverageColor(overallCoverage), fontWeight: 700 }}>{ii.dgCoverage}: {overallCoverage}%</span>
                <span className={`text-[8px] ${t.text.dimmed}`}>· {undocumentedCount} {ii.dgUndocumented}</span>
              </div>
          ...
          <div className={`flex items-center gap-2 px-6 py-2 border-b ${t.border.subtle}`}>
            {DOC_TYPES.map(dt => (
              <button key={dt.type} onClick={() => setFilterType(dt.type)}
            ...
            {FORMATS.map(f => (
              <button key={f.fmt} onClick={() => setFormat(f.fmt)}
          ...
          <div className="flex flex-1 overflow-hidden">
            {/* Item list */}
            <div className={`w-72 flex-shrink-0 border-r ${t.border.subtle} overflow-y-auto ${t.scrollbar}`}>
              {filtered.map(item => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-2 px-4 py-2.5 text-left border-b ${t.border.subtle} ${t.transition} ${
                      isSel ? (t.isDark ? 'bg-indigo-500/10' : 'bg-indigo-50/80') : t.interactive.menuItem
                    }`}
                  >
                    <TIcon className="w-3.5 h-3.5 flex-shrink-0" style={{ color: TYPE_COLORS[item.type] }} />
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        <span className={`text-[10px] truncate ${t.text.primary}`} style={{ fontWeight: 500 }}>{item.name}</span>
                        {!item.documented && <AlertTriangle className="w-2.5 h-2.5 text-amber-400 flex-shrink-0" />}
                      </div>
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className={`text-[8px] ${t.text.dimmed}`}>{item.file}</span>
                        <div className={`flex-1 h-1 rounded-full overflow-hidden ${t.isDark ? 'bg-white/[0.06]' : 'bg-slate-200'}`}>
                          <div className="h-full rounded-full" style={{ width: `${item.coverage}%`, backgroundColor: coverageColor(item.coverage) }} />
                        </div>
                        <span className="text-[7px]" style={{ color: coverageColor(item.coverage), fontWeight: 600 }}>{item.coverage}%</span>
                      </div>
                    </div>
            ...
            <div className="flex-1 flex flex-col">
              {generatedDoc && (
                <div className={`flex items-center justify-between px-4 py-1.5 border-b ${t.border.subtle}`}>
                  <div className="flex items-center gap-1">
                    <span className={`text-[9px] ${t.text.muted}`} style={{ fontWeight: 600 }}>{format.toUpperCase()} {ii.apiPreview}</span>
                    <div className={`flex items-center ml-2 rounded-lg overflow-hidden ${t.isDark ? 'bg-white/[0.04]' : 'bg-slate-100'}`}>
                  ...
                  <div className="flex items-center gap-1">
                    <button onClick={copyDoc} className={`p-1 rounded ${t.transition} ${t.interactive.iconBtn}`}>
                      {copiedDoc ? <Check className="w-3 h-3 text-emerald-400" /> : <Copy className="w-3 h-3" />}
                    </button>
                    <button onClick={() => { const blob = new Blob([generatedDoc], { type: 'text/markdown' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'documentation.md'; a.click() }}
              ...
              <div className={`flex-1 overflow-auto ${t.scrollbar}`}>
                {generating ? (
                  ...
                  viewMode === 'source' ? (
                    <pre className={`p-6 font-mono text-[9px] whitespace-pre-wrap ${t.isDark ? 'text-slate-300' : 'text-slate-700'}`}>
                      {generatedDoc}
                    </pre>
                  ) : (
                    <RichTextEditor
