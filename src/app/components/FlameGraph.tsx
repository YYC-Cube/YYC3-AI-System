...
export function FlameGraph({ open, onClose }: FlameGraphProps) {
  ...
  const frames = useMemo(() => flattenTree(rootData, 0, 1, rootData.totalTime), [rootData]);

  const filteredFrames = useMemo(
    () =>
      searchTerm
        ? frames.filter((f) => f.name.toLowerCase().includes(searchTerm.toLowerCase()))
        : frames,
    [frames, searchTerm]
  );
      ...
      <div className="fixed inset-0 z-[71] flex items-center justify-center p-4">
        ...
        >
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`text-[10px] ${t.text.dimmed}`}>
                  {i.fgSubtitle} · {frames.length} frames · {rootData.totalTime}ms
                </p>
              </div>
            </div>
          ...
          <div className={`flex items-center gap-2 px-6 py-2 border-b ${t.border.subtle}`}>
            <div
              className={`flex items-center gap-1 px-2 py-1 rounded-lg border ${t.border.subtle} flex-1 max-w-xs`}
            >
          ...
          <div className="flex flex-1 overflow-hidden">
            ...
            <div className={`flex-1 overflow-auto ${t.scrollbar}`}>
              ...
              >
                {frames.map((frame) => {
                  ...
                  const isHighlighted =
                    searchTerm && frame.name.toLowerCase().includes(searchTerm.toLowerCase());
                  ...
                  return (
                    <g
                      key={frame.id}
                      onClick={() => setFocusedFrame(isFocused ? null : frame)}
                    ...
                    >
                      ...
                      {fw > 30 && (
                        ...
                        >
                          {frame.name.length > fw / 6
                            ? frame.name.slice(0, Math.floor(fw / 6)) + '...'
                            : frame.name}
                        </text>
                      )}
                    </g>
            ...
            {focusedFrame && (
              <div
                className={`w-64 flex-shrink-0 border-l ${t.border.subtle} overflow-y-auto p-4 space-y-3 ${t.scrollbar}`}
              >
                <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p
                    className={`text-[10px] font-mono mt-0.5 ${t.isDark ? 'text-cyan-400' : 'text-cyan-600'}`}
                  >
                    {focusedFrame.name}
                  </p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className={`p-2 rounded-lg ${t.isDark ? 'bg-white/[0.03]' : 'bg-slate-50'}`}>
                    <label className={`text-[7px] uppercase ${t.text.dimmed}`}>
                      {i.fgSelfTime}
                    </label>
                    <p
                      className={`text-[12px] mt-0.5 ${t.text.primary}`}
                      style={{ fontWeight: 700 }}
                    >
                      {focusedFrame.selfTime}ms
                    </p>
                  </div>
                  <div className={`p-2 rounded-lg ${t.isDark ? 'bg-white/[0.03]' : 'bg-slate-50'}`}>
                    <label className={`text-[7px] uppercase ${t.text.dimmed}`}>
                    ...
                    <p
                      ...
                      style={{ fontWeight: 700 }}
                    >
                      {focusedFrame.totalTime}ms
                    </p>
                  </div>
                </div>
                <div className={`p-2 rounded-lg ${t.isDark ? 'bg-white/[0.03]' : 'bg-slate-50'}`}>
                  <label className={`text-[7px] uppercase ${t.text.dimmed}`}>{i.fgSamples}</label>
                  <p className={`text-[11px] mt-0.5 ${t.text.primary}`} style={{ fontWeight: 600 }}>
                    {focusedFrame.samples.toLocaleString()}
                  </p>
                </div>
                <div className={`p-2 rounded-lg ${t.isDark ? 'bg-white/[0.03]' : 'bg-slate-50'}`}>
                ...
                <div className={`p-2 rounded-lg ${t.isDark ? 'bg-white/[0.03]' : 'bg-slate-50'}`}>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <div
                      className={`flex-1 h-1.5 rounded-full overflow-hidden ${t.isDark ? 'bg-white/[0.06]' : 'bg-slate-200'}`}
                    >
                      <div
                        className="h-full rounded-full bg-gradient-to-r from-emerald-500 to-red-500"
                        style={{ width: `${(focusedFrame.totalTime / rootData.totalTime) * 100}%` }}
                      />
                    </div>
                    <span className={`text-[10px] ${t.text.primary}`} style={{ fontWeight: 600 }}>
                      {((focusedFrame.totalTime / rootData.totalTime) * 100).toFixed(1)}%
                    </span>
                  </div>
                </div>
                {/* Call stack */}
                {focusedFrame.children.length > 0 && (
                  <div>
                    <label
                      ...
                      style={{ fontWeight: 600 }}
                    >
                      {i.fgCallStack} ({focusedFrame.children.length})
                    </label>
                    <div className="mt-1 space-y-0.5">
                      {focusedFrame.children.map((child) => (
                        ...
                        >
                          ...
                          <span className={`font-mono truncate ${t.text.primary}`}>
                            {child.name}
                          </span>
                          <span className={`ml-auto text-[7px] ${t.text.dimmed}`}>
                            {child.totalTime}ms
                          </span>
                        </button>
                      ))}
