...
import {
  X, Plus, Search, LayoutGrid, List,
  ChevronDown, Trash2, Archive, Copy, CheckSquare, Square,
  AlertTriangle, ArrowUp, ArrowDown, Minus, Circle,
  Bug, Sparkles, FileCode, TestTube, BookOpen, Tag,
  Calendar, Timer, ArrowRight, Bot,
  CheckCircle2, Loader2, Ban, Eye, Pencil,
  Download, Save, FileText, Link2, GanttChart,
  Undo2, Redo2, Link, Zap, TrendingDown, GitBranch
} from 'lucide-react'
...
import {
  useTaskStore,
  type Task, type TaskStatus, type TaskPriority, type TaskType, type TaskSortBy
...
const STATUS_COLUMNS: { status: TaskStatus; colorClass: string; iconDark: string; iconLight: string }[] = [
  { status: 'todo', colorClass: 'text-slate-400', iconDark: 'bg-slate-500/15', iconLight: 'bg-slate-100' },
  { status: 'in-progress', colorClass: 'text-blue-400', iconDark: 'bg-blue-500/15', iconLight: 'bg-blue-50' },
  { status: 'review', colorClass: 'text-amber-400', iconDark: 'bg-amber-500/15', iconLight: 'bg-amber-50' },
  { status: 'done', colorClass: 'text-emerald-400', iconDark: 'bg-emerald-500/15', iconLight: 'bg-emerald-50' },
  { status: 'blocked', colorClass: 'text-red-400', iconDark: 'bg-red-500/15', iconLight: 'bg-red-50' },
]

const PRIORITY_CONFIG: Record<TaskPriority, { icon: React.ComponentType<any>; color: string; label: string }> = {
...
function priorityLabel(p: TaskPriority, i: I18nStrings): string {
  const map: Record<TaskPriority, string> = { critical: i.tbCritical || 'Critical', high: i.tbHigh || 'High', medium: i.tbMedium || 'Medium', low: i.tbLow || 'Low' }
  return map[p]
}

function typeLabel(t: TaskType, i: I18nStrings): string {
  const map: Record<TaskType, string> = { feature: i.tbFeature || 'Feature', bug: i.tbBug || 'Bug', refactor: i.tbRefactor || 'Refactor', test: i.tbTest || 'Test', documentation: i.tbDocumentation || 'Docs', other: i.tbOther || 'Other' }
  return map[t]
}
...
function StatusIcon({ status }: { status: TaskStatus }) {
  switch (status) {
    case 'todo': return <Circle className="w-3.5 h-3.5 text-slate-400" />
    case 'in-progress': return <Loader2 className="w-3.5 h-3.5 text-blue-400 animate-spin" />
    case 'review': return <Eye className="w-3.5 h-3.5 text-amber-400" />
...
export function TaskBoard({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const {
    viewMode, setViewMode, filter, setFilter, clearFilter,
    sortBy, setSortBy, sortOrder, toggleSortOrder,
    selectedTaskIds, clearSelection, batchUpdateStatus, batchDelete, batchArchive,
    getFilteredTasks, getTaskStats,
    addTask, updateTask, deleteTask, moveTask, duplicateTask,
  ...
  const filteredTasks = useMemo(() => getFilteredTasks(), [getFilteredTasks, filter, sortBy, sortOrder, useTaskStore(s => s.tasks)])
  const stats = useMemo(() => getTaskStats(), [useTaskStore(s => s.tasks)])

  const handleAddTask = useCallback(() => {
    if (!addTitle.trim()) return
    addTask({ title: addTitle.trim(), description: addDesc.trim() || undefined, status: 'todo', priority: addPriority, type: addType, source: 'manual' })
    setAddTitle('')
    setAddDesc('')
  ...
  const handleQuickMove = useCallback((taskId: string, targetStatus: TaskStatus) => {
    ...
    <AnimatePresence>
      {open && (
        <>
          ...
          >
            {/* ── Header ── */}
            <div className={`flex items-center justify-between px-5 py-3.5 border-b flex-shrink-0 ${t.isDark ? 'border-white/8' : 'border-slate-200/60'}`}>
              <div className="flex items-center gap-3">
                <div className={`p-1.5 rounded-lg ${t.isDark ? 'bg-indigo-500/15' : 'bg-indigo-50'}`}>
                  <CheckSquare className={`w-4.5 h-4.5 ${t.isDark ? 'text-indigo-400' : 'text-indigo-600'}`} />
                </div>
                <div>
              ...
              <div className="flex items-center gap-2">
                {[
                  { label: stats.total, color: t.isDark ? 'bg-slate-700/50 text-slate-300' : 'bg-slate-100 text-slate-600', tip: 'Total' },
                  { label: stats.inProgress, color: t.isDark ? 'bg-blue-500/15 text-blue-300' : 'bg-blue-50 text-blue-600', tip: 'In Progress' },
                  { label: stats.done, color: t.isDark ? 'bg-emerald-500/15 text-emerald-300' : 'bg-emerald-50 text-emerald-600', tip: 'Done' },
                  ...(stats.overdue > 0 ? [{ label: stats.overdue, color: t.isDark ? 'bg-red-500/15 text-red-300' : 'bg-red-50 text-red-600', tip: 'Overdue' }] : []),
                ].map((b, idx) => (
                  <span key={idx} className={`px-2 py-0.5 rounded-md text-[10px] ${b.color}`} style={{ fontWeight: 600 }} title={b.tip}>
                    {b.label}
                  </span>
                ))}
            ...
            {/* ── Toolbar ── */}
            <div className={`flex items-center gap-2 px-5 py-2.5 border-b flex-shrink-0 ${t.isDark ? 'border-white/5' : 'border-slate-100'}`}>
              {/* Search */}
              <div className="relative flex-1 max-w-52">
                <Search className={`absolute left-2.5 top-1/2 -translate-y-1/2 w-3 h-3 ${t.text.muted}`} />
                <input
                  type="text"
                  value={filter.search || ''}
              ...
              <select
                ...
                onChange={e => setFilter({ priority: (e.target.value || undefined) as TaskPriority | undefined })}
              ...
              >
                ...
                {(['critical', 'high', 'medium', 'low'] as TaskPriority[]).map(p => (
                  <option key={p} value={p}>{priorityLabel(p, i)}</option>
                ))}
              </select>
              ...
              <select
                value={filter.type || ''}
                onChange={e => setFilter({ type: (e.target.value || undefined) as TaskType | undefined })}
                className={`text-[11px] px-2 py-1.5 rounded-lg outline-none ${t.isDark ? 'bg-white/5 text-slate-200 border border-white/8' : 'bg-slate-50 text-slate-700 border border-slate-200'}`}
              >
                <option value="">{i.tbAllType || 'All Type'}</option>
                {(['feature', 'bug', 'refactor', 'test', 'documentation', 'other'] as TaskType[]).map(tp => (
                  <option key={tp} value={tp}>{typeLabel(tp, i)}</option>
                ))}
              </select>

              {filter.search || filter.priority || filter.type ? (
                <button onClick={clearFilter} className={`text-[10px] px-2 py-1 rounded-md ${t.isDark ? 'text-red-400 hover:bg-red-500/10' : 'text-red-500 hover:bg-red-50'}`}>
              ...
              <button onClick={toggleSortOrder} className={`p-1.5 rounded-lg ${t.interactive.iconBtn}`} title={sortOrder === 'asc' ? 'Ascending' : 'Descending'}>
                {sortOrder === 'asc' ? <ArrowUp className="w-3.5 h-3.5" /> : <ArrowDown className="w-3.5 h-3.5" />}
              </button>

              {/* View mode */}
              <div className={`flex rounded-lg overflow-hidden border ${t.isDark ? 'border-white/8' : 'border-slate-200'}`}>
                {[
                  ...
                  <button
                    ...
                    className={`p-1.5 ${viewMode === v.mode
                      ? t.isDark ? 'bg-indigo-500/20 text-indigo-300' : 'bg-indigo-50 text-indigo-600'
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] ${
                  t.isDark ? 'bg-indigo-500/20 text-indigo-300 hover:bg-indigo-500/30' : 'bg-indigo-50 text-indigo-600 hover:bg-indigo-100'
                }`}
              ...
              <button
                onClick={async () => {
                  try {
                    ...
                    const lastMessages = messages.slice(-10).map((m) => ({ role: m.role || 'user', content: m.content || '' }))
                    ...
                    toast.success(`${i.tbInferred || 'AI inferred'} ${count} ${i.tbTasks || 'tasks'}`)
                  } catch {
                    toast.error(i.tbInferError || 'AI inference failed')
                  }
                }}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] ${
                  t.isDark ? 'bg-purple-500/20 text-purple-300 hover:bg-purple-500/30' : 'bg-purple-50 text-purple-600 hover:bg-purple-100'
                }`}
                style={{ fontWeight: 600 }}
            ...
            {selectedTaskIds.length > 0 && (
              <div className={`flex items-center gap-2 px-5 py-2 border-b flex-shrink-0 ${t.isDark ? 'bg-indigo-500/5 border-indigo-500/10' : 'bg-indigo-50/50 border-indigo-100'}`}>
                <span className={`text-[11px] ${t.isDark ? 'text-indigo-300' : 'text-indigo-600'}`} style={{ fontWeight: 600 }}>
                  {selectedTaskIds.length} {i.tbSelected || 'selected'}
                </span>
                <button onClick={() => batchUpdateStatus(selectedTaskIds, 'done')} className={`text-[10px] px-2 py-1 rounded-md ${t.isDark ? 'bg-emerald-500/10 text-emerald-300' : 'bg-emerald-50 text-emerald-600'}`}>
                  <CheckCircle2 className="w-3 h-3 inline mr-1" />{i.tbMarkDone || 'Done'}
                </button>
                <button onClick={() => batchArchive(selectedTaskIds)} className={`text-[10px] px-2 py-1 rounded-md ${t.isDark ? 'bg-amber-500/10 text-amber-300' : 'bg-amber-50 text-amber-600'}`}>
                  <Archive className="w-3 h-3 inline mr-1" />{i.tbArchive || 'Archive'}
                </button>
                <button onClick={() => { batchDelete(selectedTaskIds); toast.success(i.tbDeleted || 'Deleted') }} className={`text-[10px] px-2 py-1 rounded-md ${t.isDark ? 'bg-red-500/10 text-red-300' : 'bg-red-50 text-red-600'}`}>
                  <Trash2 className="w-3 h-3 inline mr-1" />{i.tbDelete || 'Delete'}
                </button>
                <button onClick={clearSelection} className={`text-[10px] px-2 py-1 rounded-md ${t.isDark ? 'text-slate-400 hover:bg-white/5' : 'text-slate-500 hover:bg-slate-50'}`}>
            ...
            <div className="flex-1 overflow-auto custom-scrollbar">
              {viewMode === 'kanban' ? (
                <div className="flex gap-3 p-4 min-h-full" style={{ minWidth: STATUS_COLUMNS.length * 200 }}>
                  {STATUS_COLUMNS.map(col => (
                    <KanbanColumn
                      ...
                      onDelete={(id) => { deleteTask(id); toast.success(i.tbDeleted || 'Deleted') }}
                      onDuplicate={(id) => { duplicateTask(id); toast.success(i.tbDuplicated || 'Duplicated') }}
                ...
                <TimelineView tasks={filteredTasks} t={t} i={i} _onEdit={(task) => setEditingTask(task)} />
              ) : (
                /* ── List View ── */
                <div className="p-4 space-y-1">
                  {filteredTasks.map(task => (
                    <TaskListRow
                      ...
                      onDelete={() => { deleteTask(task.id); toast.success(i.tbDeleted || 'Deleted') }}
                    />
                  ))}
            ...
            {/* ── Footer ── */}
            <div className={`flex items-center justify-between px-5 py-2.5 border-t text-[10px] flex-shrink-0 ${t.isDark ? 'border-white/5 text-slate-500' : 'border-slate-100 text-slate-400'}`}>
              <span>{i.tbFooter || 'Drag tasks between columns or use quick actions. AI-inferred tasks shown with'} <Bot className="w-3 h-3 inline text-purple-400" /></span>
              <span>{stats.total} {i.tbTotalTasks || 'tasks'} · {stats.done} {i.tbCompleted || 'completed'}</span>
            </div>
            ...
            <AnimatePresence>
              {showAddForm && (
                ...
                >
                  ...
                  >
                    <h3 className={`text-[14px] mb-4 ${t.isDark ? 'text-white' : 'text-slate-900'}`} style={{ fontWeight: 600 }}>
                      {i.tbAddTask || 'Add Task'}
                    </h3>
                    <div className="space-y-3">
                      ...
                      <div className="flex gap-2">
                        <select value={addPriority} onChange={e => setAddPriority(e.target.value as TaskPriority)}
                          ...
                          {(['critical', 'high', 'medium', 'low'] as TaskPriority[]).map(p => <option key={p} value={p}>{priorityLabel(p, i)}</option>)}
                        </select>
                        <select value={addType} onChange={e => setAddType(e.target.value as TaskType)}
                          className={`flex-1 px-2 py-1.5 rounded-lg text-[11px] outline-none ${t.isDark ? 'bg-white/5 text-slate-200 border border-white/10' : 'bg-slate-50 text-slate-700 border border-slate-200'}`}>
                          {(['feature', 'bug', 'refactor', 'test', 'documentation', 'other'] as TaskType[]).map(tp => <option key={tp} value={tp}>{typeLabel(tp, i)}</option>)}
                        </select>
                      </div>
                      <div className="flex justify-end gap-2">
                        <button onClick={() => setShowAddForm(false)} className={`px-3 py-1.5 rounded-lg text-[11px] ${t.isDark ? 'text-slate-400 hover:bg-white/5' : 'text-slate-500 hover:bg-slate-50'}`}>
                          {i.tbCancel || 'Cancel'}
                        </button>
                        <button onClick={handleAddTask} disabled={!addTitle.trim()}
...
function KanbanColumn({
  ...
  onMove, onToggleSelect, onExpand, onDelete, onDuplicate,
  onToggleSubtask, onAddSubtask, onDeleteSubtask,
}: {
  status: TaskStatus; tasks: Task[]; t: ThemeTokens; i: I18nStrings
  ...
  return (
    <div ref={dropRef as unknown as React.Ref<HTMLDivElement>} key={status} className={`flex-1 min-w-[180px] rounded-xl border p-2.5 flex flex-col transition-all ${
      isOver
        ? t.isDark ? 'bg-indigo-500/10 border-indigo-500/30 ring-1 ring-indigo-500/20' : 'bg-indigo-50/60 border-indigo-300 ring-1 ring-indigo-200'
        : t.isDark ? 'bg-slate-800/30 border-white/5' : 'bg-slate-50/50 border-slate-200/50'
    }`}>
      {/* Column header */}
      <div className="flex items-center gap-2 mb-2.5 px-1">
        ...
        <span className={`text-[10px] px-1.5 py-0.5 rounded-md ${t.isDark ? 'bg-white/5 text-slate-400' : 'bg-slate-100 text-slate-500'}`}>
          {tasks.length}
        </span>
      </div>
...
function TimelineView({
  ...
  tasks: Task[]; t: ThemeTokens; i: I18nStrings
  ...
  const { updateTask, pushDateChange, undoDateChange, redoDateChange, dateUndoStack, dateRedoStack } = useTaskStore()

  // Zoom: day/week/month
  type ZoomLevel = 'day' | 'week' | 'month'
  const [zoom, setZoom] = useState<ZoomLevel>('day')
  const zoomConfig: Record<ZoomLevel, { pxPerDay: number; labelEvery: number; labelFn: (ts: number) => string }> = {
    day: { pxPerDay: 32, labelEvery: 1, labelFn: (ts) => { const d = new Date(ts); return `${d.getMonth() + 1}/${d.getDate()}` } },
    week: { pxPerDay: 12, labelEvery: 7, labelFn: (ts) => { const d = new Date(ts); return `W${Math.ceil(d.getDate() / 7)} ${d.getMonth() + 1}月` } },
    month: { pxPerDay: 4, labelEvery: 30, labelFn: (ts) => { const d = new Date(ts); return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}` } },
  }
  const zc = zoomConfig[zoom]

  const sortedTasks = useMemo(() => {
    return tasks.filter(tk => !tk.isArchived).slice().sort((a, b) => a.createdAt - b.createdAt)
  }, [tasks])

  const rangeStart = useMemo(() => {
    const earliest = sortedTasks.length > 0 ? Math.min(...sortedTasks.map(tk => tk.createdAt)) : now
  ...
  const headerLabels = useMemo(() => {
    ...
    for (let d = 0; d < totalDays; d += zc.labelEvery) {
      ...
      const isToday = zc.labelEvery === 1 && date.getFullYear() === todayDate.getFullYear() && date.getMonth() === todayDate.getMonth() && date.getDate() === todayDate.getDate()
      labels.push({ label: zc.labelFn(ts), leftPx: d * zc.pxPerDay, widthPx: zc.labelEvery * zc.pxPerDay, isToday })
    }
    return labels
  ...
  const resizeRef = useRef<{ startX: number; origTs: number; taskId: string; edge: 'left' | 'right' } | null>(null)

  const handleResizeStart = useCallback((e: React.MouseEvent, taskId: string, origTs: number, edge: 'left' | 'right') => {
    e.stopPropagation()
    e.preventDefault()
    ...
    const handleUp = () => {
      if (resizeRef.current) {
        const msg = resizeRef.current.edge === 'right' ? (i.tbDueDateUpdated || 'Due date updated') : (i.tbStartDateUpdated || 'Start date updated')
        toast.success(msg)
        // Push undo record
        const task = useTaskStore.getState().tasks.find(tk => tk.id === resizeRef.current!.taskId)
        if (task) {
          const field = resizeRef.current.edge === 'right' ? 'dueDate' as const : 'createdAt' as const
          const newValue = field === 'dueDate' ? task.dueDate : task.createdAt
          pushDateChange({ taskId: resizeRef.current.taskId, field, oldValue: resizeRef.current.origTs, newValue })
        }
      }
      setDragTooltip(null)
  ...
  const formatDate = (ts: number) => { const d = new Date(ts); return `${d.getMonth() + 1}/${d.getDate()}` }
  ...
  const criticalPathIds = useMemo(() => {
    ...
    for (const tk of sortedTasks) { adjList.set(tk.id, []); inDeg.set(tk.id, 0) }
    ...
    for (const tk of sortedTasks) { dist.set(tk.id, getDuration(tk)); pred.set(tk.id, null) }
    const q: string[] = []
    ...
    while (q.length > 0) {
      ...
      for (const next of (adjList.get(node) || [])) {
        ...
        if (nd > (dist.get(next) || 0)) { dist.set(next, nd); pred.set(next, node) }
        inDeg.set(next, (inDeg.get(next) || 1) - 1)
        if (inDeg.get(next) === 0) q.push(next)
      }
    }
    let maxD = 0, endN: string | null = null
    for (const [id, d] of dist) { if (d > maxD) { maxD = d; endN = id } }
    const cpSet = new Set<string>()
    let cur = endN
    while (cur) { cpSet.add(cur); cur = pred.get(cur) || null }
    return cpSet
  }, [sortedTasks])
  ...
  const [dragConnect, setDragConnect] = useState<{
    fromId: string; fromX: number; fromY: number; mouseX: number; mouseY: number
  } | null>(null)
  const timelineAreaRef = useRef<HTMLDivElement>(null)

  const handleDeleteDependency = useCallback((fromId: string, toId: string) => {
  ...
  const buildTimelineSvg = useCallback(() => {
    ...
    if (!container) { toast.error('No timeline to export'); return null }
    ...
    sortedTasks.forEach((task, idx) => {
      ...
      const colors: Record<string, string> = { critical: '#ef4444', high: '#f97316', medium: '#3b82f6', low: '#22c55e' }
    ...
    depArrows.forEach(arrow => {
      ...
      const fy = arrow.fromY + 20, ty = arrow.toY + 20
  ...
  }, [scrollContainerRef, totalDays, zc, sortedTasks, rangeStart, DAY_MS, now, t, criticalPathIds, LABEL_W, ROW_H])
  ...
  const handleAddDependency = useCallback((fromId: string, toId: string) => {
    if (fromId === toId) { setConnectSource(null); return }
    ...
    while (stack.length > 0) {
      ...
      if (node === fromId) { hasCycle = true; break }
  ...
  const handleDragConnectStart = useCallback((e: React.MouseEvent, taskId: string, barRightX: number, barCenterY: number) => {
    e.stopPropagation()
    e.preventDefault()
    const rect = timelineAreaRef.current?.getBoundingClientRect()
    if (!rect) return
    setDragConnect({ fromId: taskId, fromX: barRightX, fromY: barCenterY, mouseX: e.clientX - rect.left, mouseY: e.clientY - rect.top })

    const onMove = (me: MouseEvent) => {
      if (!timelineAreaRef.current) return
      const r = timelineAreaRef.current.getBoundingClientRect()
      setDragConnect(prev => prev ? { ...prev, mouseX: me.clientX - r.left, mouseY: me.clientY - r.top } : null)
    }
  ...
  const aiOptimizationSuggestions = useMemo(() => {
    ...
    for (const tk of cpTasks) {
      ...
      if (dur > longestDur) { longestDur = dur; longestTask = tk }
    }
    if (longestTask) {
      suggestions.push({
        icon: 'bottleneck',
    ...
    const nonCpTasks = sortedTasks.filter(tk => !criticalPathIds.has(tk.id) && tk.status !== 'done')
    ...
    const overdueCp = cpTasks.filter(tk => tk.dueDate && tk.dueDate < now && tk.status !== 'done')
  ...
  useEffect(() => {
    ...
    if (!el) return
    const onScr = () => { setScrollLeft(el.scrollLeft); setContainerWidth(el.clientWidth) }
    onScr()
    el.addEventListener('scroll', onScr)
    window.addEventListener('resize', onScr)
    return () => { el.removeEventListener('scroll', onScr); window.removeEventListener('resize', onScr) }
  }, [sortedTasks.length])
  ...
  const handleMinimapDrag = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    e.preventDefault()
    const startX = e.clientX
    const startScroll = scrollLeft
    const onMove = (me: MouseEvent) => {
      const dx = me.clientX - startX
      scrollContainerRef.current?.scrollTo({ left: Math.max(0, startScroll + dx / minimapScale) })
    }
  ...
  const depArrows = useMemo(() => {
    const arrows: { fromX: number; fromY: number; toX: number; toY: number; fromId: string; toId: string }[] = []
    ...
    for (const task of sortedTasks) {
      ...
      for (const depId of task.dependencies) {
        ...
        const fromEndTs = fromTask.dueDate || (fromTask.createdAt + (fromTask.estimatedHours || 8) * 3600000)
  ...
  return (
    <div className="p-4 overflow-auto custom-scrollbar">
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        <div className={`flex rounded-lg overflow-hidden border ${t.isDark ? 'border-white/8' : 'border-slate-200'}`}>
          {(['day', 'week', 'month'] as ZoomLevel[]).map(z => (
            ...
            >
              {z === 'day' ? (i.tbDay || 'Day') : z === 'week' ? (i.tbWeek || 'Week') : (i.tbMonth || 'Month')}
            </button>
          ))}
        </div>
        {/* Undo / Redo buttons */}
        <button
          onClick={() => { undoDateChange(); toast.info(i.tbUndone || 'Undo date change') }}
        ...
        <button
          onClick={() => { redoDateChange(); toast.info(i.tbRedone || 'Redo date change') }}
        ...
        <button
          ...
          className={`px-2 py-0.5 rounded text-[10px] ${showCriticalPath
            ? t.isDark ? 'bg-amber-500/20 text-amber-300 ring-1 ring-amber-500/30' : 'bg-amber-50 text-amber-700 ring-1 ring-amber-200'
            : t.isDark ? 'text-slate-400 hover:bg-white/5' : 'text-slate-500 hover:bg-slate-50'
          }`}
          style={{ fontWeight: 500 }}
          title={i.tbCriticalPath || 'Critical Path'}
        >
          {i.tbCriticalPath || 'Critical Path'} {showCriticalPath ? `(${criticalPathIds.size})` : ''}
        </button>
        {/* Dep edit mode toggle */}
        <button
          onClick={() => { setDepEditMode(v => !v); setConnectSource(null) }}
          className={`px-2 py-0.5 rounded text-[10px] ${depEditMode
            ? t.isDark ? 'bg-cyan-500/20 text-cyan-300 ring-1 ring-cyan-500/30' : 'bg-cyan-50 text-cyan-700 ring-1 ring-cyan-200'
            : t.isDark ? 'text-slate-400 hover:bg-white/5' : 'text-slate-500 hover:bg-slate-50'
          }`}
          style={{ fontWeight: 500 }}
        ...
        <button
          ...
          className={`px-2 py-0.5 rounded text-[10px] ${showDepGraph
            ? t.isDark ? 'bg-violet-500/20 text-violet-300 ring-1 ring-violet-500/30' : 'bg-violet-50 text-violet-700 ring-1 ring-violet-200'
            : t.isDark ? 'text-slate-400 hover:bg-white/5' : 'text-slate-500 hover:bg-slate-50'
          }`}
          style={{ fontWeight: 500 }}
        ...
        {showCriticalPath && criticalPathIds.size > 1 && (
          <button
            ...
            className={`px-2 py-0.5 rounded text-[10px] ${showAiOptimize
              ? t.isDark ? 'bg-purple-500/20 text-purple-300 ring-1 ring-purple-500/30' : 'bg-purple-50 text-purple-700 ring-1 ring-purple-200'
              : t.isDark ? 'text-slate-400 hover:bg-white/5' : 'text-slate-500 hover:bg-slate-50'
            }`}
            style={{ fontWeight: 500 }}
        ...
        <div className="relative">
          ...
          {showExportMenu && (
            <div className={`absolute top-full mt-1 right-0 z-50 rounded-lg py-1 min-w-[120px] border shadow-lg ${t.isDark ? 'bg-slate-800 border-white/10' : 'bg-white border-slate-200'}`}>
              <button onClick={() => { handleExportSvg(); setShowExportMenu(false) }} className={`w-full text-left px-3 py-1.5 text-[10px] ${t.isDark ? 'hover:bg-white/5 text-slate-300' : 'hover:bg-slate-50 text-slate-700'}`} style={{ fontWeight: 500 }}>
                SVG {i.tbVector || '(Vector)'}
              </button>
              <button onClick={() => { handleExportPng(); setShowExportMenu(false) }} className={`w-full text-left px-3 py-1.5 text-[10px] ${t.isDark ? 'hover:bg-white/5 text-slate-300' : 'hover:bg-slate-50 text-slate-700'}`} style={{ fontWeight: 500 }}>
        ...
        <span className={`text-[9px] ${t.text.dimmed} ml-2`}>
          {depEditMode
            ? (connectSource ? (i.tbDepClickTarget || 'Click a task bar to set target') : (i.tbDepClickSource || 'Click a task bar to set source'))
      ...
      {cycleWarning && (
        <div className={`flex items-center gap-2 px-3 py-2 rounded-lg mb-2 animate-pulse ${t.isDark ? 'bg-red-500/15 border border-red-500/30' : 'bg-red-50 border border-red-200'}`}>
          <AlertTriangle className={`w-4 h-4 flex-shrink-0 ${t.isDark ? 'text-red-400' : 'text-red-600'}`} />
          <span className={`text-[11px] ${t.isDark ? 'text-red-300' : 'text-red-700'}`} style={{ fontWeight: 600 }}>
            {i.tbCycleDetected || 'Circular dependency detected!'}{' '}
            <span style={{ fontWeight: 400 }}>
              {i.tbCycleDesc || 'Adding this dependency would create a cycle. Dependency graphs must be acyclic (DAG).'}
            </span>
          </span>
          <button onClick={() => setCycleWarning(null)} className={`ml-auto p-0.5 rounded ${t.isDark ? 'hover:bg-white/10' : 'hover:bg-red-100'}`}>
            <X className="w-3 h-3" />
          </button>
        </div>
      )}
      ...
      {showDepGraph && (
        <DependencyGraph tasks={sortedTasks} criticalPathIds={criticalPathIds} t={t} i={i} _onEdit={_onEdit} />
      )}

      {sortedTasks.length === 0 ? (
        <div className={`text-center py-12 text-[12px] ${t.text.muted}`}>{i.tbNoTasks || 'No tasks match filters'}</div>
      ) : (
        <div className="relative" ref={(el) => { (scrollContainerRef as React.MutableRefObject<HTMLDivElement | null>).current = el; (timelineAreaRef as React.MutableRefObject<HTMLDivElement | null>).current = el }} data-timeline-area style={{ overflowX: 'auto' }}>
          {/* Header row */}
            ...
            {headerLabels.map((hl, idx) => (
              <div
                ...
                className={`text-center text-[8px] border-r flex-shrink-0 ${
                  hl.isToday
                    ? t.isDark ? 'bg-indigo-500/10 text-indigo-300 border-indigo-500/20' : 'bg-indigo-50 text-indigo-600 border-indigo-200'
                    : t.isDark ? 'text-slate-500 border-white/5' : 'text-slate-400 border-slate-100'
                }`}
          ...
          {depArrows.length > 0 && (
            <svg
              ...
              style={{ width: LABEL_W + totalDays * zc.pxPerDay, height: sortedTasks.length * ROW_H }}
            >
              <defs>
                ...
                <marker id="dep-arrowhead-cp" markerWidth="6" markerHeight="4" refX="6" refY="2" orient="auto">
              ...
              {depArrows.map((arrow, idx) => {
                ...
                const isCpArrow = showCriticalPath && criticalPathIds.has(arrow.fromId) && criticalPathIds.has(arrow.toId)
                return (
                  <g key={`${arrow.fromId}-${arrow.toId}-${idx}`}>
                    ...
                    <path
                      d={pathD}
                      fill="none"
                      stroke={isCpArrow ? (t.isDark ? '#fbbf24' : '#d97706') : (t.isDark ? '#818cf8' : '#6366f1')}
          ...
          {dragConnect && (
            <svg className="absolute top-0 left-0 pointer-events-none z-20" style={{ width: '100%', height: sortedTasks.length * ROW_H }}>
              <line
                x1={dragConnect.fromX} y1={dragConnect.fromY}
                x2={dragConnect.mouseX} y2={dragConnect.mouseY}
                stroke={t.isDark ? '#34d399' : '#10b981'} strokeWidth="2" strokeDasharray="6 3"
              />
              <circle cx={dragConnect.fromX} cy={dragConnect.fromY} r="4" fill={t.isDark ? '#34d399' : '#10b981'} />
              <circle cx={dragConnect.mouseX} cy={dragConnect.mouseY} r="4" fill={t.isDark ? '#34d399' : '#10b981'} opacity="0.6" />
            </svg>
          )}

          {/* Task rows */}
          {sortedTasks.map(task => (
            <TimelineRow key={task.id} task={task} rangeStart={rangeStart} DAY_MS={DAY_MS} zc={zc} now={now} totalDays={totalDays} todayPx={todayPx} zoom={zoom} t={t} PRIORITY_BAR_COLORS={PRIORITY_BAR_COLORS} resizingTaskId={resizingTaskId} _onEdit={_onEdit} handleResizeStart={handleResizeStart} formatDate={formatDate} isCriticalPath={showCriticalPath && criticalPathIds.has(task.id)} depEditMode={depEditMode} connectSource={connectSource} onDepConnect={(taskId) => { if (!connectSource) setConnectSource(taskId); else { handleAddDependency(connectSource, taskId); } }} onDragConnectStart={handleDragConnectStart} />
          ))}
        </div>
      )}
        ...
        <div className="flex items-center gap-2 mt-2 px-1">
          ...
          >
            ...
            {sortedTasks.map((tk, idx) => {
              ...
              const left = Math.max(0, ((startTs - rangeStart) / DAY_MS) * zc.pxPerDay + LABEL_W) * minimapScale
            ...
            <div
              ...
              style={{ left: minimapViewportX, width: Math.min(minimapViewportW, MINIMAP_W - minimapViewportX), cursor: 'grab' }}
      ...
      {showAiOptimize && aiOptimizationSuggestions.length > 0 && (
        <div className={`mt-3 rounded-xl border p-3 ${t.isDark ? 'bg-purple-500/5 border-purple-500/20' : 'bg-purple-50 border-purple-200'}`}>
          <div className="flex items-center gap-2 mb-2">
            <Zap className={`w-3.5 h-3.5 ${t.isDark ? 'text-purple-400' : 'text-purple-600'}`} />
            <span className={`text-[11px] ${t.isDark ? 'text-purple-300' : 'text-purple-700'}`} style={{ fontWeight: 600 }}>
              {i.tbAiOptTitle || 'AI Critical Path Optimization'}
            </span>
          </div>
          <div className="space-y-2">
            {aiOptimizationSuggestions.map((sug, idx) => (
              <div key={idx} className={`flex items-start gap-2 p-2 rounded-lg ${t.isDark ? 'bg-slate-800/40' : 'bg-white'}`}>
                <div className="flex-shrink-0 mt-0.5">
                  {sug.icon === 'bottleneck' && <TrendingDown className={`w-3.5 h-3.5 ${t.isDark ? 'text-amber-400' : 'text-amber-600'}`} />}
                  {sug.icon === 'parallel' && <Sparkles className={`w-3.5 h-3.5 ${t.isDark ? 'text-green-400' : 'text-green-600'}`} />}
                  {sug.icon === 'overdue' && <AlertTriangle className={`w-3.5 h-3.5 ${t.isDark ? 'text-red-400' : 'text-red-600'}`} />}
                  {sug.icon === 'blocked' && <Ban className={`w-3.5 h-3.5 ${t.isDark ? 'text-orange-400' : 'text-orange-600'}`} />}
                </div>
                <div className="flex-1 min-w-0">
                  <span className={`text-[10px] block ${t.isDark ? 'text-white' : 'text-slate-800'}`} style={{ fontWeight: 600 }}>{sug.title}</span>
                  <span className={`text-[9px] block mt-0.5 ${t.text.dimmed}`}>{sug.description}</span>
                  {sug.taskIds.length > 0 && (
                    <div className="flex gap-1 mt-1 flex-wrap">
                      {sug.taskIds.map(tid => {
                        const tk = sortedTasks.find(t => t.id === tid)
                        return tk ? (
                          <button key={tid} onClick={() => _onEdit(tk)} className={`text-[8px] px-1.5 py-0.5 rounded ${t.isDark ? 'bg-white/5 text-indigo-300 hover:bg-white/10' : 'bg-indigo-50 text-indigo-600 hover:bg-indigo-100'}`}>
      ...
      {dragTooltip && (
        <div
          className={`fixed z-[9999] px-2 py-1 rounded-md text-[10px] pointer-events-none shadow-lg ${
            t.isDark ? 'bg-slate-800 text-indigo-300 border border-indigo-500/30' : 'bg-white text-indigo-700 border border-indigo-200'
          }`}
...
function TimelineRow({ task, style, rangeStart, DAY_MS, zc, now, totalDays, todayPx, zoom, t, PRIORITY_BAR_COLORS, resizingTaskId, _onEdit, handleResizeStart, formatDate, isCriticalPath, depEditMode, connectSource, onDepConnect, onDragConnectStart 
}: {
  task: Task; style?: React.CSSProperties; rangeStart: number; DAY_MS: number
  zc: { pxPerDay: number; labelEvery: number }; now: number; totalDays: number; todayPx: number
  zoom: string; t: ThemeTokens; PRIORITY_BAR_COLORS: Record<string, string>; resizingTaskId: string | null
  _onEdit: (task: Task) => void
  handleResizeStart: (e: React.MouseEvent, taskId: string, origTs: number, edge: 'left' | 'right') => void
  formatDate: (ts: number) => string
  isCriticalPath?: boolean
  depEditMode?: boolean
  connectSource?: string | null
  onDepConnect?: (taskId: string) => void
  onDragConnectStart?: (e: React.MouseEvent, taskId: string, barRightX: number, barCenterY: number) => void
}) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className={`text-[10px] truncate ${task.status === 'done' ? 'line-through opacity-50' : ''}`} style={{ fontWeight: 500 }}>
          {task.title}
        </span>
        {task.source === 'ai-inferred' && <Bot className="w-2.5 h-2.5 text-purple-400 flex-shrink-0" />}
      ...
      <div className="flex-1 relative h-full" style={{ minWidth: totalDays * zc.pxPerDay }}>
        <div className={`absolute top-0 bottom-0 w-px ${t.isDark ? 'bg-indigo-400/30' : 'bg-indigo-300/50'}`} style={{ left: todayPx }} />
        <div
        ...
        >
          {zoom !== 'month' && (
            <span className="text-[8px] text-white px-1 truncate block leading-5" style={{ fontWeight: 600 }}>
              {task.title.length > widthPx / 5 ? task.title.substring(0, Math.floor(widthPx / 5)) + '…' : task.title}
            </span>
          )}
          {/* Left resize */}
          <div className="absolute left-0 top-0 bottom-0 w-2 cursor-ew-resize group/left" onMouseDown={(e) => handleResizeStart(e, task.id, startTs, 'left')}>
            <div className={`absolute left-0 top-1 bottom-1 w-0.5 rounded-full opacity-0 group-hover/left:opacity-100 transition-opacity ${t.isDark ? 'bg-white/60' : 'bg-slate-600/60'}`} />
          </div>
          {/* Left dot — dep drop target */}
          <div data-task-bar-id={task.id} className={`absolute -left-1.5 top-1/2 -translate-y-1/2 w-3 h-3 rounded-full border-2 opacity-0 group-hover/bar:opacity-100 transition-opacity z-10 ${t.isDark ? 'bg-cyan-400 border-cyan-300' : 'bg-cyan-500 border-cyan-400'}`} />
          {/* Right resize */}
          <div className="absolute right-0 top-0 bottom-0 w-2 cursor-ew-resize group/right" onMouseDown={(e) => handleResizeStart(e, task.id, endTs, 'right')}>
            <div className={`absolute right-0 top-1 bottom-1 w-0.5 rounded-full opacity-0 group-hover/right:opacity-100 transition-opacity ${t.isDark ? 'bg-white/60' : 'bg-slate-600/60'}`} />
          </div>
            ...
            onMouseDown={(e) => {
              if (onDragConnectStart) {
                ...
                if (barEl && area) {
                  ...
                  onDragConnectStart(e, task.id, br.right - ar.left, br.top + br.height / 2 - ar.top)
                }
              }
            }}
          />
...
function TaskCard({
  task, t, i, isSelected, isExpanded,
  onToggleSelect, onExpand, onMove, onDelete, onDuplicate,
  onToggleSubtask, onAddSubtask, onDeleteSubtask,
}: {
  task: Task; t: ThemeTokens; i: I18nStrings; isSelected: boolean; isExpanded: boolean
  onToggleSelect: () => void; onExpand: () => void
  onMove: (status: TaskStatus) => void; onDelete: () => void
  onDuplicate: () => void
  ...
  return (
    <div
      ...
      className={`rounded-lg border p-2.5 cursor-grab active:cursor-grabbing transition-all ${
        ...
        isSelected
          ? t.isDark ? 'bg-indigo-500/10 border-indigo-500/30' : 'bg-indigo-50 border-indigo-300'
          : t.isDark ? 'bg-slate-900/40 border-white/5 hover:border-white/10' : 'bg-white border-slate-200/60 hover:border-slate-300'
      }`}
      onClick={onExpand}
    >
      {/* Top row */}
      <div className="flex items-start gap-1.5">
        <button onClick={e => { e.stopPropagation(); onToggleSelect() }} className="mt-0.5 flex-shrink-0">
          {isSelected
            ? <CheckSquare className={`w-3.5 h-3.5 ${t.isDark ? 'text-indigo-400' : 'text-indigo-500'}`} />
            : <Square className={`w-3.5 h-3.5 ${t.text.dimmed}`} />
          }
        </button>
        <div className="flex-1 min-w-0">
          <p className={`text-[11px] leading-snug ${t.isDark ? 'text-slate-200' : 'text-slate-800'} ${task.status === 'done' ? 'line-through opacity-60' : ''}`} style={{ fontWeight: 500 }}>
            {task.title}
          </p>
        </div>
      ...
      <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
        <span className={`inline-flex items-center gap-0.5 text-[9px] ${PRIORITY_CONFIG[task.priority].color}`}>
          <PriorityIcon className="w-2.5 h-2.5" />
          {priorityLabel(task.priority, i)}
        </span>
        <span className={`inline-flex items-center gap-0.5 text-[9px] ${TYPE_CONFIG[task.type].color}`}>
          <TypeIcon className="w-2.5 h-2.5" />
        </span>
        {totalSubs > 0 && (
          <span className={`text-[9px] ${t.text.dimmed}`}>{completedSubs}/{totalSubs}</span>
        )}
        {task.tags?.slice(0, 2).map(tag => (
          <span key={tag} className={`text-[8px] px-1 py-0.5 rounded ${t.isDark ? 'bg-white/5 text-slate-400' : 'bg-slate-100 text-slate-500'}`}>{tag}</span>
        ))}
      </div>

      {/* Expanded Details */}
      {isExpanded && (
        <div className={`mt-2 pt-2 border-t space-y-2 ${t.isDark ? 'border-white/5' : 'border-slate-100'}`} onClick={e => e.stopPropagation()}>
          {task.description && (
            <p className={`text-[10px] ${t.text.muted}`}>{task.description}</p>
          )}
            ...
            <div className="space-y-1">
              {task.subtasks.map(st => (
                <div key={st.id} className="flex items-center gap-1.5 group">
                  ...
                  <span className={`text-[10px] flex-1 ${st.isCompleted ? 'line-through opacity-50' : t.isDark ? 'text-slate-300' : 'text-slate-600'}`}>{st.title}</span>
                  <button onClick={() => onDeleteSubtask(st.id)} className={`opacity-0 group-hover:opacity-100 ${t.isDark ? 'text-red-400' : 'text-red-500'}`}>
          ...
          <div className="flex items-center gap-1.5">
            ...
            />
            <button
              onClick={() => { if (subInput.trim()) { onAddSubtask(subInput.trim()); setSubInput('') } }}
          ...
          <div className="flex items-center gap-1 flex-wrap">
            ...
            <div className="flex-1" />
            <button onClick={onDuplicate} className={`p-1 rounded ${t.isDark ? 'hover:bg-white/5 text-slate-400' : 'hover:bg-slate-50 text-slate-500'}`} title={i.tbDuplicate || 'Duplicate'}>
              <Copy className="w-3 h-3" />
            </button>
            <button onClick={onDelete} className={`p-1 rounded ${t.isDark ? 'hover:bg-red-500/10 text-red-400' : 'hover:bg-red-50 text-red-500'}`} title={i.tbDelete || 'Delete'}>
...
function TaskListRow({
  task, t, i, isSelected, onToggleSelect, onMove, onDelete,
}: {
  task: Task; t: ThemeTokens; i: I18nStrings; isSelected: boolean
  onToggleSelect: () => void; onMove: (status: TaskStatus) => void
  onDelete: () => void
}) {
  const PriorityIcon = PRIORITY_CONFIG[task.priority].icon
  const TypeIcon = TYPE_CONFIG[task.type].icon
  ...
  return (
    <div className={`flex items-center gap-3 px-3 py-2 rounded-lg border transition-all ${
      isSelected
        ...
        : t.isDark ? 'bg-slate-800/20 border-white/5 hover:bg-slate-800/40' : 'bg-white/60 border-slate-200/50 hover:bg-white'
    }`}>
      ...
      <div className="flex-1 min-w-0">
        <span className={`text-[12px] ${t.isDark ? 'text-slate-200' : 'text-slate-800'} ${task.status === 'done' ? 'line-through opacity-60' : ''}`} style={{ fontWeight: 500 }}>
          {task.title}
        </span>
        {task.description && (
      ...
      >
        {STATUS_COLUMNS.map(c => <option key={c.status} value={c.status}>{statusLabel(c.status, i)}</option>)}
      </select>

      <button onClick={onDelete} className={`p-1 rounded flex-shrink-0 ${t.isDark ? 'hover:bg-red-500/10 text-red-400' : 'hover:bg-red-50 text-red-500'}`}>
        <Trash2 className="w-3 h-3" />
      </button>
    </div>
  )
...
function DependencyGraph({ tasks, criticalPathIds, t, i, _onEdit }: {
  tasks: Task[]; criticalPathIds: Set<string>; t: ThemeTokens; i: I18nStrings; _onEdit: (task: Task) => void
}) {
  const canvasRef = useRef<HTMLCanvasElement>(null)
  const nodesRef = useRef<Map<string, { x: number; y: number; vx: number; vy: number; task: Task }>>(new Map())
  const animRef = useRef<number>(0)
  const [hovered, setHovered] = useState<string | null>(null)
  const dragRef = useRef<{ id: string; offsetX: number; offsetY: number } | null>(null)

  const PRI_COLORS: Record<string, string> = { critical: '#ef4444', high: '#f97316', medium: '#3b82f6', low: '#22c55e' }
  const STA_COLORS: Record<string, string> = { 'todo': '#94a3b8', 'in-progress': '#3b82f6', 'review': '#f59e0b', 'done': '#22c55e', 'blocked': '#ef4444' }
  ...
  useEffect(() => {
    ...
    for (const eid of existingIds) { if (!newIds.has(eid)) nodes.delete(eid) }
    for (const tk of tasks) {
      if (!nodes.has(tk.id)) {
        nodes.set(tk.id, { x: cw / 2 + (Math.random() - 0.5) * cw * 0.6, y: ch / 2 + (Math.random() - 0.5) * ch * 0.6, vx: 0, vy: 0, task: tk })
      } else { nodes.get(tk.id)!.task = tk }
    }
    const edges: { from: string; to: string }[] = []
    for (const tk of tasks) { for (const depId of (tk.dependencies || [])) { if (nodes.has(depId)) edges.push({ from: depId, to: tk.id }) } }
    let iter = 0
    const maxIter = 300
    const simulate = () => {
      if (iter > maxIter) { animRef.current = requestAnimationFrame(draw); return }
      iter++
      ...
      for (let ii = 0; ii < na.length; ii++) {
        for (let jj = ii + 1; jj < na.length; jj++) {
          const a = na[ii], b = na[jj]
          const dx = b.x - a.x, dy = b.y - a.y, dist = Math.sqrt(dx * dx + dy * dy) || 1
          const f = 8000 / (dist * dist), fx = (dx / dist) * f, fy = (dy / dist) * f
      ...
      for (const e of edges) {
        ...
        if (!a || !b) continue
        const dx = b.x - a.x, dy = b.y - a.y, dist = Math.sqrt(dx * dx + dy * dy) || 1
        const f = (dist - 120) * 0.01, fx = (dx / dist) * f, fy = (dy / dist) * f
        a.vx += fx * alpha; a.vy += fy * alpha; b.vx -= fx * alpha; b.vy -= fy * alpha
      }
      for (const n of na) {
        n.vx += (cw / 2 - n.x) * 0.001; n.vy += (ch / 2 - n.y) * 0.001
        n.vx *= 0.85; n.vy *= 0.85
        if (!dragRef.current || dragRef.current.id !== n.task.id) { n.x += n.vx; n.y += n.vy }
        n.x = Math.max(30, Math.min(cw - 30, n.x)); n.y = Math.max(30, Math.min(ch - 30, n.y))
      }
    ...
    const draw = () => {
      ...
      for (const e of edges) {
        ...
        ctx.strokeStyle = isCp ? '#fbbf24' : (isDark ? 'rgba(129,140,248,0.4)' : 'rgba(99,102,241,0.3)')
        ctx.lineWidth = isCp ? 2.5 : 1.5
        if (!isCp) ctx.setLineDash([4, 3]); else ctx.setLineDash([])
        ctx.stroke(); ctx.setLineDash([])
        const angle = Math.atan2(b.y - my, b.x - mx)
        ctx.beginPath(); ctx.moveTo(b.x, b.y)
        ctx.lineTo(b.x - 8 * Math.cos(angle - 0.3), b.y - 8 * Math.sin(angle - 0.3))
        ctx.lineTo(b.x - 8 * Math.cos(angle + 0.3), b.y - 8 * Math.sin(angle + 0.3))
        ctx.closePath(); ctx.fillStyle = isCp ? '#fbbf24' : (isDark ? '#818cf8' : '#6366f1'); ctx.fill()
      }
      for (const [nid, n] of nodes) {
        const r = hovered === nid ? 22 : 18, isCp = criticalPathIds.has(nid)
        if (isCp) { ctx.beginPath(); ctx.arc(n.x, n.y, r + 4, 0, Math.PI * 2); ctx.fillStyle = 'rgba(251,191,36,0.15)'; ctx.fill() }
        ctx.beginPath(); ctx.arc(n.x, n.y, r, 0, Math.PI * 2)
        const sc = STA_COLORS[n.task.status] || '#94a3b8'
        ctx.fillStyle = sc + (isDark ? '40' : '30'); ctx.fill(); ctx.strokeStyle = sc; ctx.lineWidth = hovered === nid ? 2.5 : 1.5; ctx.stroke()
        ctx.beginPath(); ctx.arc(n.x, n.y, 4, 0, Math.PI * 2); ctx.fillStyle = PRI_COLORS[n.task.priority] || '#3b82f6'; ctx.fill()
        ctx.fillStyle = isDark ? '#e2e8f0' : '#334155'; ctx.font = `${hovered === nid ? 600 : 500} 9px sans-serif`; ctx.textAlign = 'center'
        ctx.fillText(n.task.title.length > 16 ? n.task.title.substring(0, 16) + '..' : n.task.title, n.x, n.y + r + 12)
      }
    }
    simulate()
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (dragRef.current) { const n = nodesRef.current.get(dragRef.current.id); if (n) { n.x = mx; n.y = my }; return }
    let found: string | null = null
    for (const [id, n] of nodesRef.current) { if ((n.x - mx) ** 2 + (n.y - my) ** 2 < 400) { found = id; break } }
    setHovered(found)
  }, [])
  const handleMouseDown = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    const canvas = canvasRef.current; if (!canvas) return
    const rect = canvas.getBoundingClientRect(), mx = e.clientX - rect.left, my = e.clientY - rect.top
    for (const [id, n] of nodesRef.current) { if ((n.x - mx) ** 2 + (n.y - my) ** 2 < 400) { dragRef.current = { id, offsetX: n.x - mx, offsetY: n.y - my }; break } }
  }, [])
  const handleMouseUp = useCallback(() => { dragRef.current = null }, [])
  const handleDblClick = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    const canvas = canvasRef.current; if (!canvas) return
    const rect = canvas.getBoundingClientRect(), mx = e.clientX - rect.left, my = e.clientY - rect.top
    for (const [, n] of nodesRef.current) { if ((n.x - mx) ** 2 + (n.y - my) ** 2 < 400) { _onEdit(n.task); break } }
  }, [_onEdit])

  return (
    <div className={`mb-3 rounded-xl border overflow-hidden ${t.isDark ? 'bg-slate-900/50 border-white/8' : 'bg-white/80 border-slate-200/60'}`}>
      <div className={`flex items-center gap-2 px-3 py-1.5 border-b ${t.isDark ? 'border-white/5' : 'border-slate-100'}`}>
      ...
      <canvas ref={canvasRef} className="w-full cursor-grab active:cursor-grabbing" style={{ height: 320 }}
        onMouseMove={handleMouseMove} onMouseDown={handleMouseDown} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp} onDoubleClick={handleDblClick}
      />
    </div>
  )
}
...
function EditTaskModal({
  task, t, i, onSave, onClose,
}: {
  task: Task; t: ThemeTokens; i: I18nStrings
  ...
  const [dueDate, setDueDate] = useState(task.dueDate ? new Date(task.dueDate).toISOString().substring(0, 16) : '')
    ...
    >
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <h3 className={`text-[14px] ${t.isDark ? 'text-white' : 'text-slate-900'}`} style={{ fontWeight: 600 }}>
        ...
        <div className="space-y-3">
          {/* Title */}
          <div>
            <label className={`text-[10px] mb-1 block ${t.text.muted}`}>{i.tbTitlePlaceholder || 'Title'}</label>
            <input autoFocus value={title} onChange={e => setTitle(e.target.value)} className={inputCls} />
          </div>

          {/* Description */}
          <div>
            <label className={`text-[10px] mb-1 block ${t.text.muted}`}>{i.tbDescPlaceholder || 'Description'}</label>
          ...
          <div className="flex gap-2">
            ...
            <div className="flex-1">
              <label className={`text-[10px] mb-1 block ${t.text.muted}`}>{i.tbSortPriority || 'Priority'}</label>
              <select value={priority} onChange={e => setPriority(e.target.value as TaskPriority)} className={selectCls}>
                {(['critical', 'high', 'medium', 'low'] as TaskPriority[]).map(p => <option key={p} value={p}>{priorityLabel(p, i)}</option>)}
              </select>
            </div>
            <div className="flex-1">
              <label className={`text-[10px] mb-1 block ${t.text.muted}`}>{i.tbAllType || 'Type'}</label>
          ...
          <div>
            ...
            <input value={tags} onChange={e => setTags(e.target.value)} placeholder="p1, ai, core" className={inputCls} />
          </div>

          {/* Related files */}
          {task.relatedFiles && task.relatedFiles.length > 0 && (
            <div>
              <label className={`text-[10px] mb-1 block ${t.text.muted}`}><Link2 className="w-3 h-3 inline mr-1" />Related Files</label>
              <div className="flex flex-wrap gap-1">
                {task.relatedFiles.map(f => (
                  <span key={f} className={`text-[9px] px-1.5 py-0.5 rounded ${t.isDark ? 'bg-cyan-500/10 text-cyan-300' : 'bg-cyan-50 text-cyan-700'}`}>
                    <FileText className="w-2.5 h-2.5 inline mr-0.5" />{f}
                  </span>
                ))}
              </div>
            </div>
          ...
          {/* Timestamps */}
          <div className={`flex gap-4 text-[9px] pt-2 border-t ${t.isDark ? 'border-white/5 text-slate-500' : 'border-slate-100 text-slate-400'}`}>
          ...
          <div className="flex justify-end gap-2 pt-1">
            <button onClick={onClose} className={`px-3 py-1.5 rounded-lg text-[11px] ${t.isDark ? 'text-slate-400 hover:bg-white/5' : 'text-slate-500 hover:bg-slate-50'}`}>
              {i.tbCancel || 'Cancel'}
            </button>
