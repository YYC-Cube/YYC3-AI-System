...
const MOCK_ENTRIES: TimelineEntry[] = [
  {
    id: 'a1',
    type: 'edit',
    user: { name: 'You', avatar: 'Y', color: '#818cf8' },
    action: 'atEditedFile',
    target: 'IDELayout.tsx',
    detail: '+42 -8',
    time: 2,
  },
  {
    id: 'a2',
    type: 'comment',
    user: { name: 'Alice', avatar: 'A', color: '#f472b6' },
    action: 'atCommentedOn',
    target: 'store.ts:148',
    detail: 'Should we persist the new panel states?',
    time: 8,
  },
  {
    id: 'a3',
    type: 'deploy',
    user: { name: 'CI Bot', avatar: 'CI', color: '#22c55e' },
    action: 'atDeployedTo',
    target: 'atStaging',
    detail: 'Build #247 successful',
    time: 15,
  },
  {
    id: 'a4',
    type: 'review',
    user: { name: 'Bob', avatar: 'B', color: '#f97316' },
    action: 'atReviewedPR',
    target: '#42 Advanced Features',
  ...
  },
  {
    ...
    type: 'branch',
    user: { name: 'You', avatar: 'Y', color: '#818cf8' },
    action: 'atCreatedBranch',
    target: 'feature/advanced-panels',
    time: 40,
  },
  {
    id: 'a6',
  ...
  },
  {
    ...
    type: 'resolve',
    user: { name: 'You', avatar: 'Y', color: '#818cf8' },
    action: 'atResolvedIssue',
    target: '#15 Type safety',
    time: 95,
  },
  {
    id: 'a9',
    type: 'comment',
    user: { name: 'DevBot', avatar: 'D', color: '#a78bfa' },
    action: 'atCommentedOn',
    target: 'CommandPalette.tsx:67',
    detail: 'Consider adding fuzzy matching threshold config',
    time: 120,
  },
  {
    id: 'a10',
  ...
  },
  {
    ...
    type: 'reaction',
    user: { name: 'Alice', avatar: 'A', color: '#f472b6' },
    action: 'atAddedReaction',
    target: 'PR #42 comment',
    time: 200,
  },
];
...
export function ActivityTimeline() {
  ...
  const resolveActionTarget = (
    entry: TimelineEntry
  ): { actionText: string; targetText: string } => {
                    ...
                    {filtered.map((entry, idx) => {
                      ...
                      return (
                        ...
                        >
                          {/* Icon dot */}
                          <div
                            className={`relative z-10 w-[34px] h-[34px] rounded-full flex items-center justify-center flex-shrink-0 ${getActivityBg(entry.type, t.isDark)}`}
                          >
                          ...
                          <div className="flex-1 min-w-0 pt-0.5">
                            <div className={`text-[11px] ${t.text.primary}`}>
                              <span style={{ fontWeight: 600, color: entry.user.color }}>
                                {entry.user.name}
                              </span>{' '}
                              <span className={t.text.secondary}>{actionText}</span>{' '}
                              <span
                                className={`${t.accent.activeText}`}
                                ...
                                {entry.type === 'comment' ? (
                                  <>
                                    ...
                                    <div className="flex items-center gap-2 mt-1.5">
                                      <button
                                        className={`flex items-center gap-0.5 text-[9px] ${t.text.muted} hover:${t.accent.activeText} ${t.transition}`}
                                      >
                                        <Reply className="w-2.5 h-2.5" /> {i.atReply}
                                      </button>
                                      <button
                                        className={`flex items-center gap-0.5 text-[9px] ${t.text.muted} hover:text-pink-400 ${t.transition}`}
                                      >
