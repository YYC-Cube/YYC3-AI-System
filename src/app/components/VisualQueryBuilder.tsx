...
export function VisualQueryBuilder({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const selectAllColumns = (tableName: string) => {
    ...
    if (allSelected) {
      ...
      const newCols = table.columns
        .filter(
          (col) => !selectedColumns.some((c) => c.table === tableName && c.column === col.name)
        )
  ...
  const generateSQL = useCallback((): string => {
    if (selectedColumns.length === 0 && selectedTables.length === 0)
      return '-- Select tables and columns to build query';

    const cols =
      selectedColumns.length > 0
        ? selectedColumns
            .map((c) => `${c.table}.${c.column}${c.alias ? ` AS ${c.alias}` : ''}`)
            .join(',\n  ')
  ...
  const renderPlanNode = (node: QueryPlanNode, depth: number = 0): React.ReactNode => (
    <div key={node.id} style={{ marginLeft: `${depth * 24}px` }} className="mb-1">
      <div
        className={`flex items-center space-x-2 p-2 rounded-lg text-[11px] ${t.isDark ? 'bg-slate-800/30' : 'bg-slate-50'}`}
      >
        <div
          className={`w-2 h-2 rounded-full ${node.cost > 100 ? 'bg-red-400' : node.cost > 50 ? 'bg-amber-400' : 'bg-emerald-400'}`}
        />
        <span style={{ fontWeight: 500 }}>{node.type}</span>
        {node.table && (
          <span className={`px-1.5 py-0.5 rounded text-[9px] bg-indigo-500/20 text-indigo-400`}>
            {node.table}
          </span>
        )}
        <ArrowRight className="w-3 h-3" />
        <span className={t.text.muted}>
          {i.vqCost}: {node.cost.toFixed(1)}
        </span>
        <span className={t.text.muted}>
          {i.vqRows}: {node.rows}
        </span>
        ...
        >
          ...
          >
            ...
            <div className="flex items-center space-x-2">
              <button
                ...
                className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-lg text-[11px] ${t.interactive.menuItem}`}
              >
          ...
          <div className="flex-1 overflow-hidden flex">
            {activeTab === 'builder' && (
              <>
                ...
                <div
                  ...
                  className={`w-56 flex-shrink-0 border-r ${t.border.subtle} overflow-y-auto p-3 space-y-2`}
                >
                  <h3
                  ...
                  {tables.map((table) => (
                    <div key={table.name}>
                      ...
                      >
                        ...
                        <span className="flex-1" style={{ fontWeight: table.selected ? 500 : 400 }}>
                          {table.name}
                        </span>
                        ...
                        <div className="ml-5 mt-1 space-y-0.5">
                          ...
                          {table.columns.map((col) => (
                            <div
                              ...
                              onClick={() => {
                                if (table.selected) toggleColumn(table.name, col.name);
                              }}
                            >
                              {col.primaryKey && (
                              ...
                              {col.foreignKey && (
                                <Link2 className="w-2.5 h-2.5 text-cyan-400 flex-shrink-0" />
                              )}
                              {!col.primaryKey && !col.foreignKey && (
                                <Columns3 className="w-2.5 h-2.5 flex-shrink-0 opacity-30" />
                              )}
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <h3
                        className={`text-[11px] uppercase tracking-wider ${t.text.muted}`}
                        style={{ fontWeight: 600 }}
                      >
                        <Columns3 className="w-3.5 h-3.5 inline mr-1" />
                        {i.vqColumns} ({selectedColumns.length})
                      </h3>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {selectedColumns.map((col) => (
                        <span
                          ...
                          className={`flex items-center space-x-1 px-2 py-1 rounded-lg text-[10px] ${t.isDark ? 'bg-indigo-900/20 text-indigo-300' : 'bg-indigo-50 text-indigo-700'}`}
                        >
                          <span style={{ fontWeight: 500 }}>
                            {col.table}.{col.column}
                          </span>
                          <button
                            onClick={() => toggleColumn(col.table, col.column)}
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      >
                        ...
                        {i.vqJoins} ({joins.length})
                      </h3>
                      <button
                        onClick={addJoin}
                        disabled={selectedTables.length < 2}
                        className={`flex items-center space-x-1 px-2 py-1 rounded-lg text-[10px] ${t.interactive.menuItem} ${selectedTables.length < 2 ? 'opacity-30' : ''}`}
                      >
                    ...
                    <div className="space-y-2">
                      {joins.map((join) => (
                        <div
                          key={join.id}
                          className={`flex items-center space-x-2 p-2 rounded-xl text-[11px] ${t.isDark ? 'bg-slate-800/30' : 'bg-slate-50'}`}
                        >
                          <select
                            value={join.type}
                            onChange={(e) =>
                              setJoins((prev) =>
                                prev.map((j) =>
                                  j.id === join.id ? { ...j, type: e.target.value as JoinType } : j
                                )
                              )
                            }
                          ...
                          >
                            {(['INNER', 'LEFT', 'RIGHT', 'FULL', 'CROSS'] as JoinType[]).map(
                              (jt) => (
                          ...
                          <span
                            className={`px-1.5 py-0.5 rounded text-[9px] bg-indigo-500/20 text-indigo-400`}
                          >
                            {join.leftTable}.{join.leftColumn}
                          </span>
                          <ArrowLeftRight className="w-3 h-3 flex-shrink-0" />
                          <span
                            className={`px-1.5 py-0.5 rounded text-[9px] bg-cyan-500/20 text-cyan-400`}
                          >
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      >
                        ...
                        {i.vqWhere} ({wheres.length})
                      </h3>
                      <button
                        onClick={addWhere}
                        disabled={selectedColumns.length === 0}
                        className={`flex items-center space-x-1 px-2 py-1 rounded-lg text-[10px] ${t.interactive.menuItem} ${selectedColumns.length === 0 ? 'opacity-30' : ''}`}
                      >
                        <Plus className="w-3 h-3" />
                        <span>{i.vqAddFilter}</span>
                      </button>
                    </div>
                    <div className="space-y-1.5">
                      {wheres.map((w, idx) => (
                        <div
                          key={w.id}
                          className={`flex items-center space-x-1.5 p-2 rounded-lg text-[11px] ${t.isDark ? 'bg-slate-800/30' : 'bg-slate-50'}`}
                        >
                          {idx > 0 && (
                            <select
                              value={w.logic}
                              onChange={(e) =>
                                setWheres((prev) =>
                                  prev.map((x) =>
                                    x.id === w.id
                                      ? { ...x, logic: e.target.value as 'AND' | 'OR' }
                                      : x
                                  )
                                )
                              }
                          ...
                          <select
                            ...
                            onChange={(e) => {
                              ...
                              setWheres((prev) =>
                                prev.map((x) => (x.id === w.id ? { ...x, table, column } : x))
                              );
                            }}
                            className={`px-1 py-0.5 rounded text-[9px] ${t.isDark ? 'bg-slate-700 text-slate-300' : 'bg-white text-slate-700'} border ${t.border.subtle}`}
                          >
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              setWheres((prev) =>
                                prev.map((x) =>
                                  x.id === w.id ? { ...x, operator: e.target.value as FilterOp } : x
                                )
                              )
                            }
                            className={`px-1 py-0.5 rounded text-[9px] ${t.isDark ? 'bg-slate-700 text-slate-300' : 'bg-white text-slate-700'} border ${t.border.subtle}`}
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div>
                      <div className="flex items-center justify-between mb-1.5">
                        <h3
                          className={`text-[10px] uppercase tracking-wider ${t.text.muted}`}
                          style={{ fontWeight: 600 }}
                        >
                        ...
                        <button
                          onClick={addOrder}
                          disabled={selectedColumns.length === 0}
                          className={`p-0.5 rounded ${t.interactive.iconBtn} ${selectedColumns.length === 0 ? 'opacity-30' : ''}`}
                        >
                          <Plus className="w-3 h-3" />
                        </button>
                      </div>
                      {orders.map((o) => (
                        <div
                          key={o.id}
                          className={`flex items-center space-x-1 mb-1 text-[10px] ${t.isDark ? 'bg-slate-800/20' : 'bg-slate-50'} rounded p-1`}
                        >
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              setOrders((prev) =>
                                prev.map((x) =>
                                  x.id === o.id ? { ...x, direction: e.target.value as SortDir } : x
                                )
                              )
                            }
                          ...
                          <button onClick={() => removeOrder(o.id)} className="text-red-400">
                            <X className="w-2.5 h-2.5" />
                          </button>
                        </div>
                      ))}
                    ...
                    <div>
                      ...
                      {groups.map((g) => (
                        ...
                        >
                          ...
                          <button onClick={() => removeGroup(g.id)} className="text-red-400">
                            <X className="w-2.5 h-2.5" />
                          </button>
                        </div>
                      ))}
                    ...
                    <div>
                      <h3
                        className={`text-[10px] uppercase tracking-wider mb-1.5 ${t.text.muted}`}
                        style={{ fontWeight: 600 }}
                      >
            ...
            {activeTab === 'sql' && (
              <div className="flex-1 overflow-y-auto p-4">
                <pre
                  ...
                  className={`p-4 rounded-xl text-[12px] font-mono whitespace-pre-wrap ${t.isDark ? 'bg-slate-900/50 text-emerald-300' : 'bg-slate-50 text-emerald-700'}`}
            ...
            {activeTab === 'plan' && (
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                <div
                  className={`flex items-center space-x-2 p-3 rounded-xl text-[11px] ${t.isDark ? 'bg-indigo-900/10' : 'bg-indigo-50'}`}
                >
                ...
                <div className={`grid grid-cols-3 gap-3 mt-4`}>
                  <div
                    className={`p-3 rounded-xl text-center ${t.isDark ? 'bg-slate-800/30' : 'bg-slate-50'}`}
                  >
                    <p className={`text-[10px] ${t.text.muted}`}>{i.vqTotalCost}</p>
                    <p className="text-[16px] text-indigo-400" style={{ fontWeight: 700 }}>
                      245.5
                    </p>
                  </div>
                  <div
                  ...
                  >
                    ...
                    <p className="text-[16px] text-cyan-400" style={{ fontWeight: 700 }}>
                      150
                    </p>
                  </div>
                  <div
                  ...
                  >
                    ...
                    <p className="text-[16px] text-emerald-400" style={{ fontWeight: 700 }}>
                      2.4ms
                    </p>
                  </div>
                </div>
                ...
                {showResults ? (
                  <div>
                    ...
                    <div className="overflow-x-auto">
                      <table
                        className={`w-full text-[11px] ${t.isDark ? 'text-slate-300' : 'text-slate-700'}`}
                      >
                        <thead>
                          <tr className={`border-b ${t.border.subtle}`}>
                            {['id', 'name', 'email', 'role', 'created_at'].map((col) => (
                              <th
                                key={col}
                                className={`px-3 py-2 text-left text-[10px] uppercase ${t.text.muted}`}
                                style={{ fontWeight: 600 }}
                              >
                                {col}
                              </th>
                        ...
                        <tbody>
                          {[
                            ...
                            <tr
                              key={idx}
                              className={`border-b ${t.border.subtle} ${t.isDark ? 'hover:bg-slate-800/30' : 'hover:bg-slate-50'}`}
                            >
                              {row.map((cell, cidx) => (
