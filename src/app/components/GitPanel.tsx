...
const MOCK_COMMITS: Commit[] = [
  {
    id: 'c1',
    hash: 'a3f21b9',
    message: 'feat: add Command Palette and Search Panel',
    author: 'You',
    time: '2h ago',
    branch: 'main',
    avatar: 'Y',
  },
  {
    id: 'c2',
    hash: 'e8d4c12',
    message: 'fix: resolve i18n key sync issues',
    author: 'You',
    time: '4h ago',
    branch: 'main',
    avatar: 'Y',
  },
  {
    id: 'c3',
    hash: 'b7a6e03',
    message: 'feat: implement Notification Center',
    author: 'DevBot',
    time: '6h ago',
    branch: 'main',
    avatar: 'D',
  },
  {
    id: 'c4',
    hash: '9c5f8d1',
    message: 'refactor: migrate (i as any) to resolveKey',
    author: 'You',
    time: '1d ago',
    branch: 'main',
    avatar: 'Y',
  },
  {
    id: 'c5',
    hash: '2d1e4a7',
    message: 'feat: theme customizer with presets',
    author: 'Alice',
    time: '2d ago',
          ...
          >
            ...
            >
              <div className="flex items-center gap-2.5">
                <div
                  className={`p-1.5 rounded-lg ${t.isDark ? 'bg-orange-500/15' : 'bg-orange-50'}`}
                >
                  <GitBranch
                    className={`w-5 h-5 ${t.isDark ? 'text-orange-400' : 'text-orange-600'}`}
                  />
                </div>
                <div>
                  ...
                  <div className="flex items-center gap-1.5 mt-0.5">
                    ...
                    {MOCK_BRANCHES[0].ahead > 0 && (
                      <span
                        className={`text-[9px] px-1 rounded ${t.isDark ? 'bg-emerald-500/20 text-emerald-400' : 'bg-emerald-50 text-emerald-600'}`}
                      >
                        <ArrowUpRight className="w-2.5 h-2.5 inline" /> {MOCK_BRANCHES[0].ahead}{' '}
                        {ig.gpAhead}
                      </span>
                    )}
                  </div>
              ...
              <div className="flex items-center gap-1">
                <button
                  onClick={() => toast.success(ig.gpFetch)}
            ...
            <div className={`flex items-center gap-1.5 px-4 py-2 border-b ${t.border.subtle}`}>
              <button
                onClick={() => toast.success(ig.gpPull)}
                className={`flex items-center gap-1 px-2.5 py-1 rounded-md text-[10px] ${t.interactive.iconBtn} ${t.transition}`}
                style={{ fontWeight: 500 }}
              >
                <Download className="w-3 h-3" /> {ig.gpPull}
              </button>
              <button
                onClick={() => toast.success(ig.gpPush)}
                className={`flex items-center gap-1 px-2.5 py-1 rounded-md text-[10px] ${t.interactive.iconBtn} ${t.transition}`}
                style={{ fontWeight: 500 }}
              >
              ...
              <button
                onClick={() => toast.success(ig.gpStash)}
                className={`flex items-center gap-1 px-2.5 py-1 rounded-md text-[10px] ${t.interactive.iconBtn} ${t.transition}`}
                style={{ fontWeight: 500 }}
              >
              ...
              {tabs.map((tab) => {
                ...
                return (
                  ...
                  >
                    ...
                    {tab.count !== undefined && (
                      <span
                        className={`px-1.5 py-px rounded-full text-[8px] ${isActive ? 'opacity-80' : 'opacity-50'}`}
            ...
            <div className={`flex-1 overflow-y-auto ${t.scrollbar}`}>
                  ...
                  <div className="flex-1 overflow-y-auto px-3 py-2 space-y-1">
                    ...
                    <div>
                      <button
                        ...
                        className={`flex items-center justify-between w-full py-1.5 text-[10px] ${t.text.muted}`}
                        style={{ fontWeight: 600 }}
                      >
                        <div className="flex items-center gap-1">
                        ...
                        {expandedSections.has('staged') &&
                          staged.map((f) => {
                            ...
                            return (
                              ...
                              >
                                <Icon
                                  className={`w-3.5 h-3.5 ${getStatusColor(f.status, t.isDark)}`}
                                />
                                <span className={`flex-1 text-[11px] ${t.text.secondary} truncate`}>
                                  {f.name}
                                </span>
                                <span
                                  className={`text-[9px] ${t.isDark ? 'text-emerald-400' : 'text-emerald-600'}`}
                                >
                                  +{f.additions}
                                </span>
                                {f.deletions > 0 && (
                                  <span
                                    className={`text-[9px] ${t.isDark ? 'text-red-400' : 'text-red-500'}`}
                                  >
                                    -{f.deletions}
                                  </span>
                                )}
                                <button
                                  onClick={() => useAppStore.getState().setGitDiffFile(f.name)}
                                  className={`opacity-0 group-hover:opacity-100 p-0.5 rounded ${t.interactive.iconBtn} ${t.transition}`}
                                  title={ig.gdViewDiff}
                                >
                                  <GitCompare className="w-3 h-3" />
                                </button>
                                <button
                                  onClick={() => unstageFile(f.id)}
                                  className={`opacity-0 group-hover:opacity-100 p-0.5 rounded ${t.interactive.iconBtn} ${t.transition}`}
                                >
                      ...
                      >
                        ...
                        {unstaged.length > 0 && (
                          <button
                            ...
                            className={`text-[9px] ${t.accent.activeText} hover:underline`}
                          >
                            {ig.gpStageAll}
                          </button>
                        )}
                          ...
                          unstaged.map((f) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <span
                                  className={`text-[9px] px-1 rounded ${t.isDark ? 'bg-white/5' : 'bg-slate-100'} ${getStatusColor(f.status, t.isDark)}`}
                                  style={{ fontWeight: 600 }}
                                >
              ...
              {activeTab === 'commits' && (
                <div className="px-3 py-2 space-y-0.5">
                  {MOCK_COMMITS.map((c, idx) => (
                    <div
                      key={c.id}
                      className={`flex items-start gap-3 px-3 py-2.5 rounded-lg ${t.interactive.hoverBg} ${t.transition}`}
                    >
                      {/* Timeline connector */}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className={`flex items-center gap-2 mt-1 text-[9px] ${t.text.dimmed}`}>
                          <code
                            className={`px-1 py-px rounded ${t.isDark ? 'bg-white/5' : 'bg-slate-100'}`}
                          >
                            {c.hash}
                          </code>
                          <span>{c.author}</span>
              ...
              {activeTab === 'branches' && (
                <div className="px-3 py-2 space-y-1">
                  <button
                    className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg border border-dashed ${t.border.medium} ${t.interactive.hoverBg} ${t.transition} text-[11px] ${t.text.muted}`}
                    style={{ fontWeight: 500 }}
                  >
                  ...
                  {MOCK_BRANCHES.map((b) => (
                    ...
                    >
                      <GitBranch
                        className={`w-4 h-4 ${b.current ? t.accent.activeText : t.text.muted}`}
                      />
                      <div className="flex-1 min-w-0">
                        <div
                          className={`text-[11px] ${b.current ? t.accent.activeText : t.text.primary}`}
                          style={{ fontWeight: b.current ? 600 : 400 }}
                        >
                          {b.name}
                          {b.current && (
                            <span
                              className={`ml-1.5 text-[8px] px-1.5 py-px rounded-full ${t.isDark ? 'bg-indigo-500/20' : 'bg-indigo-100'}`}
                              style={{ fontWeight: 700 }}
                            >
                              HEAD
                            </span>
                        ...
                        <div
                          className={`flex items-center gap-2 mt-0.5 text-[9px] ${t.text.dimmed}`}
                        >
                          <span>{b.lastCommit}</span>
                          {b.ahead > 0 && (
                            <span
                              className={`${t.isDark ? 'text-emerald-400' : 'text-emerald-600'}`}
                            >
                      ...
                      {!b.current && (
                        <button
                          className={`p-1 rounded-md text-[9px] ${t.interactive.iconBtn} ${t.transition}`}
                          title={ig.gpSwitchBranch}
                        >
