...
export function SnippetManager({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const filteredSnippets = useMemo(() => {
    ...
    if (tagFilter)
      result = result.filter((s) => s.tags.includes(tagFilter) || s.aiTags.includes(tagFilter));
    return result;
  ...
  const insertCode = (snippet: Snippet) => {
    setSnippets((prev) =>
      prev.map((s) =>
        s.id === snippet.id ? { ...s, usageCount: s.usageCount + 1, lastUsed: Date.now() } : s
      )
    );
  ...
  const handleAiTag = (snippet: Snippet) => {
    ...
    setTimeout(() => {
      ...
      setSnippets((prev) =>
        prev.map((s) => (s.id === snippet.id ? { ...s, aiTags: possibleTags } : s))
      );
      setAiTagging(false);
          ...
          <div className={`w-80 border-r ${t.border.subtle} flex flex-col flex-shrink-0`}>
            ...
            <div className={`p-3 border-b ${t.border.subtle}`}>
              <div className="relative">
                <Search
                  className={`absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 ${t.text.dimmed}`}
                />
                <input
                  value={searchQuery}
                ...
                <button
                  ...
                  className={`flex-1 flex items-center justify-center space-x-1 py-2 text-[10px] ${t.transition} border-b-2 ${
                    activeTab === tab.id
                      ...
                      : `${t.text.muted} border-transparent hover:border-slate-400/30`
                  }`}
            ...
            <div className={`flex items-center gap-1.5 p-2 border-b ${t.border.subtle} flex-wrap`}>
              {langFilter && (
                <button
                  ...
                  className="flex items-center space-x-1 px-2 py-0.5 rounded-full bg-indigo-500/20 text-indigo-400 text-[10px]"
                >
                  <span>{langFilter}</span>
              ...
              {tagFilter && (
                <button
                  ...
                  className="flex items-center space-x-1 px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-400 text-[10px]"
                >
                  <span>{tagFilter}</span>
                  <X className="w-2.5 h-2.5" />
                </button>
              ...
              {!langFilter && !tagFilter && (
                <span className={`text-[10px] ${t.text.dimmed}`}>
                  {filteredSnippets.length} {ii.snAll.toLowerCase()}
                </span>
              )}
            </div>
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <div className="flex items-center space-x-1.5 flex-shrink-0">
                        ...
                        <span
                          className={`text-[9px] px-1.5 py-0.5 rounded ${t.isDark ? 'bg-slate-700/60' : 'bg-slate-200/60'}`}
                        >
                          {s.language}
                        </span>
                      </div>
                    ...
                    <div className="flex flex-wrap gap-1 mt-1.5">
                      {s.tags.map((tag) => (
                        <span
                          key={tag}
                          className="text-[9px] px-1.5 py-0.5 rounded-full bg-indigo-500/15 text-indigo-400"
                        >
                          {tag}
                        </span>
                      ))}
                      {s.aiTags.map((tag) => (
                        <span
                          key={tag}
                          className="text-[9px] px-1.5 py-0.5 rounded-full bg-purple-500/15 text-purple-400"
                        >
            ...
            {showAddForm ? (
              ...
              <div className="flex-1 p-5 overflow-y-auto space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label
                      className={`text-[10px] uppercase tracking-wider ${t.text.muted} mb-1 block`}
                ...
                <div className="flex space-x-2">
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg text-[12px] ${t.accent.activeBg} ${t.accent.activeText}`}
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg text-[12px] ${t.interactive.iconBtn}`}
                  >
                    {ii.msCancel}
                  </button>
                </div>
              ...
              <div className="flex-1 flex flex-col overflow-hidden">
                {/* Snippet header */}
                <div
                  className={`flex items-center justify-between px-5 py-3 border-b ${t.border.subtle}`}
                >
                  <div>
                    <div className="text-[14px]" style={{ fontWeight: 600 }}>
                  ...
                  <div className="flex items-center space-x-2">
                    <button
                      onClick={() => handleAiTag(selectedSnippet)}
                      className={`px-2.5 py-1 rounded-lg text-[10px] flex items-center space-x-1 ${t.transition} ${t.interactive.iconBtn}`}
                    >
                      <Sparkles
                        className={`w-3 h-3 ${aiTagging ? 'animate-pulse text-purple-400' : ''}`}
                      />
                    ...
                    >
                      <Heart
                        className={`w-4 h-4 ${selectedSnippet.isFavorite ? 'fill-red-400 text-red-400' : ''}`}
                      />
                    </button>
                    <button
                    ...
                    <button
                      onClick={() => insertCode(selectedSnippet)}
                      className={`px-3 py-1.5 rounded-lg text-[11px] flex items-center space-x-1 ${t.accent.activeBg} ${t.accent.activeText}`}
                    >
                    ...
                    <button
                      ...
                      className={`p-1.5 rounded-lg ${t.transition} hover:bg-red-500/15`}
                    >
                ...
                {/* Meta bar */}
                <div
                  className={`flex items-center space-x-4 px-5 py-2 border-b ${t.border.subtle}`}
                >
                  <span
                    className={`text-[10px] px-2 py-0.5 rounded ${t.isDark ? 'bg-slate-700/60' : 'bg-slate-200/60'}`}
                  >
                    {selectedSnippet.language}
                  </span>
                  <span className={`text-[10px] ${t.text.dimmed}`}>
                    {ii.snUsageCount}: {selectedSnippet.usageCount}
                  </span>
                  <span className={`text-[10px] ${t.text.dimmed}`}>
                    {ii.snLastUsed}: {timeAgo(selectedSnippet.lastUsed)}
                  </span>
                  <span className={`text-[10px] ${t.text.dimmed}`}>
                    {ii.snCreated}: {timeAgo(selectedSnippet.createdAt)}
                  </span>
                </div>

                {/* Tags */}
                <div
                  className={`flex items-center flex-wrap gap-1.5 px-5 py-2 border-b ${t.border.subtle}`}
                >
                ...
                <div className={`flex-1 overflow-auto p-5 ${t.scrollbar}`}>
                  <pre
                    className={`p-4 rounded-xl text-[12px] leading-relaxed font-mono overflow-x-auto ${
