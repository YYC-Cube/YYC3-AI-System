...
import {
  X, Wifi, WifiOff, HardDrive, RefreshCw, Trash2,
  FileCode, Image, Type, Database, Shield,
  CheckCircle, Download, ToggleLeft, ToggleRight,
  Loader2
} from 'lucide-react'
...
export function OfflineCache({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const [categories, setCategories] = useState<CacheCategory[]>([
    { id: 'assets', name: '', icon: FileCode, strategy: 'cache-first', enabled: true, itemCount: 142, size: 3_200_000 },
    { id: 'api', name: '', icon: Database, strategy: 'network-first', enabled: true, itemCount: 38, size: 450_000 },
    { id: 'fonts', name: '', icon: Type, strategy: 'cache-first', enabled: true, itemCount: 8, size: 820_000 },
    { id: 'images', name: '', icon: Image, strategy: 'stale-while-revalidate', enabled: true, itemCount: 56, size: 5_600_000 },
  ])

  // Resolve names based on i18n
  const getCatName = (id: string) => {
    switch (id) {
      case 'assets': return i.swAssets
      case 'api': return i.swApi
      case 'fonts': return i.swFonts
      case 'images': return i.swImages
      default: return id
    }
  }
  ...
  return (
    <>
      ...
      <div className={`fixed inset-0 z-[61] flex items-center justify-center p-8`}>
        <div className={`w-full max-w-3xl max-h-[85vh] rounded-2xl overflow-hidden flex flex-col ${t.surface.popover} ${t.border.popover} shadow-2xl`}>
          {/* Header */}
          <div className={`flex items-center justify-between px-5 py-3 border-b ${t.border.subtle} flex-shrink-0`}>
            <div className="flex items-center space-x-2.5">
              ...
              <div>
                <span className="text-[14px]" style={{ fontWeight: 600 }}>{i.swTitle}</span>
                <span className={`ml-2 text-[11px] ${t.text.muted}`}>{i.swSubtitle}</span>
              </div>
            </div>
            <div className="flex items-center space-x-2">
              {/* Online/Offline indicator */}
              <div className={`flex items-center space-x-1.5 px-2.5 py-1 rounded-full text-[10px] ${
          ...
          <div className={`flex-1 overflow-y-auto p-5 space-y-5 ${t.scrollbar}`}>
            ...
            <div className="grid grid-cols-4 gap-3">
              <div className={`p-3 rounded-xl ${t.isDark ? 'bg-slate-800/40' : 'bg-slate-50'}`}>
                ...
                <div className="flex items-center space-x-1.5 mt-1">
                  {swEnabled ? <CheckCircle className="w-4 h-4 text-emerald-400" /> : <X className="w-4 h-4 text-slate-400" />}
                  <span className="text-[13px]" style={{ fontWeight: 600 }}>{swEnabled ? i.swEnabled : i.swDisabled}</span>
                </div>
              </div>
              <div className={`p-3 rounded-xl ${t.isDark ? 'bg-slate-800/40' : 'bg-slate-50'}`}>
                <div className={`text-[10px] ${t.text.dimmed}`}>{i.swCacheSize}</div>
                <div className="text-[13px] mt-1" style={{ fontWeight: 600 }}>{formatSize(totalCacheSize)}</div>
              </div>
              <div className={`p-3 rounded-xl ${t.isDark ? 'bg-slate-800/40' : 'bg-slate-50'}`}>
                <div className={`text-[10px] ${t.text.dimmed}`}>{i.swCached}</div>
                <div className="text-[13px] mt-1" style={{ fontWeight: 600 }}>{totalItems} items</div>
              </div>
              <div className={`p-3 rounded-xl ${t.isDark ? 'bg-slate-800/40' : 'bg-slate-50'}`}>
                <div className={`text-[10px] ${t.text.dimmed}`}>{i.swLastSync}</div>
                <div className="text-[13px] mt-1" style={{ fontWeight: 600 }}>{timeAgo(lastSync)} ago</div>
              </div>
            </div>

            {/* SW toggle + actions */}
            <div className={`flex items-center justify-between p-4 rounded-xl ${t.isDark ? 'bg-slate-800/40' : 'bg-slate-50'}`}>
              <div className="flex items-center space-x-3">
                <Shield className={`w-5 h-5 ${swEnabled ? 'text-indigo-400' : t.text.dimmed}`} />
                <div>
                  <div className="text-[12px]" style={{ fontWeight: 500 }}>Service Worker</div>
              ...
              <div className="flex items-center space-x-2">
                ...
                >
                  {precaching ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Download className="w-3.5 h-3.5" />}
                ...
                <button onClick={() => setSwEnabled(!swEnabled)} className={`p-1 rounded ${t.transition}`}>
                  {swEnabled ? <ToggleRight className="w-6 h-6 text-emerald-400" /> : <ToggleLeft className="w-6 h-6 text-slate-400" />}
                </button>
              </div>
            </div>
            ...
            {/* Cache categories */}
            <div>
              <div className={`text-[11px] uppercase tracking-wider mb-3 ${t.text.muted}`} style={{ fontWeight: 600 }}>{i.swStrategy}</div>
              <div className="space-y-2">
                {categories.map(cat => {
                  const Icon = cat.icon
                  return (
                    <div key={cat.id} className={`p-4 rounded-xl ${t.isDark ? 'bg-slate-800/40' : 'bg-slate-50'}`}>
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center space-x-2.5">
                          <Icon className={`w-4 h-4 ${cat.enabled ? 'text-indigo-400' : t.text.dimmed}`} />
                          <span className="text-[12px]" style={{ fontWeight: 500 }}>{getCatName(cat.id)}</span>
                        </div>
                        <div className="flex items-center space-x-3">
                          <span className={`text-[10px] ${t.text.dimmed}`}>{cat.itemCount} items · {formatSize(cat.size)}</span>
                          <button onClick={() => toggleCategory(cat.id)} className={`p-0.5 rounded ${t.transition}`}>
                            {cat.enabled ? <ToggleRight className="w-5 h-5 text-emerald-400" /> : <ToggleLeft className="w-5 h-5 text-slate-400" />}
                          </button>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center space-x-1.5">
                        {([
                          ...
                          { id: 'stale-while-revalidate' as CacheStrategy, label: i.swStaleRevalidate },
                        ]).map(s => (
                          <button
                            key={s.id}
                            onClick={() => updateStrategy(cat.id, s.id)}
                            className={`px-2.5 py-1 rounded-lg text-[10px] ${t.transition} ${
                              cat.strategy === s.id ? `${t.accent.activeBg} ${t.accent.activeText}` : t.interactive.iconBtn
                            }`}
                      ...
                      {/* Progress bar */}
                      <div className={`mt-2.5 h-1.5 rounded-full overflow-hidden ${t.isDark ? 'bg-slate-700/40' : 'bg-slate-200'}`}>
                        <div
            ...
            <div>
              <div className={`text-[11px] uppercase tracking-wider mb-2 ${t.text.muted}`} style={{ fontWeight: 600 }}>Workbox Config</div>
              <pre className={`p-4 rounded-xl text-[11px] font-mono overflow-x-auto ${t.isDark ? 'bg-slate-900/60 text-slate-300' : 'bg-slate-100 text-slate-700'}`}>{`// sw.js (Workbox Runtime Caching)
