...
const mockQueryOptimizer = {
  ...
  listIndexes: vi.fn(() => Promise.resolve([] as Array<{ name: string; table: string; columns: string[] }>)),
  analyzeIndexUsage: vi.fn((_name: string) => Promise.resolve({ recommendations: [] as Array<{ type: string; reason: string }> })),
  ...
  // Query Analyzer
  analyze: vi.fn((_query: string) => Promise.resolve({ executionTime: 0, recommendations: [] as Array<{ type: string; severity?: string; message?: string }> })),
  explain: vi.fn((_query: string) => Promise.resolve({ plan: { nodeType: 'Seq Scan', cost: { startup: 0, total: 0 }, rows: 0 } })),
  analyzePlan: vi.fn((_plan: unknown) => Promise.resolve({ cost: 0, recommendations: [] as Array<{ type: string; severity?: string; message?: string }> })),
  detectFullTableScan: vi.fn((_plan: unknown) => false),
  detectMissingIndexes: vi.fn((_query: string) => [] as Array<{ table: string; columns: string[] }>),
  
  // Slow Query Monitor
  monitor: vi.fn((_query: string, _duration: number) => {}),
  getSlowQueries: vi.fn(() => Promise.resolve([] as Array<{ id: string; sql: string; duration: number }>)),
...
describe('Query Optimizer Service - Full Coverage', () => {
  ...
  describe('Query Analyzer', () => {
    describe('analyze', () => {
      ...
      it('should provide optimization recommendations', async () => {
        mockQueryOptimizer.analyze.mockResolvedValueOnce({
          executionTime: 150,
          recommendations: [
            { type: 'index', severity: 'warning', message: 'Add index on email' },
          ],
        })
    ...
    describe('detectMissingIndexes', () => {
      it('should detect missing indexes', () => {
        const missing = mockQueryOptimizer.detectMissingIndexes('SELECT * FROM users WHERE email = ?')
        expect(Array.isArray(missing)).toBe(true)
      })

      it('should suggest index columns', () => {
        mockQueryOptimizer.detectMissingIndexes.mockReturnValueOnce([{ table: 'users', columns: ['email'] }])
        const missing = mockQueryOptimizer.detectMissingIndexes('SELECT * FROM users WHERE email = ?')
  ...
  describe('Slow Query Monitor', () => {
    ...
    describe('logSlowQuery', () => {
      ...
      it('should include query details', () => {
        ...
        expect(mockQueryOptimizer.logSlowQuery).toHaveBeenCalledWith('SELECT * FROM users', 5000, 'conn-1')
      })
    })
  })
  ...
  describe('Query Optimization Recommendations', () => {
    it('should recommend adding index', async () => {
      mockQueryOptimizer.analyze.mockResolvedValueOnce({
        executionTime: 200,
        recommendations: [
          { type: 'index', severity: 'warning', message: 'Add index' },
        ],
      })
      const result = await mockQueryOptimizer.analyze('SELECT * FROM users WHERE email = ?')
    ...
    it('should recommend query rewrite', async () => {
      ...
      const result = await mockQueryOptimizer.analyze('SELECT * FROM orders WHERE user_id IN (SELECT id FROM users)')
  ...
  describe('Error Handling', () => {
    ...
    it('should handle connection errors', async () => {
      ...
      await expect(mockQueryOptimizer.explain('SELECT * FROM users')).rejects.toThrow('Connection failed')
    })
