...
const mockAiCompletion = {
  registerCompletionProvider: vi.fn((_context: unknown) => ({ dispose: vi.fn(), setEnabled: vi.fn() })),
  provideInlineCompletions: vi.fn((_document: unknown, _position: unknown, _context: unknown) => Promise.resolve({ items: [] })),
...
describe('AI Completion Service - High Priority Coverage', () => {
  ...
  describe('Inline Completions', () => {
    it('should provide inline completions', async () => {
      ...
      const completions = await mockAiCompletion.provideInlineCompletions({} as unknown, {} as unknown, {} as unknown)
  ...
  describe('Error Handling', () => {
    it('should handle invalid model', async () => {
      ...
      await expect(mockAiCompletion.provideInlineCompletions(null as unknown, {} as unknown, {} as unknown))
        .rejects.toThrow('Invalid model')
    })

    it('should handle cancellation', async () => {
      mockAiCompletion.provideInlineCompletions.mockRejectedValueOnce(new Error('Cancelled'))
      await expect(mockAiCompletion.provideInlineCompletions({} as unknown, {} as unknown, { isCancellationRequested: true }))
...
const mockPreviewEngine = {
  render: vi.fn((_html: string, _css: string, _js: string) => Promise.resolve({ html: '', css: '', js: '' })),
...
const mockCollaboration = {
  ...
  getHistory: vi.fn(() => Promise.resolve([] as Array<{ id: string; type: string; timestamp: number }>)),
}
