        ...
        >
          ...
          >
            <div className="flex items-center gap-3">
              <div
                className={`w-8 h-8 rounded-xl flex items-center justify-center ${t.isDark ? 'bg-gradient-to-br from-orange-500/20 to-red-500/20' : 'bg-gradient-to-br from-orange-50 to-red-50'}`}
              >
          ...
          <div className={`flex items-center gap-2 px-6 py-3 border-b ${t.border.subtle}`}>
            ...
            <div className="relative">
              ...
              {showMethodDrop && (
                <div
                  className={`absolute top-full left-0 mt-1 z-10 rounded-lg overflow-hidden ${t.surface.popover} ${t.border.popover} shadow-xl`}
                >
                  {METHODS.map((m) => (
                    <button
                      key={m}
                      ...
                      style={{ color: METHOD_COLORS[m], fontWeight: 700 }}
                    >
                      {m}
                    </button>
                  ))}
          ...
          <div className="flex flex-1 overflow-hidden">
              ...
              <div className={`flex items-center gap-0 px-4 border-b ${t.border.subtle}`}>
                {REQ_TABS.map((rt) => (
                  ...
                  >
                    ...
                    {rt.tab === 'headers' && (
                      <span className={`ml-1 text-[7px] ${t.text.dimmed}`}>{headers.length}</span>
                    )}
                  </button>
                ))}
              </div>
              ...
              <div className={`flex-1 overflow-y-auto p-4 ${t.scrollbar}`}>
                {reqTab === 'headers' && (
                  <div className="space-y-1">
                    {headers.map((h, idx) => (
                      <div key={idx} className="flex items-center gap-2">
                        ...
                        <input
                          ...
                          placeholder="Key"
                          className={`flex-1 font-mono text-[9px] px-2 py-1.5 rounded outline-none ${t.isDark ? 'bg-white/[0.03] text-slate-300' : 'bg-slate-50 text-slate-600'} border ${t.border.subtle}`}
                        />
                        <input
                          value={h.value}
                          onChange={(e) => {
                    ...
                    <button
                      onClick={addHeader}
                      className={`w-full flex items-center justify-center gap-1 py-1.5 rounded text-[9px] border border-dashed ${t.border.subtle} ${t.transition} ${t.interactive.menuItem}`}
                    >
            ...
            <div className="flex-1 flex flex-col">
              ...
              {response && (
                <div className={`flex items-center gap-3 px-4 py-2 border-b ${t.border.subtle}`}>
                  <span
                    ...
                    style={{ color: statusColor(response.status), fontWeight: 700 }}
                  >
                    {response.status < 300 ? (
                  ...
                  resTab === 'body' ? (
                    ...
                    <div className="p-4 space-y-1">
                      {response.headers.map((h, idx) => (
                        <div
                          key={idx}
                          className={`flex items-center gap-2 px-2 py-1.5 rounded ${t.isDark ? 'bg-white/[0.02]' : 'bg-slate-50'}`}
                        >
                          <span
                            className={`text-[9px] font-mono ${t.isDark ? 'text-cyan-400' : 'text-cyan-600'}`}
              ...
              {history.length > 0 && (
                <div
                  className={`border-t ${t.border.subtle} max-h-28 overflow-y-auto ${t.scrollbar}`}
                >
                  ...
                  {history.slice(0, 5).map((h) => (
                    ...
                    >
                      <span style={{ color: METHOD_COLORS[h.method], fontWeight: 700, width: 40 }}>
                        {h.method}
                      </span>
                      <span className={`flex-1 font-mono truncate ${t.text.muted}`}>
                        {h.url.replace(/^https?:\/\/[^/]+/, '')}
                      </span>
