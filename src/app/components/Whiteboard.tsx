...
export function Whiteboard({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    if (currentElement.type === 'brush' || currentElement.type === 'eraser') {
      ...
      setCurrentElement((prev) =>
        prev ? { ...prev, w: pos.x - (prev.x || 0), h: pos.y - (prev.y || 0) } : prev
      );
    ...
    <>
      ...
      >
        ...
        >
          ...
          <div className="flex items-center space-x-2">
            ...
            <button
              ...
              className={`px-2 py-1 rounded-lg text-[11px] ${t.transition} ${t.interactive.iconBtn}`}
              title={i.wbExportPng}
            >
        ...
        <div className="flex flex-1 overflow-hidden">
          {/* Left Toolbar */}
          <div
            className={`w-12 flex flex-col items-center py-2 space-y-1 border-r ${t.border.subtle} flex-shrink-0`}
          >
          ...
          {/* Right Properties */}
          <div
            className={`w-48 border-l ${t.border.subtle} p-3 space-y-4 flex-shrink-0 overflow-y-auto ${t.scrollbar}`}
          >
            {/* Color */}
            <div>
              <div
            ...
            <div>
              ...
              />
              <div className={`text-[10px] text-center ${t.text.dimmed}`}>
            ...
            <div className={`pt-3 border-t ${t.border.subtle}`}>
              <div className={`text-[10px] ${t.text.dimmed}`}>
                {i.dpFiles}: {elements.length}
              </div>
              <div className={`text-[10px] ${t.text.dimmed}`}>
                {i.wbUndo}: {undoStack.length} / {i.wbRedo}: {redoStack.length}
              </div>
            </div>
          </div>
        </div>
