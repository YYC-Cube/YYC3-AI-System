...
import {
  X, Brain, AlertTriangle, Zap, Shield, Lightbulb,
  TestTube, ChevronDown, ChevronRight, Wrench, Eye, RefreshCw,
  CheckCircle2, XCircle, Info, TrendingUp, Sparkles, FileCode
} from 'lucide-react'
...
const MOCK_FINDINGS: Finding[] = [
  { id: 'f1', category: 'error', severity: 'critical', line: 42, message: 'Unhandled Promise rejection in async handler', file: 'ChatInterface.tsx', fix: 'Wrap in try/catch block' },
  { id: 'f2', category: 'error', severity: 'high', line: 118, message: 'Missing null check before property access', file: 'store.ts', fix: 'Add optional chaining (?.)' },
  { id: 'f3', category: 'warning', severity: 'medium', line: 67, message: 'useEffect missing dependency: setCollaborators', file: 'IDELayout.tsx', fix: 'Add to dependency array' },
  { id: 'f4', category: 'warning', severity: 'low', line: 203, message: 'Unused import: useState', file: 'Header.tsx', fix: 'Remove unused import' },
  { id: 'f5', category: 'performance', severity: 'medium', line: 89, message: 'Large component re-renders on every state change', file: 'FileManager.tsx', fix: 'Wrap with React.memo()' },
  { id: 'f6', category: 'performance', severity: 'low', line: 156, message: 'Inline object creation in JSX causes re-renders', file: 'CodeEditor.tsx', fix: 'Extract to useMemo()' },
  { id: 'f7', category: 'security', severity: 'high', line: 34, message: 'API key exposed in client-side code', file: 'utils/api.ts', fix: 'Move to environment variables' },
  { id: 'f8', category: 'security', severity: 'medium', line: 12, message: 'dangerouslySetInnerHTML without sanitization', file: 'PreviewPanel.tsx', fix: 'Use DOMPurify.sanitize()' },
  { id: 'f9', category: 'suggestion', severity: 'info', line: 1, message: 'Consider splitting into smaller components', file: 'ModelSettings.tsx' },
  { id: 'f10', category: 'suggestion', severity: 'info', line: 45, message: 'TypeScript strict mode can catch more errors', file: 'tsconfig.json' },
  { id: 'f11', category: 'test', severity: 'low', line: 0, message: 'Missing unit tests for useAppStore hooks', file: 'store.ts' },
  { id: 'f12', category: 'test', severity: 'medium', line: 0, message: 'No integration tests for ChatInterface flow', file: 'ChatInterface.tsx' },
]
...
function getSeverityClasses(severity: Severity, isDark: boolean): string {
  const map: Record<Severity, string> = {
    critical: isDark ? 'bg-red-500/20 text-red-400 border-red-500/30' : 'bg-red-50 text-red-600 border-red-200',
    ...
    low: isDark ? 'bg-blue-500/20 text-blue-400 border-blue-500/30' : 'bg-blue-50 text-blue-600 border-blue-200',
    info: isDark ? 'bg-slate-500/20 text-slate-400 border-slate-500/30' : 'bg-slate-100 text-slate-600 border-slate-200',
  }
  return map[severity]
}
...
function ScoreRing({ score, size = 80, isDark }: { score: number; size?: number; isDark: boolean }) {
  const r = (size - 10) / 2
...
export function AiCodeIntel() {
  ...
  const score = useMemo(() => {
    ...
    const penalties: Record<Severity, number> = { critical: 15, high: 10, medium: 5, low: 2, info: 0 }
    const total = findings.filter(f => !dismissedFindings.has(f.id))
      .reduce((sum, f) => sum + penalties[f.severity], 0)
    return Math.max(0, 100 - total)
  }, [findings, dismissedFindings])

  const scoreLabel = score >= 80 ? i.aciScoreExcellent : score >= 60 ? i.aciScoreGood : score >= 40 ? i.aciScoreFair : i.aciScorePoor
          ...
          >
            ...
            <div className={`flex items-center justify-between px-5 py-4 border-b ${t.border.subtle}`}>
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h2 className={`text-[14px] ${t.text.primary}`} style={{ fontWeight: 600 }}>{i.aciTitle}</h2>
            ...
            <div className={`px-5 py-4 border-b ${t.border.subtle} flex items-center gap-5`}>
              ...
              <div className="flex-1">
                <div className={`text-[11px] ${t.text.muted} uppercase tracking-wider`} style={{ fontWeight: 600 }}>
                  {i.aciOverallScore}
                </div>
                ...
                <div className="flex items-center gap-2 mt-2 flex-wrap">
                  {counts.error > 0 && (
                    <span className={`px-1.5 py-0.5 rounded text-[9px] ${t.isDark ? 'bg-red-500/20 text-red-400' : 'bg-red-50 text-red-600'}`} style={{ fontWeight: 600 }}>
                      {counts.error} {i.aciErrors}
                    </span>
                  )}
                  {counts.warning > 0 && (
                    <span className={`px-1.5 py-0.5 rounded text-[9px] ${t.isDark ? 'bg-amber-500/20 text-amber-400' : 'bg-amber-50 text-amber-600'}`} style={{ fontWeight: 600 }}>
                      {counts.warning} {i.aciWarnings}
                    </span>
                  )}
                  {counts.security > 0 && (
                    <span className={`px-1.5 py-0.5 rounded text-[9px] ${t.isDark ? 'bg-purple-500/20 text-purple-400' : 'bg-purple-50 text-purple-600'}`} style={{ fontWeight: 600 }}>
            ...
            <div className={`flex items-center gap-1 px-4 py-2.5 border-b ${t.border.subtle} overflow-x-auto ${t.scrollbar}`}>
              {(['all', 'error', 'warning', 'performance', 'security', 'suggestion', 'test'] as const).map(cat => {
                ...
                return (
                  ...
                  >
                    {cat !== 'all' && React.createElement(CATEGORY_CONFIG[cat].icon, { className: 'w-3 h-3' })}
                    <span>{cat === 'all' ? i.atAll : categoryI18n[cat]}</span>
                    {count > 0 && (
                      <span className={`px-1 py-px rounded text-[8px] ${isActive ? 'opacity-80' : 'opacity-60'}`} style={{ fontWeight: 700 }}>
                        {count}
                      </span>
                    )}
                  </button>
                      ...
                      return (
                        <motion.div
                          ...
                          className={`rounded-xl border overflow-hidden ${t.transition} ${
                            t.isDark ? 'border-white/6 bg-white/[0.02] hover:bg-white/[0.04]' : 'border-slate-200/50 bg-white/40 hover:bg-white/60'
                          }`}
                        >
                          {/* Finding header */}
                          <button
                          ...
                          >
                            <CatIcon className={`w-4 h-4 mt-0.5 flex-shrink-0 ${getCategoryColor(f.category, t.isDark)}`} />
                            <div className="flex-1 min-w-0">
                              <div className={`text-[11px] ${t.text.primary} break-words`} style={{ fontWeight: 500 }}>
                                {f.message}
                              </div>
                              <div className={`flex items-center gap-2 mt-1`}>
                                <span className={`inline-flex items-center gap-1 px-1.5 py-px rounded text-[8px] border ${getSeverityClasses(f.severity, t.isDark)}`} style={{ fontWeight: 600 }}>
                                  {severityI18n[f.severity]}
                                </span>
                                <span className={`text-[9px] ${t.text.dimmed}`}>
                                  <FileCode className="w-2.5 h-2.5 inline mr-0.5" />{f.file}
                                </span>
                                {f.line > 0 && (
                                  <span className={`text-[9px] ${t.text.dimmed}`}>
                                    {i.aciLine} {f.line}
                            ...
                            {expanded && (
                              ...
                              >
                                <div className="px-3 py-2.5 flex items-center gap-2">
                                  {f.fix && (
                                    <button className={`flex items-center gap-1 px-2 py-1 rounded-md text-[10px] ${t.isDark ? 'bg-emerald-500/15 text-emerald-400 hover:bg-emerald-500/25' : 'bg-emerald-50 text-emerald-600 hover:bg-emerald-100'} ${t.transition}`} style={{ fontWeight: 500 }}>
                                  ...
                                  <button className={`flex items-center gap-1 px-2 py-1 rounded-md text-[10px] ${t.text.muted} ${t.interactive.hoverBg} ${t.transition}`} style={{ fontWeight: 500 }}>
                                    <Info className="w-3 h-3" />
                                    {i.aciLearnMore}
                                  </button>
                                </div>
                                {f.fix && (
                                  <div className={`mx-3 mb-2.5 px-2.5 py-2 rounded-lg text-[10px] ${t.isDark ? 'bg-emerald-500/5 text-emerald-300/80' : 'bg-emerald-50 text-emerald-700'}`}>
                                    <TrendingUp className="w-3 h-3 inline mr-1" />{f.fix}
                                  </div>
                                )}
                              </motion.div>
                            )}
            ...
            {!analyzing && counts.total > 0 && (
              <div className={`px-4 py-3 border-t ${t.border.subtle} flex items-center justify-between`}>
                <span className={`text-[10px] ${t.text.muted}`}>
                  {counts.total} {i.aciSuggestions.toLowerCase()}
                </span>
                <button className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] ${t.accent.solidBtn} ${t.transition}`} style={{ fontWeight: 600 }}>
