...
function saveHistory(
  history: Record<string, { value: string; timestamp: number; userId: string }[]>
): void {
  try {
...
function ItemCard({ item }: { item: EditableItem }) {
  ...
  const { editingId, startEdit, cancelEdit, saveEdit, removeItem, duplicateItem } =
    useEditableContent();
  ...
  return (
    ...
    >
      <div className="px-4 py-3">
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            ...
            <div className={`text-[11px] ${t.text.muted} mb-2`}>{item.description}</div>
          </div>
            ...
            {item.value && (
              <button
                onClick={() => {
                  ...
                  toast.success(i.codeCopied || 'Copied');
                }}
                className={`p-1.5 rounded-lg ${t.isDark ? 'hover:bg-white/[0.06]' : 'hover:bg-slate-100'}`}
              >
      ...
      <div
        className={`px-4 py-2 flex items-center justify-between text-[10px] ${t.text.dimmed} border-t ${t.isDark ? 'border-white/[0.04]' : 'border-slate-100'}`}
      >
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-[81] flex items-center justify-center p-6">
        ...
        >
          <div
            className={`px-6 py-4 border-b ${t.isDark ? 'border-white/10' : 'border-slate-200'}`}
          >
          ...
          <div
            className={`px-6 py-4 border-t ${t.isDark ? 'border-white/10' : 'border-slate-200'} flex justify-end gap-3`}
          >
            <button
              onClick={onClose}
...
export function EditableContentManager({ open, onClose }: EditableContentManagerProps) {
  ...
  const [history, setHistory] = useState<
    Record<string, { value: string; timestamp: number; userId: string }[]>
  >({});
  ...
  const addItem = useCallback(
    (newItem: Omit<EditableItem, 'id' | 'syncStatus' | 'lastModified' | 'version'>) => {
    ...
    <EditableContentContext.Provider value={contextValue}>
      <>
        ...
        <div className="fixed inset-0 z-[71] flex items-center justify-center p-6">
          ...
          >
            <div
              className={`flex items-center justify-between px-6 py-4 border-b ${t.isDark ? 'border-white/10' : 'border-slate-200'}`}
            >
              <div className="flex items-center gap-3">
                <div
                  className={`w-10 h-10 rounded-xl flex items-center justify-center ${t.isDark ? 'bg-gradient-to-br from-indigo-500/20 to-purple-500/20' : 'bg-gradient-to-br from-indigo-50 to-purple-50'}`}
                >
                  <Edit3
                    className={`w-5 h-5 ${t.isDark ? 'text-indigo-400' : 'text-indigo-500'}`}
                  />
                </div>
                <div>
                  <h2 className={`text-[15px] ${t.text.primary}`} style={{ fontWeight: 700 }}>
                    {i.ecTitle || 'Editable Content Manager'}
                  </h2>
                  <p className={`text-[11px] ${t.text.dimmed}`}>
                    {i.ecSubtitle || 'Manage your configurations, API keys, and secrets'} ·{' '}
                    {items.length} items
                  </p>
                </div>
              </div>
            ...
            <div
              className={`flex items-center gap-3 px-6 py-3 border-b ${t.isDark ? 'border-white/10' : 'border-slate-200'}`}
            >
              <div className="flex items-center gap-2">
                {categories.map((cat) => (
                  <button
              ...
              <div
                className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border ${t.isDark ? 'border-white/10' : 'border-slate-200'}`}
              >
            ...
            <div
              className={`flex-1 overflow-y-auto p-4 space-y-3 ${t.isDark ? 'custom-scrollbar-dark' : 'custom-scrollbar'}`}
            >
            ...
            <div
              className={`px-6 py-3 border-t ${t.isDark ? 'border-white/10' : 'border-slate-200'} flex items-center justify-between text-[10px] ${t.text.dimmed}`}
            >
              <div className="flex items-center gap-4">
                <span className="flex items-center gap-1">
                  <Lock className="w-3 h-3" /> {i.ecLocalOnly || 'All data stored locally'}
                </span>
                <span className="flex items-center gap-1">
                  <Users className="w-3 h-3" /> {i.ecCollabReady || 'Collaboration ready'}
                </span>
              </div>
