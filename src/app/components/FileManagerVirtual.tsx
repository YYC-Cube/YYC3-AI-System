...
export function FileManagerVirtual() {
  ...
  const flattenTree = useCallback((nodes: FileNode[], level = 0, path: number[] = [], startIndex = 0): FlatNodeWithIndex[] => {
    const result: FlatNodeWithIndex[] = []
    nodes.filter(n => matchesSearch(n)).forEach((node, i) => {
      const currentPath = [...path, i]
      result.push({ key: `${level}-${i}-${node.name}`, node, level, path: currentPath, index: startIndex + result.length })
      if (node.type === 'folder' && node.expanded && node.children) {
        result.push(...flattenTree(node.children, level + 1, currentPath, startIndex + result.length))
      }
    })
    return result
  }, [searchQuery])
  ...
  return (
    <div className={`flex flex-col h-full relative overflow-hidden rounded-xl ${t.surface.glass} ${t.isDark ? 'border border-white/8' : 'border border-slate-200/40'} shadow-inner`}>
      ...
      <div className={`px-3 py-1.5 border-t text-[10px] ${t.border.subtle} ${t.text.dimmed}`}>
        {selectedFile ? `${ii.selectedFile}: ${selectedFile}` : ii.noFileSelected} &middot; {ii.totalFiles.replace('{n}', String(flatNodes.length))}
      </div>
        ...
        <div className={`border-t ${t.border.subtle} max-h-[200px] overflow-y-auto`}>
          ...
          {versionLoading ? (
            <div className="flex items-center justify-center py-8">
              <div className={`w-5 h-5 border-2 ${t.border.medium} ${t.accent.primary} rounded-full animate-spin`} style={{ borderTopColor: 'transparent', borderRightColor: 'transparent' }} />
            </div>
          ) : fileVersions.length > 0 ? (
            <div className="px-3 py-2">
              {fileVersions.map((v, index) => (
                <div key={v.id} className={`flex items-center justify-between py-2 ${index < fileVersions.length - 1 ? `border-b ${t.border.subtle}` : ''}`}>
