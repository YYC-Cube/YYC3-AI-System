...
import {
  ...
  Monitor, Hammer, Users, Sparkles, ChevronDown, ChevronRight,
...
function createMockNotifications(): Notification[] {
  const now = Date.now()
  return [
    { id: 'n1', category: 'system', level: 'success', titleKey: 'notifSystemReady', descKey: 'ncSystem', timestamp: now - 60000, read: false },
    { id: 'n2', category: 'system', level: 'info', titleKey: 'notifUpdateAvailable', descKey: 'ncSystem', timestamp: now - 300000, read: false },
    { id: 'n3', category: 'build', level: 'success', titleKey: 'notifBuildComplete', descKey: 'ncBuild', timestamp: now - 600000, read: false },
    { id: 'n4', category: 'ai', level: 'success', titleKey: 'notifAiConnected', descKey: 'ncAi', timestamp: now - 1800000, read: false },
    { id: 'n5', category: 'collaboration', level: 'info', titleKey: 'notifSyncComplete', descKey: 'ncCollaboration', timestamp: now - 3600000, read: true },
    { id: 'n6', category: 'build', level: 'info', titleKey: 'notifDashboardBuild', descKey: 'ncBuild', timestamp: now - 7200000, read: true },
    { id: 'n7', category: 'system', level: 'info', titleKey: 'notifAutoSaved', descKey: 'ncSystem', timestamp: now - 86400000, read: true },
  ]
}
...
export function NotificationCenter() {
  ...
  const [expandedCategories, setExpandedCategories] = useState<Set<string>>(new Set(['today', 'earlier']))
    ...
    return (
      <div key={sectionKey} className="mb-2">
        ...
        >
          ...
          <span>{title}</span>
          <span className={`ml-auto px-1.5 py-0.5 rounded-full text-[9px] ${t.isDark ? 'bg-slate-700/50' : 'bg-slate-200'}`}>{notifs.length}</span>
        </button>
        <AnimatePresence>
          {isExpanded && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
              <div className="space-y-1 px-1">
                {notifs.map(n => (
                  <NotifItem key={n.id} notif={n} t={t} i={i} onDismiss={dismissNotif} onRead={markRead} />
                ))}
              </div>
            </motion.div>
          )}
  ...
  return (
    <>
      <div className="fixed inset-0 z-[60] bg-black/50 backdrop-blur-sm" onClick={() => setNotificationCenterOpen(false)} />
      <motion.div
        initial={{ x: '100%' }}
        animate={{ x: 0 }}
        exit={{ x: '100%' }}
      ...
      >
        ...
        <div className={`flex items-center justify-between px-5 py-4 border-b ${t.border.subtle}`}>
          <div className="flex items-center space-x-2.5">
            ...
            <span className="text-[15px]" style={{ fontWeight: 600 }}>{i.ncTitle}</span>
            {unreadCount > 0 && (
              <span className="px-1.5 py-0.5 rounded-full bg-red-500/20 text-red-400 text-[10px]" style={{ fontWeight: 700 }}>{unreadCount}</span>
            )}
          </div>
          <div className="flex items-center space-x-1">
            <button onClick={markAllRead} className={`p-1.5 rounded-lg ${t.transition} ${t.interactive.iconBtn}`} title={i.ncMarkAllRead}>
              <CheckCheck className="w-4 h-4" />
            </button>
            <button onClick={clearAll} className={`p-1.5 rounded-lg ${t.transition} ${t.interactive.iconBtn}`} title={i.ncClearAll}>
              <Trash2 className="w-4 h-4" />
            </button>
            <button onClick={() => setNotificationCenterOpen(false)} className={`p-1.5 rounded-lg ${t.transition} ${t.interactive.iconBtn}`}>
              <X className="w-4 h-4" />
            </button>
          </div>
        </div>
        ...
        <div className={`flex items-center space-x-1 px-3 py-2 border-b ${t.border.subtle} overflow-x-auto`}>
          {categories.map(cat => (
            <button
              key={cat.key}
        ...
        <div className={`flex-1 overflow-hidden p-2 ${t.scrollbar}`}>
          {filtered.length === 0 ? (
            <div className={`flex flex-col items-center justify-center h-full text-center ${t.text.muted}`}>
            ...
            <VirtualList
              ...
              focusedClassName={t.isDark ? 'bg-indigo-500/8 rounded-xl' : 'bg-indigo-50/50 rounded-xl'}
  ...
  return (
    <div
      ...
      className={`group flex items-start space-x-3 px-3 py-2.5 rounded-xl cursor-pointer ${t.transition} ${
        notif.read
          ? t.isDark ? 'bg-transparent hover:bg-slate-800/30' : 'bg-transparent hover:bg-slate-50'
          : t.isDark ? 'bg-slate-800/40 hover:bg-slate-700/40' : 'bg-indigo-50/40 hover:bg-indigo-50/60'
      }`}
    >
      {/* Level indicator */}
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between">
          <span className={`text-[12px] truncate ${notif.read ? t.text.muted : t.text.secondary}`} style={{ fontWeight: notif.read ? 400 : 500 }}>
            {titleText}
          </span>
          <span className={`text-[9px] flex-shrink-0 ml-2 ${t.text.dimmed}`}>{formatTime(notif.timestamp)}</span>
        </div>
        <div className="flex items-center space-x-1.5 mt-0.5">
          <CatIcon className={`w-3 h-3 ${t.text.dimmed}`} />
          <span className={`text-[10px] ${t.text.dimmed}`}>
            {resolveKey(i, notif.descKey)}
          </span>
        </div>
      </div>
