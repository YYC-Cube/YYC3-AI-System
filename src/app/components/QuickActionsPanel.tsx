  ...
  const filteredActions = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      actions = actions.filter((a) => {
        ...
        return (
          title.toLowerCase().includes(q) || desc.toLowerCase().includes(q) || a.type.includes(q)
        );
      });
    }
    return actions;
  ...
  const handleExecuteAction = useCallback(
    async (action: QuickAction) => {
      ...
      const context: ActionContext = {
        selection: { text },
        file: selectedFile
          ? { path: selectedFile, name: selectedFile, language: inferLanguage(selectedFile) }
          : undefined,
      };
      ...
      if (result.status === 'success') {
        if (['copy', 'copy-markdown', 'copy-html'].includes(action.type)) {
          ...
          setShowResult(true);
          toast.success(
            `${(i as unknown as Record<string, string>)[action.titleKey] || action.type} completed (${result.duration}ms)`
          );
        }
      } else {
      ...
      {open && (
        <>
          ...
          >
            {/* ── Header ── */}
            <div
              className={`flex items-center justify-between px-5 py-3.5 border-b ${t.isDark ? 'border-white/8' : 'border-slate-200/60'}`}
            >
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  <div
                    className={`w-1.5 h-1.5 rounded-full ${hasActiveModel ? 'bg-emerald-400' : 'bg-amber-400'} animate-pulse`}
                  />
            ...
            <div className={`flex items-center gap-1 px-5 pt-3 pb-2`}>
              {[
                { id: 'actions' as const, label: i.qaActionsTab || 'Actions', icon: Sparkles },
                {
                  ...
                  label: i.qaClipboardTab || 'Clipboard',
                  icon: Clipboard,
                },
              ].map((tab) => (
                <button
              ...
              {activeTab === 'actions' && (
                <div className="flex-1 flex justify-end">
                  <div className="relative">
                    <Search
                      className={`absolute left-2 top-1/2 -translate-y-1/2 w-3 h-3 ${t.text.muted}`}
                    />
                    <input
                      type="text"
                      value={searchQuery}
                ...
                <>
                  ...
                  <div className="flex items-center gap-1 mb-3">
                    ...
                    {CATEGORIES.map((cat) => (
                      ...
                      >
                        <cat.icon
                          className={`w-3 h-3 ${activeCategory === cat.id ? '' : cat.color}`}
                        />
                  ...
                  {/* Selection Input */}
                  <div
                    className={`mb-3 rounded-xl border p-3 ${t.isDark ? 'bg-slate-800/40 border-white/8' : 'bg-slate-50/80 border-slate-200/60'}`}
                  >
                    ...
                    <textarea
                      ...
                      placeholder={
                        i.qaSelectionPlaceholder ||
                        'Paste or type code/text here, or select text in the editor...'
                      }
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {filteredActions.map((action) => {
                      ...
                      const title =
                        (i as unknown as Record<string, string>)[action.titleKey] || action.type;
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-1.5">
                              <span
                                className={`text-[12px] ${t.isDark ? 'text-slate-200' : 'text-slate-800'}`}
                              ...
                              {action.requiresAI && (
                                <span
                                  className={`text-[8px] px-1 py-0.5 rounded ${t.isDark ? 'bg-purple-500/20 text-purple-300' : 'bg-purple-50 text-purple-600'}`}
                                >
                                  AI
                                </span>
                              )}
                          ...
                          {action.shortcut && (
                            <span
                              className={`text-[9px] ${t.text.dimmed} flex-shrink-0 self-center`}
                            >
                              {action.shortcut}
                            </span>
                          )}
                  ...
                  <AnimatePresence>
                    {showResult && lastResult?.content && (
                      ...
                      >
                        <div
                          className={`flex items-center justify-between px-3 py-2 border-b ${t.isDark ? 'border-white/5' : 'border-slate-100'}`}
                        >
                          <div className="flex items-center gap-2">
                        ...
                        <div className={`p-3 max-h-48 overflow-y-auto custom-scrollbar`}>
                          <pre
                            className={`text-[11px] font-mono whitespace-pre-wrap ${t.isDark ? 'text-slate-300' : 'text-slate-700'}`}
                          >
                            {lastResult.content}
                          </pre>
                          {lastResult.explanation && (
                            <div
                              className={`mt-2 pt-2 border-t text-[11px] ${t.isDark ? 'border-white/5 text-slate-400' : 'border-slate-100 text-slate-500'}`}
                            >
                              {lastResult.explanation}
                            </div>
                          )}
                  ...
                  {clipboardHistory.length === 0 ? (
                    <div className={`text-center py-12 ${t.text.muted}`}>
                      ...
                      <p className="text-[12px]">
                        {i.qaClipboardEmpty || 'Clipboard history is empty'}
                      </p>
                      <p className="text-[10px] mt-1 opacity-60">
                        {i.qaClipboardHint || 'Use Copy actions to build your history'}
                      </p>
                    </div>
                  ) : (
                        ...
                        >
                          ...
                          >
                            {item.type === 'code' ? (
                              ...
                              <FileType className="w-3 h-3 text-orange-400" />
                            ) : (
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-0.5">
                              <span
                                className={`text-[10px] ${t.isDark ? 'text-slate-400' : 'text-slate-500'}`}
                              >
                                {item.type}
                              ...
                              {item.sourceFile && (
                                <span className={`text-[9px] ${t.text.dimmed} truncate max-w-24`}>
                                  {item.sourceFile}
                                </span>
                              )}
                            </div>
                            <p
                              className={`text-[11px] font-mono truncate ${t.isDark ? 'text-slate-300' : 'text-slate-700'}`}
                            >
                              {item.preview}
                            </p>
                          ...
                          <button
                            onClick={(e) => {
                              ...
                              setClipboardHistory((prev) => prev.filter((h) => h.id !== item.id));
                            }}
            ...
            {/* ── Footer ── */}
            <div
              className={`flex items-center justify-between px-5 py-2.5 border-t text-[10px] ${t.isDark ? 'border-white/5 text-slate-500' : 'border-slate-100 text-slate-400'}`}
            >
              <span>
                {i.qaFooterHint ||
                  'Select text in editor, then use actions. AI actions require a connected model.'}
              </span>
