...
import { Monitor, Tablet, Smartphone, X, Camera, ArrowUpDown } from 'lucide-react';
...
const DEVICE_ICONS: Record<string, React.ElementType> = {
  ...
  mobile: Smartphone,
  'mobile-se': Smartphone,
  'mobile-pro': Smartphone,
  android: Smartphone,
};
...
export function MultiDevicePreview({ code, language, open, onClose }: MultiDevicePreviewProps) {
  ...
  return (
    <div className="fixed inset-0 z-[90] flex flex-col">
      ...
      />
      <div
        className={`relative flex flex-col h-full max-h-[95vh] m-4 ${t.surface.modal} rounded-2xl overflow-hidden backdrop-blur-xl`}
      ...
      >
        ...
        <div className={`flex items-center justify-between px-4 py-3 border-b ${t.border.subtle}`}>
          <div className="flex items-center gap-3">
            ...
            <span
              className={`text-[10px] px-2 py-0.5 rounded-full ${t.isDark ? 'bg-white/5 text-white/40' : 'bg-slate-100 text-slate-500'}`}
            >
        ...
        <div
          className={`flex items-center gap-1 px-4 py-2 border-b ${t.border.subtle} overflow-x-auto`}
        >
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[9px] whitespace-nowrap ${t.transition} ${
                  active
                    ...
                    : `${t.text.dimmed} ${t.interactive.iconBtn}`
                }`}
              >
              ...
              return (
                <div key={device.id} className="flex flex-col items-center gap-2">
                  ...
                  <div className={`text-[10px] ${t.text.dimmed} flex items-center gap-1.5`}>
                    {React.createElement(DEVICE_ICONS[device.id] || Monitor, {
                      className: 'w-3 h-3',
                    })}
                    {device.name}
                    <span
                      className={`px-1.5 py-0.5 rounded text-[8px] ${t.isDark ? 'bg-white/5' : 'bg-slate-100'}`}
                    >
                  ...
                  {/* Viewport frame */}
                  <div
                    className={`rounded-xl overflow-hidden border-2 ${t.isDark ? 'border-white/10' : 'border-slate-200'}`}
