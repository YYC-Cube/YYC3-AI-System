...
export function PreviewHistory({
  ...
  return (
    <div
      className={`flex flex-col h-full border-l ${t.border.medium} ${t.surface.glass}`}
      style={{ width: 280 }}
    >
      {/* Header */}
      <div className={`flex items-center justify-between px-3 py-2.5 border-b ${t.border.subtle}`}>
        <div className="flex items-center gap-2">
          ...
          <span
            className={`text-[9px] px-1.5 py-0.5 rounded-full ${t.isDark ? 'bg-white/5 text-white/40' : 'bg-slate-100 text-slate-500'}`}
          >
            {snapshots.length}
          </span>
        </div>
      ...
      {allTags.length > 0 && (
        <div
          className={`flex items-center gap-1 px-3 py-1.5 border-b ${t.border.subtle} overflow-x-auto`}
        >
          <button
                ...
                >
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div
                        className={`text-[10px] ${t.text.secondary} truncate`}
                      ...
                      <div className={`text-[8px] ${t.text.dimmed} flex items-center gap-2 mt-0.5`}>
                        ...
                        {snap.tags.map((tag) => (
                          <span
                            key={tag}
                            className={`px-1 py-0 rounded ${t.isDark ? 'bg-white/5' : 'bg-slate-200'}`}
                          >
                            {tag}
                          </span>
                        ))}
                    ...
                    {snap.isAuto && (
                      <span
                        className={`text-[7px] px-1 py-0.5 rounded ${t.isDark ? 'bg-amber-500/10 text-amber-400' : 'bg-amber-50 text-amber-600'}`}
                      >
                        自动
                      </span>
                    )}
                  ...
                  {expanded && (
                    <div className={`px-2.5 pb-2 pt-0 space-y-1.5`}>
                      {/* Code preview */}
                      <pre
                        className={`text-[8px] ${t.text.dimmed} p-2 rounded-lg overflow-hidden max-h-16 ${t.isDark ? 'bg-black/20' : 'bg-white'}`}
                      >
