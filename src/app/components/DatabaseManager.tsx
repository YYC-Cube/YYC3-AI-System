...
import {
  Database, X, Plus, Trash2, Play,
  ChevronRight, ChevronDown, Table2, RefreshCw,
  Download, Upload, Loader2, Copy,
  Plug, HardDrive, Zap,
  Clock, ArrowUpDown, AlertTriangle
} from 'lucide-react'
...
import {
  indexManager, queryAnalyzer, slowQueryMonitor, queryCache,
  type IndexStats, type QueryAnalysis, type SlowQuery,
...
const SQL_TEMPLATES = [
  ...
  { label: 'CREATE', sql: 'CREATE TABLE new_table (\n  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),\n  name VARCHAR(255) NOT NULL,\n  created_at TIMESTAMP DEFAULT NOW()\n);' },
]
...
const SQL_KEYWORDS = [
  ...
  'ORDER', 'BY', 'GROUP', 'HAVING', 'LIMIT', 'OFFSET', 'AS', 'COUNT',
  'SUM', 'AVG', 'MAX', 'MIN', 'DISTINCT', 'NULL', 'DEFAULT', 'PRIMARY',
  'KEY', 'FOREIGN', 'REFERENCES', 'CASCADE', 'CONSTRAINT', 'UNIQUE',
  'CHECK', 'EXISTS', 'BETWEEN', 'CASE', 'WHEN', 'THEN', 'ELSE', 'END',
]

/* ═══════════════════════════════════════════ */

interface DatabaseManagerProps { open: boolean; onClose: () => void }
...
export function DatabaseManager({ open, onClose }: DatabaseManagerProps) {
  ...
  const [newConn, setNewConn] = useState<Partial<DBConnectionProfile>>({
    name: '', type: 'postgresql', host: 'localhost', port: 5432, database: '', username: '', password: '', ssl: false,
  })
  ...
  useEffect(() => {
    ...
    const loadSchema = async () => {
      ...
      } catch { /* ignore — will use empty schema */ }
    }
    ...
    storageService.getDBProfiles().then(profiles => {
      if (profiles.length > 0) {
        ...
        const demos: DBConnectionProfile[] = [
          { id: 'c1', name: 'YYC3 Dev', type: 'postgresql', host: 'localhost', port: 5432, username: 'yyc3_dev', password: '', database: 'yyc3_development', ssl: false },
          { id: 'c2', name: 'Cache Server', type: 'redis', host: 'localhost', port: 6379, username: '', password: '', database: '0', ssl: false },
        ]
  ...
  useEffect(() => {
    ...
    setHistory(h.map((e, i) => ({ id: `h-${i}`, sql: e.sql, timestamp: e.timestamp, duration: e.duration, rows: e.rowCount })))
  }, [open])

  const activeConnection = useMemo(() => connections.find(c => c.id === activeConn), [connections, activeConn])
  ...
  const autoDetect = useCallback(async () => {
    ...
    try {
      ...
      setEngines(detected)
      toast.success(`${ii.dbDetected}: ${detected.filter(e => e.status === 'running').map(e => `${e.type} ${e.version}`).join(', ')}`)
    } catch { toast.error('Detection failed') }
    setDetecting(false)
  }, [ii])

  // ── Test connection ──
  const testConnection = useCallback(async (profile: DBConnectionProfile) => {
    try {
  ...
  const addConnection = useCallback(async () => {
    ...
    setNewConn({ name: '', type: 'postgresql', host: 'localhost', port: 5432, database: '', username: '', password: '', ssl: false })
  ...
  const executeQuery = useCallback(async () => {
    ...
    try {
      ...
      setHistory(prev => [{
        id: `q-${Date.now()}`, sql: sql.trim(), timestamp: Date.now(),
        duration: result.duration, rows: result.rowCount,
      }, ...prev.slice(0, 49)])
  ...
  const handleBackup = useCallback(async () => {
    ...
    try {
      ...
      toast.success(`Backup complete: ${(result.size / 1024 / 1024).toFixed(1)}MB in ${(result.duration / 1000).toFixed(1)}s`)
  ...
  const handleRestore = useCallback(async () => {
    ...
    input.onchange = async (e) => {
      ...
      try {
        ...
        toast.success(`Restore complete: ${result.tablesRestored} tables in ${(result.duration / 1000).toFixed(1)}s`)
        ...
        >
          ...
          <div className={`flex items-center justify-between px-6 py-3 border-b ${t.border.subtle}`}>
            <div className="flex items-center gap-3">
              <div className={`w-8 h-8 rounded-xl flex items-center justify-center ${t.isDark ? 'bg-gradient-to-br from-blue-500/20 to-cyan-500/20' : 'bg-gradient-to-br from-blue-50 to-cyan-50'}`}>
                <Database className={`w-4 h-4 ${t.isDark ? 'text-blue-400' : 'text-blue-500'}`} />
              </div>
              <div>
                <h2 className={`text-[14px] ${t.text.primary}`} style={{ fontWeight: 700 }}>{ii.dbTitle}</h2>
                <p className={`text-[10px] ${t.text.dimmed}`}>
                  {ii.dbSubtitle} · {Object.values(connStatuses).filter(s => s.connected).length}/{connections.length} {ii.dbConnections}
                  {engines.length > 0 && ` · ${engines.filter(e => e.status === 'running').length} engines detected`}
                </p>
              </div>
            </div>
          ...
          {engines.length > 0 && (
            <div className={`flex items-center gap-2 px-6 py-1.5 border-b ${t.border.subtle} overflow-x-auto`}>
              <span className={`text-[8px] ${t.text.dimmed}`}>Detected:</span>
              {engines.map(eng => (
                <div key={eng.type} className={`flex items-center gap-1 px-2 py-0.5 rounded text-[8px] ${
                  eng.status === 'running'
                    ? (t.isDark ? 'bg-emerald-500/10 text-emerald-400' : 'bg-emerald-50 text-emerald-600')
                    : (t.isDark ? 'bg-red-500/10 text-red-400' : 'bg-red-50 text-red-600')
                }`}>
                  <span>{ENGINE_META[eng.type]?.icon ?? '?'}</span>
                  <span style={{ fontWeight: 500 }}>{eng.type} {eng.version}</span>
                  <span>:{eng.defaultPort}</span>
                  <span className={`w-1.5 h-1.5 rounded-full ${eng.status === 'running' ? 'bg-emerald-400' : 'bg-red-400'}`} />
                </div>
              ))}
            </div>
          )}
          ...
          <div className="flex flex-1 overflow-hidden">
            ...
            <div className={`w-56 flex-shrink-0 flex flex-col border-r ${t.border.subtle}`}>
              <div className={`flex items-center justify-between px-3 py-2 border-b ${t.border.subtle}`}>
                <span className={`text-[9px] uppercase tracking-wider ${t.text.dimmed}`} style={{ fontWeight: 600 }}>{ii.dbConnections}</span>
                <button onClick={() => setShowAddConn(!showAddConn)}
                  className={`p-0.5 rounded ${t.transition} ${t.interactive.iconBtn}`} title={ii.dbAddConnection}>
              ...
              {showAddConn && (
                <div className={`p-2 border-b ${t.border.subtle} space-y-1.5`}>
                  <input value={newConn.name || ''} onChange={e => setNewConn(p => ({ ...p, name: e.target.value }))}
                    placeholder="Connection name" className={`w-full px-2 py-1 rounded text-[9px] outline-none ${t.input.base}`} />
                  <select value={newConn.type || 'postgresql'}
                    onChange={e => setNewConn(p => ({ ...p, type: e.target.value as DbEngine, port: ENGINE_META[e.target.value]?.defaultPort ?? 5432 }))}
                    className={`w-full px-2 py-1 rounded text-[9px] outline-none ${t.input.base}`}>
                    {Object.keys(ENGINE_META).map(k => <option key={k} value={k}>{ENGINE_META[k].icon} {k}</option>)}
                  </select>
                  <div className="flex gap-1">
                    <input value={newConn.host || ''} onChange={e => setNewConn(p => ({ ...p, host: e.target.value }))}
                      placeholder="Host" className={`flex-1 px-2 py-1 rounded text-[9px] outline-none ${t.input.base}`} />
                    <input type="number" value={newConn.port || 5432} onChange={e => setNewConn(p => ({ ...p, port: parseInt(e.target.value) }))}
                      placeholder="Port" className={`w-16 px-2 py-1 rounded text-[9px] outline-none ${t.input.base}`} />
                  </div>
                  <input value={newConn.database || ''} onChange={e => setNewConn(p => ({ ...p, database: e.target.value }))}
                    placeholder="Database" className={`w-full px-2 py-1 rounded text-[9px] outline-none ${t.input.base}`} />
                  <div className="flex gap-1">
                    <input value={newConn.username || ''} onChange={e => setNewConn(p => ({ ...p, username: e.target.value }))}
                      placeholder="Username" className={`flex-1 px-2 py-1 rounded text-[9px] outline-none ${t.input.base}`} />
                    <input type="password" value={newConn.password || ''} onChange={e => setNewConn(p => ({ ...p, password: e.target.value }))}
                      placeholder="Password" className={`flex-1 px-2 py-1 rounded text-[9px] outline-none ${t.input.base}`} />
                  </div>
                  <div className="flex gap-1">
                    ...
                    <button onClick={() => setShowAddConn(false)}
                      className={`px-2 py-1 rounded text-[8px] ${t.interactive.iconBtn}`}>Cancel</button>
                  </div>
                </div>
              )}
                  ...
                  return (
                    <div key={conn.id}
                      className={`group w-full flex items-center gap-2 px-3 py-2.5 text-left border-b ${t.border.subtle} ${t.transition} cursor-pointer ${
                        activeConn === conn.id ? (t.isDark ? 'bg-indigo-500/10' : 'bg-indigo-50/80') : t.interactive.menuItem
                      }`}
                      onClick={() => setActiveConn(conn.id)}>
                      <span className="text-[14px]">{meta.icon}</span>
                      <div className="flex-1 min-w-0">
                        <div className={`text-[10px] truncate ${t.text.primary}`} style={{ fontWeight: 500 }}>{conn.name}</div>
                        <div className={`text-[8px] ${t.text.dimmed}`}>{conn.host}:{conn.port}</div>
                      </div>
                      <div className={`w-2 h-2 rounded-full flex-shrink-0 ${
                        status === 'connected' ? 'bg-emerald-500' : status === 'testing' ? 'bg-amber-400 animate-pulse' : 'bg-slate-500'
                      }`} />
              ...
              {activeConnection && (
                <div className={`border-t ${t.border.subtle} p-3 space-y-1.5`}>
                  <div className="flex items-center gap-2">
                    <span className="text-[12px]">{ENGINE_META[activeConnection.type]?.icon ?? '?'}</span>
                    <span className={`text-[10px] ${t.text.primary}`} style={{ fontWeight: 600 }}>{activeConnection.name}</span>
                  </div>
                  {[
                    ...
                    <div key={f.label} className="flex items-center gap-2">
                      <span className={`text-[7px] w-10 flex-shrink-0 ${t.text.dimmed}`}>{f.label}</span>
                      <span className={`text-[8px] font-mono ${t.text.muted}`}>{f.value}</span>
                    </div>
                  ))}
                  {connStatuses[activeConnection.id] && (
                    <div className={`text-[7px] px-2 py-1 rounded ${
                  ...
                  <div className="flex items-center gap-1 pt-1">
                    <button onClick={() => testConnection(activeConnection)}
                    ...
                    <button onClick={handleBackup} disabled={backingUp}
                      className={`flex-1 flex items-center justify-center gap-1 py-1 rounded-lg text-[8px] ${t.transition} ${t.interactive.iconBtn}`}>
                      {backingUp ? <Loader2 className="w-2.5 h-2.5 animate-spin" /> : <Download className="w-2.5 h-2.5" />} {ii.dbBackup}
                    </button>
                  </div>
                  <button onClick={handleRestore} disabled={restoring}
                    ...
                    {restoring ? <Loader2 className="w-2.5 h-2.5 animate-spin" /> : <Upload className="w-2.5 h-2.5" />} Restore
                  </button>
                </div>
              )}
            </div>
            ...
            <div className="flex-1 flex flex-col">
              ...
              <div className={`flex items-center gap-0 px-4 border-b ${t.border.subtle}`}>
                {([
                  ...
                  <button key={mt.tab} onClick={() => setMainTab(mt.tab)}
                    className={`flex items-center gap-1 px-3 py-2 text-[9px] ${t.transition} border-b-2 ${
                      mainTab === mt.tab ? `${t.accent.primary} border-current` : `${t.text.muted} border-transparent`
                    }`} style={{ fontWeight: mainTab === mt.tab ? 600 : 400 }}>
                    <mt.icon className="w-3 h-3" /> {mt.label}
                    {mt.tab === 'history' && history.length > 0 && (
                      <span className={`ml-1 text-[7px] px-1 rounded-full ${t.isDark ? 'bg-white/5' : 'bg-slate-100'}`}>{history.length}</span>
                    )}
                  </button>
                ))}
                ...
                {mainTab === 'console' && (
                  <div className="flex items-center gap-0.5">
                    {SQL_TEMPLATES.slice(0, 4).map(tmpl => (
                      <button key={tmpl.label} onClick={() => setSql(tmpl.sql.replace('{table}', expandedTable || 'users'))}
              ...
              {mainTab === 'console' && (
                <>
                  ...
                  <div className={`relative border-b ${t.border.subtle}`}>
                        ...
                        onMount={(editor, monaco) => {
                          ...
                          monaco.languages.registerCompletionItemProvider('sql', {
                            ...
                            provideCompletionItems: (model: any, position: any) => {
                              ...
                              const dotMatch = lineContent.substring(0, position.column - 1).match(/(\w+)\.\s*$/)
                              if (dotMatch) {
                              ...
                              const fromMatch = fullText.match(/(?:FROM|JOIN|INTO|UPDATE)\s+(\w+)/gi)
                              ...
                              const colTables = contextTables.size > 0
                    ...
                    <div className={`flex items-center gap-2 px-3 py-1.5 border-t ${t.border.subtle}`}>
                      <button onClick={executeQuery} disabled={executing || !activeConn}
                      ...
                      {!activeConn && <span className={`text-[8px] text-amber-400 flex items-center gap-1`}><AlertTriangle className="w-2.5 h-2.5" /> Select a connection</span>}
                      <div className="flex-1" />
                      {resultMeta && (
                        <div className="flex items-center gap-3">
                          <span className={`text-[8px] ${t.text.dimmed}`}>{resultMeta.rowCount} {ii.dbRows}</span>
                          <span className={`text-[8px] ${t.text.dimmed}`}>{resultMeta.duration}ms</span>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className={`flex-1 overflow-hidden`}>
                    {executing ? (
                      ...
                      <div className={`flex flex-col items-center justify-center h-full gap-2 ${t.text.dimmed}`}>
                        ...
                        <span className={`text-[8px] ${t.text.muted}`}>Write a SQL query above and press Execute</span>
                      </div>
                    )}
                  </div>
                </>
              ...
              {mainTab === 'explorer' && (
                <div className={`flex-1 overflow-y-auto ${t.scrollbar} p-2`}>
                  {explorerLoading ? (
                    ...
                    <div className={`flex flex-col items-center justify-center py-8 ${t.text.dimmed} gap-2`}>
                      <HardDrive className="w-5 h-5 opacity-20" />
                      <span className="text-[10px]">No schemas found</span>
                      <button onClick={loadExplorer} className={`text-[9px] ${t.accent.primary}`}>Refresh</button>
                    </div>
                  ) : (
                    schemas.map(schema => (
                      <div key={schema}>
                        <button onClick={() => setExpandedSchema(expandedSchema === schema ? '' : schema)}
                          className={`w-full flex items-center gap-2 px-2 py-1.5 text-[10px] rounded-lg ${t.transition} ${t.interactive.menuItem}`}>
                          {expandedSchema === schema ? <ChevronDown className="w-3 h-3" /> : <ChevronRight className="w-3 h-3" />}
                          <HardDrive className="w-3.5 h-3.5 text-indigo-400" />
                          <span className={t.text.primary} style={{ fontWeight: 600 }}>{schema}</span>
                          {schema === 'public' && <span className={`text-[8px] ${t.text.dimmed}`}>{tables.length} {ii.dbTables}</span>}
                        </button>

                        {expandedSchema === schema && tables.map(table => (
                          <div key={table.name} className="ml-5">
                            <button onClick={() => toggleTable(table.name)}
                              className={`group w-full flex items-center gap-2 px-2 py-1 text-[9px] rounded-lg ${t.transition} ${
                                expandedTable === table.name ? (t.isDark ? 'bg-indigo-500/10' : 'bg-indigo-50') : t.interactive.menuItem
                              }`}>
                              {expandedTable === table.name ? <ChevronDown className="w-2.5 h-2.5" /> : <ChevronRight className="w-2.5 h-2.5" />}
                              <Table2 className="w-3 h-3 text-cyan-400" />
                              <span className={t.text.primary} style={{ fontWeight: 500 }}>{table.name}</span>
                              <span className={`text-[7px] px-1 rounded ${t.isDark ? 'bg-white/[0.04]' : 'bg-slate-100'} ${t.text.dimmed}`}>
                                {(table.rowCount ?? 0).toLocaleString()} {ii.dbRows}
                              </span>
                              <button onClick={e => { e.stopPropagation(); setSql(`SELECT * FROM ${table.name} LIMIT 20;`); setMainTab('console') }}
                            ...
                            {expandedTable === table.name && (
                              <div className="ml-6 my-1 space-y-0.5">
                                {(tableColumns[table.name] || table.columns).map(col => (
                                  <div key={col.name} className={`flex items-center gap-2 px-2 py-1 rounded text-[8px] ${t.isDark ? 'bg-white/[0.01]' : 'bg-slate-50/50'}`}>
                                    {col.primaryKey && <span className="text-[6px] px-1 rounded bg-amber-500/15 text-amber-400" style={{ fontWeight: 700 }}>PK</span>}
                                    <span className={`font-mono ${t.text.primary}`} style={{ fontWeight: 500 }}>{col.name}</span>
                                    <span className={`font-mono ${t.isDark ? 'text-cyan-400/50' : 'text-cyan-600/50'}`}>{col.type}</span>
                                    {col.nullable && <span className={`text-[6px] ${t.text.dimmed}`}>NULL</span>}
                                    {col.defaultValue && <span className={`text-[7px] ${t.text.dimmed}`}>= {col.defaultValue}</span>}
                                  </div>
                                ))}
                                <div className="flex items-center gap-1 px-2 pt-1">
                                  {SQL_TEMPLATES.slice(0, 3).map(tmpl => (
                                    <button key={tmpl.label}
                                      onClick={() => { setSql(tmpl.sql.replace('{table}', table.name)); setMainTab('console') }}
                ...
                <div className={`flex-1 overflow-y-auto ${t.scrollbar}`}>
                  {history.length === 0 ? (
                    ...
                    <div className="divide-y divide-white/[0.04]">
                      {history.map(h => (
                        <button key={h.id} onClick={() => { setSql(h.sql); setMainTab('console') }}
                          ...
                          <div className="flex-1 min-w-0">
                            <div className={`font-mono text-[9px] ${t.text.primary} line-clamp-2`}>{h.sql}</div>
                            <div className={`flex items-center gap-3 mt-1 text-[7px] ${t.text.dimmed}`}>
                              <span>{new Date(h.timestamp).toLocaleTimeString()}</span>
                              <span>{h.rows} rows</span>
                              <span>{h.duration}ms</span>
                            </div>
                          </div>
                          <button onClick={e => { e.stopPropagation(); navigator.clipboard.writeText(h.sql); toast.success('SQL copied') }}
              ...
              {/* ── Indexes Tab ── */}
              {mainTab === 'indexes' && (
                <IndexesTab connId={activeConn} t={t} ii={ii} />
              )}
...
function IndexesTab({ connId, t, ii }: { connId: string | null; t: ReturnType<typeof getThemeTokens>; ii: ReturnType<typeof getI18n> }) {
  const [indexes, setIndexes] = useState<IndexStats[]>([])
  const [loading, setLoading] = useState(false)
  const [recs, setRecs] = useState<{ table: string; configs: { table: string; columns: string[]; type?: string }[] }[]>([])

  const load = useCallback(async () => {
    if (!connId) return
    setLoading(true)
    try { setIndexes(await indexManager.getIndexes(connId)) } catch {}
    setLoading(false)
  }, [connId])

  useEffect(() => { load() }, [load])

  const recommend = useCallback(async (table: string) => {
    if (!connId) return
    const cfgs = await indexManager.recommendIndexes(connId, table)
    setRecs(prev => [...prev.filter(r => r.table !== table), ...(cfgs.length ? [{ table, configs: cfgs }] : [])])
    toast.success(cfgs.length ? `${cfgs.length} ${ii.dbIndexRecommendations}` : ii.dbNoIndexes)
  }, [connId, ii])

  const create = useCallback(async (cfg: IndexConfig) => {
    if (!connId) return
  ...
  return (
    <div className={`flex-1 overflow-y-auto p-4 space-y-4 ${t.scrollbar}`}>
      <div className="flex items-center justify-between">
        <div>
          <h3 className={`text-[12px] ${t.text.primary}`} style={{ fontWeight: 700 }}>{ii.dbIndexManager}</h3>
          <p className={`text-[9px] ${t.text.dimmed}`}>{indexes.length} {ii.dbIndexes} · {tables.length} {ii.dbTables}</p>
        </div>
        <button onClick={load} disabled={loading}
          className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[9px] ${t.transition} ${t.interactive.iconBtn}`}>
          {loading ? <Loader2 className="w-3 h-3 animate-spin" /> : <RefreshCw className="w-3 h-3" />} {ii.dbRefresh}
        </button>
      </div>

      {tables.map(table => (
        <div key={table} className={`rounded-xl border ${t.border.subtle} overflow-hidden`}>
          <div className={`flex items-center justify-between px-3 py-2 ${t.isDark ? 'bg-white/[0.02]' : 'bg-slate-50'}`}>
            <div className="flex items-center gap-2">
              <Table2 className={`w-3.5 h-3.5 ${t.isDark ? 'text-cyan-400' : 'text-cyan-600'}`} />
              <span className={`text-[10px] ${t.text.primary}`} style={{ fontWeight: 600 }}>{table}</span>
              <span className={`text-[8px] px-1.5 rounded ${t.isDark ? 'bg-white/[0.04]' : 'bg-slate-100'} ${t.text.dimmed}`}>
                {indexes.filter(i => i.table === table).length} idx
              </span>
            </div>
            <button onClick={() => recommend(table)}
          ...
          <div className={`divide-y ${t.isDark ? 'divide-white/[0.04]' : 'divide-slate-100'}`}>
            {indexes.filter(i => i.table === table).map(idx => (
              <div key={idx.name} className={`flex items-center gap-3 px-3 py-2 group ${t.transition} ${t.interactive.menuItem}`}>
                <ArrowUpDown className={`w-3 h-3 flex-shrink-0 ${t.text.dimmed}`} />
                <div className="flex-1 min-w-0">
                  <div className={`text-[9px] font-mono ${t.text.primary}`} style={{ fontWeight: 500 }}>{idx.name}</div>
                  <div className={`flex items-center gap-3 text-[7px] ${t.text.dimmed}`}>
                    <span>{idx.columns.join(', ')}</span>
                    <span>{idx.sizeFormatted}</span>
                    <span>{ii.dbIndexScans}: {idx.scans.toLocaleString()}</span>
                  </div>
                </div>
                <div className="flex gap-1 opacity-0 group-hover:opacity-100">
                  <button onClick={() => rebuild(idx.name)} className={`p-1 rounded ${t.interactive.iconBtn}`} title={ii.dbIndexRebuild}>
                    <RefreshCw className="w-2.5 h-2.5" />
                  </button>
                  <button onClick={() => drop(idx.name)} className="p-1 rounded text-red-400/60 hover:text-red-400" title={ii.dbIndexDrop}>
                    <Trash2 className="w-2.5 h-2.5" />
                  </button>
                </div>
              </div>
          ...
          {recs.filter(r => r.table === table).flatMap(r => r.configs).map((cfg, i) => (
            <div key={`rec-${i}`} className={`flex items-center gap-3 px-3 py-2 border-t ${t.border.subtle} ${t.isDark ? 'bg-amber-500/[0.03]' : 'bg-amber-50/50'}`}>
              <AlertTriangle className="w-3 h-3 flex-shrink-0 text-amber-400" />
              <span className={`flex-1 text-[9px] ${t.isDark ? 'text-amber-300' : 'text-amber-700'}`}>
                {cfg.type?.toUpperCase()} on ({cfg.columns.join(', ')})
              </span>
              <button onClick={() => create(cfg as IndexConfig)}
...
function PerformanceTab({ connId, sql, t, ii }: { connId: string | null; sql: string; t: ReturnType<typeof getThemeTokens>; ii: ReturnType<typeof getI18n> }) {
  ...
  useEffect(() => { setSlowQueries(slowQueryMonitor.getSlowQueries()) }, [])

  const analyze = useCallback(async () => {
    if (!connId || !sql.trim()) return
    setAnalyzing(true)
    try { setAnalysis(await queryAnalyzer.analyzeQuery(connId, sql.trim())) } catch { toast.error(ii.dbAnalyzingQuery + ' ✗') }
    setAnalyzing(false)
  }, [connId, sql, ii])

  const sevCls = (s: string) =>
    s === 'critical' ? (t.isDark ? 'text-red-400 bg-red-500/10' : 'text-red-600 bg-red-50')
    : s === 'warning' ? (t.isDark ? 'text-amber-400 bg-amber-500/10' : 'text-amber-600 bg-amber-50')
    : (t.isDark ? 'text-blue-400 bg-blue-500/10' : 'text-blue-600 bg-blue-50')

  return (
    <div className={`flex-1 overflow-y-auto p-4 space-y-4 ${t.scrollbar}`}>
      {/* Analyzer */}
      <div className={`rounded-xl border ${t.border.subtle} overflow-hidden`}>
        <div className={`flex items-center justify-between px-4 py-2.5 ${t.isDark ? 'bg-white/[0.02]' : 'bg-slate-50'}`}>
          <div>
            <h3 className={`text-[11px] ${t.text.primary}`} style={{ fontWeight: 700 }}>{ii.dbQueryAnalyzer}</h3>
            <p className={`text-[8px] ${t.text.dimmed}`}>EXPLAIN ANALYZE — {ii.dbAnalyzeSql}</p>
          </div>
          <button onClick={analyze} disabled={analyzing || !connId}
        ...
        {analysis && (
          <div className="p-4 space-y-3">
            <div className="grid grid-cols-4 gap-2">
              {[
                { label: ii.dbExecTime, value: `${analysis.executionTime}ms`, cls: analysis.executionTime > 100 ? 'text-amber-400' : 'text-emerald-400' },
                { label: ii.dbRowsScanned, value: analysis.rowsScanned.toLocaleString(), cls: analysis.rowsScanned > 1000 ? 'text-amber-400' : 'text-emerald-400' },
                { label: ii.dbRowsReturned, value: analysis.rowsReturned.toLocaleString(), cls: '' },
                { label: ii.dbIndexUsed, value: analysis.indexUsed || 'None', cls: analysis.indexUsed ? 'text-emerald-400' : 'text-red-400' },
              ].map(m => (
                <div key={m.label} className={`px-3 py-2 rounded-lg ${t.isDark ? 'bg-white/[0.03]' : 'bg-slate-50'}`}>
                  <div className={`text-[7px] ${t.text.dimmed}`}>{m.label}</div>
                  <div className={`text-[11px] font-mono ${m.cls || t.text.primary}`} style={{ fontWeight: 600 }}>{m.value}</div>
                </div>
              ))}
            </div>
            <div className={`p-3 rounded-lg ${t.isDark ? 'bg-white/[0.02]' : 'bg-slate-50'}`}>
              <div className={`text-[8px] ${t.text.dimmed} mb-1`} style={{ fontWeight: 600 }}>{ii.dbQueryPlan}</div>
              <div className={`font-mono text-[9px] ${t.text.muted} space-y-0.5`}>
                <div>{analysis.plan.nodeType} on {analysis.plan.relation || '?'} (cost={analysis.plan.cost.startup.toFixed(2)}..{analysis.plan.cost.total.toFixed(2)} rows={analysis.plan.rows})</div>
                {analysis.plan.indexName && <div className="ml-4">Index: {analysis.plan.indexName}</div>}
                {analysis.plan.filter && <div className="ml-4">Filter: {analysis.plan.filter}</div>}
                {analysis.plan.children?.map((c, i) => (
                  <div key={i} className="ml-6">→ {c.nodeType} (cost={c.cost.total.toFixed(2)} rows={c.rows})</div>
                ))}
              </div>
            </div>
            {analysis.recommendations.length > 0 && (
              <div className="space-y-1.5">
                <div className={`text-[8px] ${t.text.dimmed}`} style={{ fontWeight: 600 }}>{ii.dbQueryRecommendations}</div>
                {analysis.recommendations.map((r, i) => (
                  <div key={i} className={`flex items-start gap-2 px-3 py-2 rounded-lg ${sevCls(r.severity)}`}>
                    <AlertTriangle className="w-3 h-3 flex-shrink-0 mt-0.5" />
                    <div>
                      <div className="text-[9px]" style={{ fontWeight: 500 }}>{r.message}</div>
                      {r.detail && <div className="text-[8px] mt-0.5 opacity-70">{r.detail}</div>}
                      {r.suggestedSql && (
                        <div className={`font-mono text-[8px] mt-1 px-2 py-1 rounded ${t.isDark ? 'bg-black/20' : 'bg-white/60'}`}>{r.suggestedSql}</div>
                      )}
                    </div>
                  </div>
                ))}
      ...
      <div className={`rounded-xl border ${t.border.subtle} overflow-hidden`}>
        <div className={`px-4 py-2.5 ${t.isDark ? 'bg-white/[0.02]' : 'bg-slate-50'}`}>
          <h3 className={`text-[11px] ${t.text.primary}`} style={{ fontWeight: 700 }}>{ii.dbQueryCache}</h3>
        </div>
        <div className="grid grid-cols-4 gap-2 p-3">
          {[
            ...
            <div key={s.label} className={`px-3 py-2 rounded-lg ${t.isDark ? 'bg-white/[0.03]' : 'bg-slate-50'}`}>
              <div className={`text-[7px] ${t.text.dimmed}`}>{s.label}</div>
              <div className={`text-[11px] font-mono ${t.text.primary}`} style={{ fontWeight: 600 }}>{s.value}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className={`rounded-xl border ${t.border.subtle} overflow-hidden`}>
        <div className={`flex items-center justify-between px-4 py-2.5 ${t.isDark ? 'bg-white/[0.02]' : 'bg-slate-50'}`}>
          <div>
            <h3 className={`text-[11px] ${t.text.primary}`} style={{ fontWeight: 700 }}>{ii.dbSlowQueries}</h3>
            <p className={`text-[8px] ${t.text.dimmed}`}>
              {ii.dbSlowThreshold}: {slowQueryMonitor.getThreshold()}ms · {ii.dbSlowTotal}: {slowStats.total} · {ii.dbSlowAvg}: {slowStats.avgDuration}ms
            </p>
          </div>
          <button onClick={() => { slowQueryMonitor.clear(); setSlowQueries([]) }}
            className={`text-[8px] px-2 py-0.5 rounded ${t.transition} ${t.interactive.iconBtn}`}>{ii.dbSlowClear}</button>
        </div>
        <div className={`divide-y ${t.isDark ? 'divide-white/[0.04]' : 'divide-slate-100'} max-h-[200px] overflow-y-auto`}>
          {slowQueries.length === 0 ? (
            <div className={`flex items-center justify-center py-6 ${t.text.dimmed} text-[10px]`}>{ii.dbNoSlowQueries}</div>
          ) : slowQueries.map(sq => (
            <div key={sq.id} className={`flex items-start gap-3 px-4 py-2 ${t.interactive.menuItem}`}>
              <span className={`px-1.5 py-0.5 rounded text-[8px] font-mono flex-shrink-0 ${
                sq.duration > 1000 ? (t.isDark ? 'bg-red-500/10 text-red-400' : 'bg-red-50 text-red-600')
                  : (t.isDark ? 'bg-amber-500/10 text-amber-400' : 'bg-amber-50 text-amber-600')
              }`} style={{ fontWeight: 600 }}>{sq.duration}ms</span>
              <div className="flex-1 min-w-0">
                <div className={`font-mono text-[8px] ${t.text.primary} line-clamp-2`}>{sq.sql}</div>
                <div className={`text-[7px] ${t.text.dimmed} mt-0.5`}>{new Date(sq.timestamp).toLocaleTimeString()} · {sq.rowsReturned} rows</div>
              </div>
            </div>
          ))}
        </div>
