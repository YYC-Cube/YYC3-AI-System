...
async function callModelAPI(
  ...
  const endpoint = providerConfig?.baseURL
...
export function ChatInterface() {
  const { theme, language, messages, addMessage, aiModels, activeModelId, openModelSettings } =
    useAppStore();
      ...
      if (activeModel && activeModel.status === 'connected') {
        ...
        try {
          ...
          if (error.name === 'AbortError') {
            ...
            try {
              ...
              addMessage({
                role: 'system',
                content: `${i.ciModelCallFailed}: ${error2.message || error.message || i.ciUnknownError}\n\n${i.ciFallbackMsg}`,
              });
              addMessage({
                role: 'ai',
        ...
        if (trimmedText.startsWith('/code')) {
          ...
          const noModelHint =
            aiModels.length === 0
              ? '\n\n' + i.ciNoModelHint
              : activeModel && activeModel.status !== 'connected'
                ? '\n\n' + i.ciModelNotTested
                : '';
