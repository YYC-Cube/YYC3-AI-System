...
import {
  Bot, X, Send, Copy, Check, Loader2,
  Code, Wand2, TestTube2, FileText, Bug, Zap,
  MessageCircle, Plus,
  Sparkles, ArrowRight
} from 'lucide-react'
...
const QUICK_ACTIONS: QuickAction[] = [
  { id: 'explain', icon: MessageCircle, labelKey: 'icExplain', prompt: 'Explain this code in detail.' },
  { id: 'refactor', icon: Wand2, labelKey: 'icRefactor', prompt: 'Refactor this code for better readability and performance.' },
  { id: 'test', icon: TestTube2, labelKey: 'icAddTests', prompt: 'Generate comprehensive unit tests for this code.' },
  { id: 'docs', icon: FileText, labelKey: 'icAddDocs', prompt: 'Add JSDoc documentation to this code.' },
  { id: 'fix', icon: Bug, labelKey: 'icFixError', prompt: 'Find and fix potential bugs in this code.' },
  { id: 'optimize', icon: Zap, labelKey: 'icOptimize', prompt: 'Optimize this code for better performance.' },
]
...
export function InlineAIChat({ visible, onClose, anchorLine, selectedCode, currentFile }: InlineAIChatProps) {
  ...
  const handleQuickAction = useCallback((action: QuickAction) => {
    ...
    setTimeout(() => {
      ...
      const { text, codeBlock } = resp ? resp(selectedCode || '') : { text: 'I can help with that! Let me analyze the code...', codeBlock: undefined }
      const aiMsg: InlineChatMessage = {
        id: `a-${Date.now()}`, role: 'ai', text, timestamp: Date.now(), codeBlock,
      }
  ...
  const handleSend = useCallback(() => {
    ...
    const userMsg: InlineChatMessage = {
      id: `u-${Date.now()}`, role: 'user', text: input.trim(), timestamp: Date.now(),
    }
    setMessages(prev => [...prev, userMsg])
    setInput('')
    setThinking(true)
  ...
  return (
    <motion.div
      ...
      className={`absolute z-30 right-2 rounded-2xl overflow-hidden shadow-2xl ${
        t.isDark ? 'bg-[#0f1729]/95 border border-white/[0.08]' : 'bg-white/95 border border-slate-200'
      } backdrop-blur-xl`}
    ...
    >
      ...
      <div className={`flex items-center justify-between px-3 py-2 border-b ${t.border.subtle}`}>
        <div className="flex items-center gap-2">
          <div className={`w-5 h-5 rounded-lg flex items-center justify-center ${t.isDark ? 'bg-violet-500/20' : 'bg-violet-50'}`}>
            <Sparkles className={`w-3 h-3 ${t.isDark ? 'text-violet-400' : 'text-violet-500'}`} />
          </div>
          <span className={`text-[10px] ${t.text.primary}`} style={{ fontWeight: 600 }}>{i.icTitle}</span>
          {selectedCode && (
            <span className={`text-[8px] px-1.5 py-0.5 rounded ${t.isDark ? 'bg-indigo-500/15 text-indigo-400' : 'bg-indigo-50 text-indigo-500'}`}>
              L{anchorLine} · {i.icSelectedLines}
            </span>
          )}
        </div>
        <div className="flex items-center gap-1">
          <button onClick={handleNewChat} className={`p-0.5 rounded ${t.transition} ${t.interactive.iconBtn}`} title={i.icNewChat}>
            <Plus className="w-3 h-3" />
          </button>
          <button onClick={onClose} className={`p-0.5 rounded ${t.transition} ${t.interactive.iconBtn}`}>
            <X className="w-3 h-3" />
          </button>
        </div>
      </div>
      ...
      {messages.length === 0 && (
        <div className={`px-3 py-2 border-b ${t.border.subtle}`}>
          <div className={`text-[7px] uppercase tracking-wider mb-1.5 ${t.text.dimmed}`} style={{ fontWeight: 600 }}>{i.icAskAbout}</div>
      ...
      <div ref={scrollRef} className={`overflow-y-auto ${t.scrollbar}`} style={{ maxHeight: 300 }}>
        {messages.map(msg => (
          <div key={msg.id} className={`px-3 py-2 ${msg.role === 'ai' ? (t.isDark ? 'bg-white/[0.02]' : 'bg-slate-50/50') : ''}`}>
            <div className="flex items-start gap-2">
              <div className={`w-4 h-4 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${
                msg.role === 'ai'
                  ? (t.isDark ? 'bg-violet-500/20' : 'bg-violet-50')
                  : (t.isDark ? 'bg-indigo-500/20' : 'bg-indigo-50')
              }`}>
                {msg.role === 'ai'
                  ? <Bot className={`w-2.5 h-2.5 ${t.isDark ? 'text-violet-400' : 'text-violet-500'}`} />
                  : <span className={`text-[7px] ${t.isDark ? 'text-indigo-400' : 'text-indigo-500'}`} style={{ fontWeight: 700 }}>Y</span>
                }
              </div>
              <div className="flex-1 min-w-0">
                ...
                {msg.codeBlock && (
                  <div className="mt-1.5 relative group">
                    <pre className={`text-[8px] p-2 rounded-lg overflow-x-auto font-mono ${
                      t.isDark ? 'bg-[#0a0f1f] text-emerald-300/70 border border-emerald-500/10' : 'bg-slate-100 text-slate-700 border border-slate-200'
                    }`}>
                      {msg.codeBlock}
                    </pre>
                    <div className="absolute top-1 right-1 flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <button
                        ...
                        className={`flex items-center gap-0.5 p-1 rounded text-[7px] ${
                          t.isDark ? 'bg-violet-500/20 text-violet-400 hover:bg-violet-500/30' : 'bg-violet-50 text-violet-500 hover:bg-violet-100'
                        }`}
                      >
        ...
        {thinking && (
          <div className={`px-3 py-2 ${t.isDark ? 'bg-white/[0.02]' : 'bg-slate-50/50'}`}>
            <div className="flex items-center gap-2">
              <div className={`w-4 h-4 rounded-full flex items-center justify-center ${t.isDark ? 'bg-violet-500/20' : 'bg-violet-50'}`}>
                <Loader2 className={`w-2.5 h-2.5 animate-spin ${t.isDark ? 'text-violet-400' : 'text-violet-500'}`} />
              </div>
