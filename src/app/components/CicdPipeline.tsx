      ...
      jobs: [
        {
          ...
          status: 'passed',
          duration: 12,
          logs: ['> eslint --ext .ts,.tsx src/', '✓ 0 errors, 0 warnings', '✓ 147 files checked'],
        },
        {
          id: 'j2',
          name: 'TypeCheck',
          ...
          status: 'passed',
          duration: 8,
          logs: ['> tsc --noEmit', '✓ No type errors found', '✓ 862 i18n keys verified'],
        },
        {
          id: 'j3',
          name: 'Prettier',
          ...
          status: 'passed',
          duration: 5,
          logs: ['> prettier --check "src/**/*.{ts,tsx}"', '✓ All files formatted'],
        },
      ],
    },
    {
...
export function CicdPipeline({ open, onClose }: CicdPipelineProps) {
    ...
    (jobId: string) => {
      setPipeline((prev) => ({
        ...
        stages: prev.stages.map((s) => ({
          ...
          jobs: s.jobs.map((j) =>
            j.id === jobId
              ? { ...j, status: 'running' as JobStatus, duration: 0, logs: ['Retrying...'] }
              : j
          ),
        })),
      }));
          ...
          stages: prev.stages.map((s) => ({
            ...
            jobs: s.jobs.map((j) =>
              j.id === jobId
                ? {
                    ...j,
                    status: 'passed' as JobStatus,
                    duration: 15,
                    logs: [...j.logs, '✓ Retry successful'],
                  }
                : j
            ),
          })),
      ...
      setTimeout(() => {
        setPipeline((prev) => ({
          ...
          stages: prev.stages.map((s) => ({
            ...s,
            jobs: s.jobs.map((j) =>
              j.id === job.id ? { ...j, status: 'running' as JobStatus } : j
            ),
          })),
        }));
      }, delay);
  ...
  const sel = useMemo(
    () => pipeline.stages.flatMap((s) => s.jobs).find((j) => j.id === selectedJob),
        ...
        >
          ...
          >
            <div className="flex items-center gap-3">
              <div
                className={`w-8 h-8 rounded-xl flex items-center justify-center ${t.isDark ? 'bg-gradient-to-br from-emerald-500/20 to-cyan-500/20' : 'bg-gradient-to-br from-emerald-50 to-cyan-50'}`}
              >
                <Workflow
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  {(() => {
                    ...
                    return (
                      ...
                      >
                        <SIcon
                          className={`w-2.5 h-2.5 ${overallStatus === 'running' ? 'animate-spin' : ''}`}
                        />
                ...
                <p className={`text-[10px] ${t.text.dimmed}`}>
                  {i.ciSubtitle} · {i.ciDuration}: {formatDuration(totalDuration)}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
          ...
          <div className="flex flex-1 overflow-hidden">
            ...
            <div className={`flex-1 overflow-auto p-6 ${t.scrollbar}`}>
                  ...
                  >
                    <div className="p-3">
                      <div className="flex items-center gap-2 mb-2">
                        <Sparkles
                          className={`w-3.5 h-3.5 ${t.isDark ? 'text-violet-400' : 'text-violet-500'}`}
                        />
                        <span
                      ...
                      <div className="space-y-1.5">
                        {pipeline.aiSuggestions.map((sug, _idx) => (
                          <div
                            key={_idx}
                            className={`flex items-start gap-2 text-[9px] ${t.text.muted}`}
                          >
              ...
              <div className="flex items-start gap-4 min-w-max">
                {pipeline.stages.map((stage, sIdx) => {
                  const stageStatus = stage.jobs.some((j) => j.status === 'failed')
                    ? 'failed'
                    : stage.jobs.some((j) => j.status === 'running')
                      ? 'running'
                      : stage.jobs.every((j) => j.status === 'passed' || j.status === 'skipped')
                        ? 'passed'
                        : 'pending';
                    ...
                    <React.Fragment key={stage.id}>
                      {sIdx > 0 && (
                        <div className="flex items-center pt-8 flex-shrink-0">
                          <ArrowRight
                            className={`w-5 h-5 ${stageStatus === 'pending' ? t.text.dimmed : ''}`}
                            style={{ color: stageStatus !== 'pending' ? sc.color : undefined }}
                          />
                        </div>
                      )}
                      <div
                        className={`w-52 rounded-xl border ${t.isDark ? 'bg-white/[0.02] border-white/[0.06]' : 'bg-white/60 border-slate-200/60'}`}
                      >
                        {/* Stage header */}
                        <div
                        ...
                        >
                          <div className="flex items-center gap-1.5">
                            {(() => {
                              ...
                              return (
                                <SI
                                  className={`w-3 h-3 ${stageStatus === 'running' ? 'animate-spin' : ''}`}
                            ...
                            <span
                              ...
                              style={{ fontWeight: 600 }}
                            >
                              {(i as unknown as Record<string, string>)[stage.labelKey] ||
                                stage.name}
                            </span>
                          </div>
                          <span className={`text-[7px] ${t.text.dimmed}`}>
                            {stage.jobs.length} {i.ciJob}
                          </span>
                        </div>
                            ...
                            return (
                              ...
                              >
                                ...
                                <div className="flex-1 min-w-0">
                                  <span
                                    className={`text-[9px] truncate block ${t.text.primary}`}
                                  ...
                                  {job.env && (
                                    <span
                                      ...
                                      style={{
                                        backgroundColor: ENV_COLORS[job.env] + '20',
                                        color: ENV_COLORS[job.env],
                                      }}
                                    >
                                      {
                                        (i as unknown as Record<string, string>)[
                                          job.env === 'dev'
                                            ? 'ciEnvDev'
                                            : job.env === 'staging'
                                              ? 'ciEnvStaging'
            ...
            <div
              className={`w-72 flex-shrink-0 border-l ${t.border.subtle} flex flex-col overflow-hidden`}
            >
              {showYaml ? (
                <div className="flex flex-col h-full">
                  ...
                  <pre
                    className={`flex-1 overflow-auto p-3 font-mono text-[8px] ${t.isDark ? 'text-emerald-300/70' : 'text-slate-600'} ${t.scrollbar}`}
                  >
                    {yaml}
                  </pre>
                </div>
                ...
                <div className="flex flex-col h-full">
                  ...
                  >
                    ...
                    <div className="flex gap-1">
                      {(sel.status === 'failed' || sel.status === 'cancelled') && (
                        <button
                          onClick={() => retryJob(sel.id)}
                          className={`flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[8px] ${t.transition} ${t.interactive.iconBtn}`}
                        >
                      ...
                      {sel.status === 'running' && (
                        <button
                          onClick={() => cancelJob(sel.id)}
                          className={`flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[8px] text-red-400 ${t.transition} ${t.interactive.iconBtn}`}
                        >
                  ...
                  {/* Status */}
                  <div
                    className={`flex items-center gap-2 px-3 py-1.5 border-b ${t.border.subtle}`}
                  >
                    {(() => {
                      const SI = STATUS_CONFIG[sel.status].icon;
                      return (
                        <SI
                          className={`w-3 h-3 ${sel.status === 'running' ? 'animate-spin' : ''}`}
                          style={{ color: STATUS_CONFIG[sel.status].color }}
                        />
                      );
                    })()}
                    <span
                      className="text-[9px]"
                      style={{ color: STATUS_CONFIG[sel.status].color, fontWeight: 500 }}
                    >
                      {(i as unknown as Record<string, string>)[STATUS_CONFIG[sel.status].labelKey]}
                    </span>
                    {sel.duration > 0 && (
                      <span className={`text-[8px] ${t.text.dimmed}`}>
                        {i.ciDuration}: {formatDuration(sel.duration)}
                      </span>
                    )}
                  </div>
                  {/* Log output */}
                  <div
                    className={`flex-1 overflow-auto p-3 font-mono text-[8px] ${t.isDark ? 'bg-[#0a0f1f] text-slate-400' : 'bg-slate-50 text-slate-600'} ${t.scrollbar}`}
                  >
                    {sel.logs.length === 0 ? (
                      ...
                      sel.logs.map((line, _idx) => (
                        <div
                          key={_idx}
                          className={`py-0.5 ${line.startsWith('✓') ? 'text-emerald-400' : line.startsWith('✗') ? 'text-red-400' : ''}`}
                        >
                          {line}
                        </div>
                      ))
