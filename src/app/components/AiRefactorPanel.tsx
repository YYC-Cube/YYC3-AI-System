...
async function analyzeWithAI(file: string, code: string): Promise<RefactorSuggestion[]> {
  ...
  optResult.improvements.forEach((imp: CodeImprovement, idx: number) => {
    const severity: Severity =
      imp.type === 'security' ? 'high' : imp.type === 'performance' ? 'medium' : 'low';
    ...
    suggestions.push({
      ...
      title: imp.reason,
      titleKey:
        refType === 'optimize-perf'
          ? 'arOptimizePerf'
          : refType === 'add-types'
            ? 'arAddTypes'
            : refType === 'improve-naming'
              ? 'arImproveNaming'
              : 'arSimplifyLogic',
...
function generateFallbackSuggestions(file: string): RefactorSuggestion[] {
  ...
  if (file.includes('ChatInterface') || file.includes('chat')) {
    base.push(
      {
        ...
        severity: 'high',
        titleKey: 'arExtractComponent',
        title: 'Extract MessageBubble into standalone component',
        description: 'The rendering logic is complex enough to be a separate component.',
        line: 115,
        endLine: 118,
        confidence: 92,
        before: '{messages.map(msg => <MessageBubble />)}',
      ...
      },
      {
        ...
        type: 'simplify-logic',
        severity: 'medium',
        titleKey: 'arSimplifyLogic',
        title: 'Simplify slash command dispatch',
        description: 'Replace switch with Record lookup table.',
        line: 88,
        endLine: 100,
        confidence: 87,
        before: 'switch (command) { ... }',
        after: 'const COMMANDS: Record<string, () => void> = { ... }',
      }
    );
    ...
    base.push({
      ...
      type: 'remove-unused',
      severity: 'high',
      titleKey: 'arRemoveUnused',
      title: 'Remove reference to undefined initialDesignRoot',
      description: 'initialDesignRoot is used but not defined.',
      line: 35,
      endLine: 35,
      confidence: 98,
      before: 'designRoot: initialDesignRoot,',
      after: 'designRoot: { version: "1.0.0", theme: "dark" },',
    });
  }
  return base;
}
...
export function AiRefactorPanel() {
  ...
  const getSeverityLabel = (s: Severity) => {
    switch (s) {
      case 'high':
        return i.arSeverityHigh;
      case 'medium':
        return i.arSeverityMedium;
      case 'low':
        return i.arSeverityLow;
    }
  };
      ...
      <div className={`flex items-center justify-between px-3 py-2 border-b ${t.border.subtle}`}>
        <div className="flex items-center gap-2">
          ...
          {!analyzing && visibleSuggestions.length > 0 && (
            <span
              className={`text-[9px] px-1.5 py-0.5 rounded-full ${t.isDark ? 'bg-violet-500/15 text-violet-400' : 'bg-violet-50 text-violet-500'}`}
            >
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className={`flex items-center gap-2 mt-0.5 text-[8px] ${t.text.dimmed}`}>
                        ...
                        <span>
                          {i.edLine} {s.line}-{s.endLine}
                        </span>
                        <span>
                          {i.arConfidence}: {s.confidence}%
                        </span>
                      </div>
                    </div>
                    {isApplied && (
                      <Check className={`w-3.5 h-3.5 flex-shrink-0 ${t.status.success}`} />
                    )}
                  </button>
                    ...
                    >
                      <div className={`px-3 pb-3 space-y-2`}>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <div>
                            <div
                              className={`text-[8px] uppercase tracking-wider mb-1 ${t.status.error}`}
                              style={{ fontWeight: 600 }}
                            >
                              Before
                            </div>
                            <pre
                              className={`text-[9px] p-2 rounded-lg overflow-x-auto font-mono ${t.isDark ? 'bg-red-500/5 text-red-300/70' : 'bg-red-50 text-red-700/70'}`}
                            >
                              {s.before}
                            </pre>
                          </div>
                          <div>
                            <div
                              className={`text-[8px] uppercase tracking-wider mb-1 ${t.status.success}`}
                              style={{ fontWeight: 600 }}
                            >
                              After
                            </div>
                            <pre
                              className={`text-[9px] p-2 rounded-lg overflow-x-auto font-mono ${t.isDark ? 'bg-emerald-500/5 text-emerald-300/70' : 'bg-emerald-50 text-emerald-700/70'}`}
                            >
                              {s.after}
                            </pre>
                          </div>
