...
interface AppState {
  ...
  // ── Layout Manager ──
  savedLayouts: { id: string; name: string; panelMap: { left: string; middle: string; right: string }; viewMode: ViewMode; isDefault: boolean; createdAt: number }[]
  ...
  collaborators: { id: string; name: string; color: string; cursor: { file: string; line: number } | null; online: boolean }[]
  setCollaborators: (c: { id: string; name: string; color: string; cursor: { file: string; line: number } | null; online: boolean }[]) => void
...
const initialDesignRoot: DesignRoot = {
  ...
  tokens: '',
  panels: [
    { id: 'left-panel', type: 'container', layout: { x: 0, y: 0, w: 25, h: 100, minW: 15, maxW: 40 }, style: {} },
    { id: 'mid-panel', type: 'container', layout: { x: 25, y: 0, w: 45, h: 100, minW: 25, maxW: 60 }, style: {} },
    { id: 'right-panel', type: 'container', layout: { x: 70, y: 0, w: 30, h: 100, minW: 20, maxW: 50 }, style: {} },
  ],
  components: [],
...
const defaultProjects: ProjectMeta[] = [
  { id: 'p1', name: 'CloudPivot Dashboard', description: 'projDescDashboard', updatedAt: Date.now() - 3600000, status: 'active', color: '#6366f1' },
  { id: 'p2', name: 'YYC3 Design System', description: 'projDescDesignSystem', updatedAt: Date.now() - 86400000, status: 'active', color: '#3b82f6' },
  { id: 'p3', name: 'AI Chat Module', description: 'projDescAiChat', updatedAt: Date.now() - 172800000, status: 'draft', color: '#14b8a6' },
  { id: 'p4', name: 'Code Generator', description: 'projDescCodeGen', updatedAt: Date.now() - 604800000, status: 'archived', color: '#f59e0b' },
]
    ...
    (set) => ({
      ...
      setTerminalHeight: (h) => {
        ...
        if (!debouncedTerminalHeightUpdate) {
          debouncedTerminalHeightUpdate = debounce((height: number) => {
      ...
      addMessage: (msg) =>
        set((state) => {
          const newMessage = { ...msg, id: Math.random().toString(36).substring(2, 11), timestamp: Date.now() }
      ...
      updatePanel: (panelId, updates) => {
        if (!debouncedPanelUpdate) {
          debouncedPanelUpdate = debounce((pid: string, upd: Partial<PanelSpec>) => {
            set((state) => ({
              designRoot: {
                ...state.designRoot,
      ...
      addProject: (project) =>
        set((state) => ({
          recentProjects: [{ ...project, id: Math.random().toString(36).substring(2, 11) }, ...state.recentProjects]
        })),
      removeProject: (id) =>
      ...
      setModelStatus: (id, status, result) => {
        ...
        set((state) => ({
          aiModels: state.aiModels.map(m =>
            m.id === id ? { ...m, status, lastTestResult: result ?? m.lastTestResult, lastTestTime: Date.now() } : m
          )
        }))
      },
      ...
      customThemeConfig: {
        ...
        type: 'dark',
        colors: { primary: '#6366f1', secondary: '#3b82f6', accent: '#f97316', background: '#0f172a', card: '#1e293b', border: '#334155' },
        fonts: { sans: 'Inter, -apple-system, BlinkMacSystemFont, sans-serif', mono: "'Fira Code', monospace" },
        radius: { sm: '8px', md: '12px', lg: '16px' },
        branding: { appName: 'YYC³ CloudPivot Intelli-Matrix', slogan: '言启象限 | 语枢未来', subSlogan: 'Words Initiate Quadrants, Language Serves as Core for Future' }
      },
      updateCustomThemeConfig: (updates) => {
        if (!debouncedThemeConfigUpdate) {
          debouncedThemeConfigUpdate = debounce((upd: Partial<CustomThemeConfig>) => {
      ...
      updateCustomColors: (colors) => {
        if (!debouncedColorsUpdate) {
          debouncedColorsUpdate = debounce((cols: Partial<CustomThemeConfig['colors']>) => {
            set((state) => ({ customThemeConfig: { ...state.customThemeConfig, colors: { ...state.customThemeConfig.colors, ...cols } } }))
          }, { delay: 100, leading: true, trailing: true })
        }
      ...
      resetCustomThemeConfig: () =>
        set(() => ({
          customThemeConfig: {
            name: getI18n('zh').defaultThemeName, type: 'dark',
            colors: { primary: '#6366f1', secondary: '#3b82f6', accent: '#f97316', background: '#0f172a', card: '#1e293b', border: '#334155' },
            fonts: { sans: 'Inter, -apple-system, BlinkMacSystemFont, sans-serif', mono: "'Fira Code', monospace" },
            radius: { sm: '8px', md: '12px', lg: '16px' },
            branding: { appName: 'YYC³ CloudPivot Intelli-Matrix', slogan: '言启象限 | 语枢未来', subSlogan: 'Words Initiate Quadrants, Language Serves as Core for Future' }
      ...
      saveLayout: (name) =>
        set((state) => {
          const layout = { id: 'layout-' + Date.now(), name, panelMap: { ...state.panelMap }, viewMode: state.viewMode, isDefault: state.savedLayouts.length === 0, createdAt: Date.now() }
      ...
      loadLayout: (id) =>
        set((state) => {
          ...
          return { panelMap: { ...layout.panelMap }, viewMode: layout.viewMode, activeLayoutId: id }
        }),
      deleteLayout: (id) =>
        set((state) => ({ savedLayouts: state.savedLayouts.filter(l => l.id !== id), activeLayoutId: state.activeLayoutId === id ? null : state.activeLayoutId })),
      renameLayout: (id, name) =>
        set((state) => ({ savedLayouts: state.savedLayouts.map(l => l.id === id ? { ...l, name } : l) })),
      setDefaultLayout: (id) =>
        set((state) => ({ savedLayouts: state.savedLayouts.map(l => ({ ...l, isDefault: l.id === id })) })),
      ...
      setFileModified: (file, modified) =>
        set((state) => ({
          modifiedFiles: modified
            ? state.modifiedFiles.includes(file) ? state.modifiedFiles : [...state.modifiedFiles, file]
            : state.modifiedFiles.filter(f => f !== file)
        })),
      setFileError: (file, hasError) =>
        set((state) => ({
          errorFiles: hasError
            ? state.errorFiles.includes(file) ? state.errorFiles : [...state.errorFiles, file]
            : state.errorFiles.filter(f => f !== file)
        })),
      setFileReadOnly: (file, readOnly) =>
        set((state) => ({
          readOnlyFiles: readOnly
            ? state.readOnlyFiles.includes(file) ? state.readOnlyFiles : [...state.readOnlyFiles, file]
      ...
      injectCode: (filename, code, language) => set({ pendingCodeInjection: { filename, code, language } }),
      clearCodeInjection: () => set({ pendingCodeInjection: null }),

      // ── Dialogs & Panels ──
      shortcutsDialogOpen: false, setShortcutsDialogOpen: (open) => set({ shortcutsDialogOpen: open }),
      projectsPanelOpen: false, setProjectsPanelOpen: (open) => set({ projectsPanelOpen: open }),
      githubPanelOpen: false, setGithubPanelOpen: (open) => set({ githubPanelOpen: open }),
      sharePanelOpen: false, setSharePanelOpen: (open) => set({ sharePanelOpen: open }),
      deployPanelOpen: false, setDeployPanelOpen: (open) => set({ deployPanelOpen: open }),
      quickActionsPanelOpen: false, setQuickActionsPanelOpen: (open) => set({ quickActionsPanelOpen: open }),
      helpPanelOpen: false, setHelpPanelOpen: (open) => set({ helpPanelOpen: open }),
      pluginPanelOpen: false, setPluginPanelOpen: (open) => set({ pluginPanelOpen: open }),
      gitPanelOpen: false, setGitPanelOpen: (open) => set({ gitPanelOpen: open }),
      searchPanelOpen: false, setSearchPanelOpen: (open) => set({ searchPanelOpen: open }),

      // ── Command Palette ──
      commandPaletteOpen: false, setCommandPaletteOpen: (open) => set({ commandPaletteOpen: open }),

      // ── Notification Center ──
      notificationCenterOpen: false, setNotificationCenterOpen: (open) => set({ notificationCenterOpen: open }),

      // ── Advanced Feature Panels ──
      aiCodeIntelOpen: false, setAiCodeIntelOpen: (open) => set({ aiCodeIntelOpen: open }),
      activityTimelineOpen: false, setActivityTimelineOpen: (open) => set({ activityTimelineOpen: open }),
      performanceMonitorOpen: false, setPerformanceMonitorOpen: (open) => set({ performanceMonitorOpen: open }),

      // ── File Tabs ──
      openTabs: ['ChatInterface.tsx'],
      pinnedTabs: [],
      addOpenTab: (file) => set((state) => ({ openTabs: state.openTabs.includes(file) ? state.openTabs : [...state.openTabs, file] })),
      removeOpenTab: (file) => set((state) => ({ openTabs: state.openTabs.filter((tab) => tab !== file) })),
      ...
      closeOtherTabs: (file) => set((state) => ({ openTabs: state.openTabs.filter((tab) => tab === file) })),
      closeRightTabs: (file) => set((state) => ({ openTabs: state.openTabs.slice(0, state.openTabs.indexOf(file) + 1) })),
      closeAllTabs: () => set(() => ({ openTabs: [], pinnedTabs: [] })),
      togglePinTab: (file) =>
        set((state) => ({
          pinnedTabs: state.pinnedTabs.includes(file) ? state.pinnedTabs.filter((tab) => tab !== file) : [...state.pinnedTabs, file]
        })),

      // ── Git Diff Viewer ──
      gitDiffFile: null, setGitDiffFile: (file) => set({ gitDiffFile: file }),

      // ── Feature Panels ──
      templateMarketOpen: false, setTemplateMarketOpen: (open) => set({ templateMarketOpen: open }),
      visualCanvasOpen: false, setVisualCanvasOpen: (open) => set({ visualCanvasOpen: open }),
      conflictResolverOpen: false, setConflictResolverOpen: (open) => set({ conflictResolverOpen: open }),
      collabReplayOpen: false, setCollabReplayOpen: (open) => set({ collabReplayOpen: open }),
      rbacPanelOpen: false, setRbacPanelOpen: (open) => set({ rbacPanelOpen: open }),
      gitGraphOpen: false, setGitGraphOpen: (open) => set({ gitGraphOpen: open }),
      cicdPipelineOpen: false, setCicdPipelineOpen: (open) => set({ cicdPipelineOpen: open }),
      flameGraphOpen: false, setFlameGraphOpen: (open) => set({ flameGraphOpen: open }),
      envVarsOpen: false, setEnvVarsOpen: (open) => set({ envVarsOpen: open }),
      codeTranslatorOpen: false, setCodeTranslatorOpen: (open) => set({ codeTranslatorOpen: open }),
      erDiagramOpen: false, setErDiagramOpen: (open) => set({ erDiagramOpen: open }),
      apiTesterOpen: false, setApiTesterOpen: (open) => set({ apiTesterOpen: open }),
      docGeneratorOpen: false, setDocGeneratorOpen: (open) => set({ docGeneratorOpen: open }),
      workspaceManagerOpen: false, setWorkspaceManagerOpen: (open) => set({ workspaceManagerOpen: open }),
      databaseManagerOpen: false, setDatabaseManagerOpen: (open) => set({ databaseManagerOpen: open }),
      layoutManagerOpen: false, setLayoutManagerOpen: (open) => set({ layoutManagerOpen: open }),
      whiteboardOpen: false, setWhiteboardOpen: (open) => set({ whiteboardOpen: open }),
      dependencyGraphOpen: false, setDependencyGraphOpen: (open) => set({ dependencyGraphOpen: open }),
      snippetManagerOpen: false, setSnippetManagerOpen: (open) => set({ snippetManagerOpen: open }),
      pluginSystemOpen: false, setPluginSystemOpen: (open) => set({ pluginSystemOpen: open }),
      offlineCacheOpen: false, setOfflineCacheOpen: (open) => set({ offlineCacheOpen: open }),
      systemDashboardOpen: false, setSystemDashboardOpen: (open) => set({ systemDashboardOpen: open }),
      themeManagerOpen: false, setThemeManagerOpen: (open) => set({ themeManagerOpen: open }),
      multiWindowOpen: false, setMultiWindowOpen: (open) => set({ multiWindowOpen: open }),
      realtimeCollabEnhancedOpen: false, setRealtimeCollabEnhancedOpen: (open) => set({ realtimeCollabEnhancedOpen: open }),
      codeSandboxOpen: false, setCodeSandboxOpen: (open) => set({ codeSandboxOpen: open }),
      visualQueryBuilderOpen: false, setVisualQueryBuilderOpen: (open) => set({ visualQueryBuilderOpen: open }),
      taskBoardOpen: false, setTaskBoardOpen: (open) => set({ taskBoardOpen: open }),
      multiInstancePanelOpen: false, setMultiInstancePanelOpen: (open) => set({ multiInstancePanelOpen: open }),
      sandboxPreviewCode: null, sandboxPreviewLang: null,
      setSandboxPreviewCode: (code, lang) => set({ sandboxPreviewCode: code, sandboxPreviewLang: lang }),
