...
const DEMO_PEERS: WSPeer[] = [
  { id: 'peer-alice', name: 'Alice', color: '#6366f1', file: 'ChatInterface.tsx', cursor: { line: 42, col: 15 }, online: true, lastSeen: Date.now() },
  { id: 'peer-bob', name: 'Bob', color: '#f59e0b', file: 'store.ts', cursor: { line: 18, col: 8 }, online: true, lastSeen: Date.now() },
  { id: 'peer-carol', name: 'Carol', color: '#10b981', file: null, cursor: null, online: false, lastSeen: Date.now() - 300000 },
  { id: 'peer-dave', name: 'Dave', color: '#ec4899', file: 'App.tsx', cursor: { line: 7, col: 22 }, online: true, lastSeen: Date.now() },
]

const DEMO_FILES = ['ChatInterface.tsx', 'store.ts', 'App.tsx', 'types.ts', 'routes.ts', 'IDELayout.tsx', 'Header.tsx']
...
export class WSCollabManager {
  ...
  get syncStatus(): WSSyncStatus { return this._syncStatus }
  get latency(): number { return this._latency }
  get peers(): WSPeer[] { return Array.from(this._peers.values()) }
  get onlinePeers(): WSPeer[] { return this.peers.filter(p => p.online) }
  get fileLocks(): WSFileLock[] { return Array.from(this._fileLocks.values()) }
  get operationLog(): WSEditOperation[] { return this._operationLog.slice(-50) }
  get queueSize(): number { return this._messageQueue.length }
  get isRealMode(): boolean { return this._realMode }
  ...
  /* ── Edit operations ── */
  broadcastEdit(file: string, type: 'insert' | 'delete' | 'replace', line: number, col: number, content: string) {
      ...
      try {
        ...
        this._ws.onopen = () => {
          ...
          this._ws?.send(JSON.stringify({
            type: 'presence',
            payload: { userId: this._localUser.id, name: this._localUser.name, color: this._localUser.color },
            timestamp: Date.now(),
          }))
  ...
  private _handleRealMessage(msg: WSMessage) {
    switch (msg.type) {
      ...
      case 'cursor': {
        ...
        if (peer) {
          this._peers.set(c.userId, { ...peer, file: c.file, cursor: { line: c.line, col: c.col }, lastSeen: Date.now() })
          this.emit('peers-updated', this.peers)
        }
        break
      }
