...
import { getLocalStorageUsage } from './storage-monitor';
...
export function cleanupStorage(options: CleanupOptions = {}): CleanupResult {
  ...
  if (opts.aggressive) {
    ...
    try {
      ...
      const taskStoreSize =
        (TASK_STORE_KEY.length + (localStorage.getItem(TASK_STORE_KEY)?.length || 0)) * 2;
      details.push({
        key: TASK_STORE_KEY,
        action: 'removed',
        before: 1,
...
export function getCleanupPreview(): {
  ...
  // Check projects
  const projects =
    JSON.parse(localStorage.getItem(APP_STORE_KEY) || '{}')?.state?.recentProjects || [];
