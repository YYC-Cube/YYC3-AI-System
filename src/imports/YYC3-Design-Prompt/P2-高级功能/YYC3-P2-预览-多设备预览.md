...
interface DeviceConfig {
  id: string; name: string; type: DeviceType; // desktop|laptop|tablet|mobile|custom
  width: number; height: number; dpr: number; userAgent: string;
  icon: string; rotatable: boolean; orientation: 'portrait'|'landscape';
}
```
